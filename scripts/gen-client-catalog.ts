/**
 * Generate the model-facing client slot catalog consumed by
 * `dsh_cordis_inspect_query` (Client Slots). A dynamic package's browser half can only contribute UI
...
const CLIENT_NOTES: readonly string[] = [
  ...
  'This catalog is the COMPILE-TIME contract of the shipped web bundle, not a snapshot of one page: a key is registrable only where the owner that declares it is mounted. A failed registration surfaces in the browser-half load report — read it back with `dsh_cordis_inspect_self`.',
]
...
export function renderClientCatalog(entries: readonly SlotEntry[]): string {
  const lines: string[] = [
    ...
    ' * `dsh_cordis_inspect_query` (Client Slots) serves it to the model: every SlotMap key a',
