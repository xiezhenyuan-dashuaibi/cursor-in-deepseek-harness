...
CODE_PROMPT = "Use dsh_run_code to compute the packaged worker smoke value."
...
def completion_chunks(body: dict[str, object]) -> list[dict[str, object]]:
    ...
    if minimal_prompt is not None:
        ...
        if names != {"bash", "dsh_str_replace_editor"}:
    ...
    if prompt == SNAPSHOT_PROMPT:
        assert_advertised_tool(body, "dsh_cordis_define")
        return tool_call_chunks(
            "advanced-define",
            "dsh_cordis_define",
            {
    ...
    if prompt == CODE_PROMPT:
        assert_advertised_tool(body, "dsh_run_code")
        return tool_call_chunks(
            "call-code-worker",
            "dsh_run_code",
...
def minimal_tool_followup(
    ...
    if call_id == "minimal-editor" and tool_name == "dsh_str_replace_editor":
...
def advanced_tool_followup(
    ...
    if not call_id.startswith("advanced-"):
        return None
    if call_id == "advanced-define" and tool_name == "dsh_cordis_define":
        if "Defined snap-1/pkg-1 (Snapshot Double)" not in tool_text:
            raise AssertionError(f"dsh_cordis_define returned no dynamic Package ids: {tool_text}")
        if "snapshot_double" in advertised_tool_names(body):
            raise AssertionError("snapshot_double was advertised before dsh_cordis_run")
        assert_advertised_tool(body, "dsh_cordis_run")
        return tool_call_chunks(
            "advanced-run",
            "dsh_cordis_run",
            {"pluginId": "snap-1", "packageId": "pkg-1", "mode": "run"},
        )
    if call_id == "advanced-run" and tool_name == "dsh_cordis_run":
        if "snap-1/pkg-1 is running (run-1)" not in tool_text:
            raise AssertionError(f"dsh_cordis_run returned no running Package ids: {tool_text}")
        assert_advertised_tool(body, "dsh_run_code")
    ...
    if call_id == "advanced-code" and tool_name == "dsh_run_code":
        if "42" not in tool_text:
            raise AssertionError(f"dsh_run_code returned no dynamic-tool value: {tool_text}")
    ...
    if call_id == "advanced-workflow" and tool_name == "workflow":
        ...
        assert_advertised_tool(body, "dsh_cordis_undefine")
        return tool_call_chunks(
            "advanced-undefine",
            "dsh_cordis_undefine",
            {"pluginId": "snap-1"},
        )
    if call_id == "advanced-undefine" and tool_name == "dsh_cordis_undefine":
        if "Removed dynamic Plugin snap-1 and all of its Packages." not in tool_text:
            raise AssertionError(f"dsh_cordis_undefine returned no removal result: {tool_text}")
        if "snapshot_double" in advertised_tool_names(body):
            raise AssertionError("snapshot_double remained advertised after dsh_cordis_undefine")
