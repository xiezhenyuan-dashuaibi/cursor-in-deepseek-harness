...
/** Attachment seam marker that makes the attachments-conditional `dsh_read_image` schema harvestable. */
...
export interface ToolPackage {
   ...
   * model-facing tool (`dsh_run_code`, registered under a non-native `mode`), so
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    ...
    note:
      'dsh_ask_user_question pauses the tool call until the active UI provider returns a human answer.',
  },
  {
    ...
    // The registry's OWN tool: dsh_run_code exists only under a non-native mode
  ...
  },
  {
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@deepseek-ai/dsh-pwsh-local` backs `ctx.shell`); it mirrors the dsh_bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@deepseek-ai/dsh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
  ...
  },
  {
    ...
    requires: ['ctx.tools', 'ctx.fs', 'ctx.systemPrompt', 'ctx.attachments (dsh_read_image registration)', 'ctx.llm + an image-capable route (dsh_read_image execution)'],
    writes: ['tool/call', 'fs/write-intent or fs/edit-intent for mutations', 'fs/observed after read presence/absence or successful file operation', 'durable attachment (dsh_read_image)', 'tool/result'],
    async mount(ctx) {
      ...
      // the attachments-conditional dsh_read_image schema without attachment I/O.
      await ctx.plugin(LocalFileSystem)
      await ctx.plugin(CatalogAttachmentStore)
      await ctx.plugin(ToolFs)
    },
    note:
      'The read-before-write/edit policy is added by `@deepseek-ai/dsh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `dsh_read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
  ...
  },
  {
    ...
    note:
      'The six terminal tools are opt-in and complement one-shot shell/filesystem tools. `dsh_terminal_send(run_in_background: true)` registers with `ctx.jobs`; TUI, named key sequences, BEL, resize, auto-start, and cross-agent sharing are absent from the schema.',
  },
  {
  ...
  },
  {
    ...
    shippedNames: ['dsh_subagent', 'dsh_subagent_fork'],
    async mount(ctx) {
      await ctx.plugin(SubagentRuntime)
      registerCatalogSubagentProvider(ctx, 'mock')
    ...
    note:
      'The registered tool name is the load-time `toolName` config (default `dsh_subagent`); the schema above is that default. The shipped compositions load this package once per subagent backend, so the model additionally sees `dsh_subagent_fork` bound to the fork backend. Each instance\'s description, `run_in_background` parameter, and system-prompt policy follow its own `backgroundMode` and `enableRunInBackground`, so the two shipped schemas are not identical: `dsh_subagent` is `continuable` and defaults omitted calls to background with automatic settlement delivery, while `dsh_subagent_fork` stays `one-shot` and defaults them to foreground — see `packages/bundle/base/cordis.patch.yml` and `examples/acp-agent/cordis.yml`.',
  },
  {
    ...
    requires: ['ctx.tools', 'ctx.subagents', 'ctx.agents and ctx.sessionProjections (dsh_list_agents only)'],
    ...
    note:
      'The globally named control tools over continuable background subagents: provider-bound `tool-subagent` instances register distinct delegation tools, while this package registers `dsh_send_message` and `dsh_interrupt_agent` once, plus `dsh_list_agents` from its separately loaded `/list-agents` plugin (whose catalog rows use the sessionProjections and live Agent registries).',
  },
  {
    ...
    note:
      ...
      + '`dsh_send_message` tool is installed independently.',
  },
  {
  ...
  },
  {
    ...
    note:
      'dsh_web_search and dsh_web_fetch keep provider selection behind ctx.web so model-visible schemas stay stable across backend swaps.',
  },
]
...
/** Harvested and shipped model-facing names. MCP-imported remotes are outside this catalog. */
const SHIPPED_MODEL_TOOL_NAME = /^dsh_[a-z0-9_]+$/

/**
 * Reject a harvested or shipped alias that is not a `dsh_*` model-facing name.
 * @param pkg - catalog package id, for the error.
 * @param name - candidate tool name.
 */
export function assertShippedModelToolName(pkg: string, name: string): void {
  if (SHIPPED_MODEL_TOOL_NAME.test(name)) return
  throw new Error(
    `gen-tool-catalog: ${pkg} harvested tool "${name}" is not a dsh_* model-facing name `
    + '(expected /^dsh_[a-z0-9_]+$/). MCP-imported remote tools are outside this catalog; '
    + 'test fixtures such as echo are not harvested.',
  )
}
...
export async function collectToolCatalog(packages: ToolPackage[] = TOOL_PACKAGES): Promise<ToolCatalog> {
  ...
  for (const entry of packages) {
    ...
    try {
      ...
      for (const schema of schemas) assertShippedModelToolName(entry.pkg, schema.name)
      for (const shipped of entry.shippedNames ?? []) assertShippedModelToolName(entry.pkg, shipped)
      catalog.push({
        pkg: entry.pkg,
