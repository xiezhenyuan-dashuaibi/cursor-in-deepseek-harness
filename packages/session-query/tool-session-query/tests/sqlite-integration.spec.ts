...
describe('tool-session-query with the real SQLite provider', () => {
  it('searches live prior-step history and a persisted same-workspace log', { timeout: 20_000 }, async () => {
    ...
    const sessions = await execute('dsh_session_search', { query: 'persisted integration needle' })
    ...
    const persistedEvents = await execute('dsh_session_event_search', {
    ...
    expect(persistedEvents.content.map(block => block.type === 'text' ? block.text : '').join('\n'))
      .toContain('seq 0')
    const liveEvents = await execute('dsh_session_event_search', { query: 'live integration needle' })
