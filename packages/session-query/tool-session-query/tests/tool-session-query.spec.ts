...
describe('registration and schemas', () => {
  it('registers the five cursor-free tools, prompt, timeouts, and pure generic presenters, then disposes them', async () => {
    ...
    const sessionSchema = mounted.ctx.tools.schemas().find(schema => schema.name === 'dsh_session_search')
    ...
    expect(mounted.ctx.tools.get('dsh_session_search')?.timeoutMs).toBe(1234)
    expect(mounted.ctx.tools.get('dsh_session_trace')?.timeoutMs).toBeUndefined()
    const parallelArgs: Record<string, unknown> = {
      dsh_session_trace: {},
      dsh_session_event_trace: { seq: 0 },
      dsh_session_event_read: { seq: 0 },
    }
    for (const [name, args] of Object.entries(parallelArgs)) {
      expect(mounted.ctx.tools.get(name)?.isConcurrencySafe?.(args)).toBe(true)
    }
    expect(mounted.ctx.tools.get('dsh_session_search')?.output.render({}, 'rendered'))
      .toEqual([{ type: 'text', text: 'rendered' }])
    expect(mounted.ctx.tools.get('dsh_session_search')?.presentCall?.({ query: 'needle' }))
      .toEqual({ card: 'generic', kind: 'search', title: 'Search prior sessions', rawInput: 'needle' })
    expect(mounted.ctx.tools.get('dsh_session_event_search')?.presentCall?.({ query: 'needle' }))
      .toEqual({ card: 'generic', kind: 'search', title: 'Search session events', rawInput: 'needle' })
    expect(mounted.ctx.tools.get('dsh_session_trace')?.presentCall?.({}))
      .toEqual({ card: 'generic', kind: 'read', title: 'Trace current session' })
    expect(mounted.ctx.tools.get('dsh_session_trace')?.presentCall?.({ session_id: 'other' }))
      .toEqual({ card: 'generic', kind: 'read', title: 'Trace session other', rawInput: 'other' })
    expect(mounted.ctx.tools.get('dsh_session_event_trace')?.presentCall?.({ session_id: 'other', seq: 3 }))
      .toEqual({
        card: 'generic',
        kind: 'read',
        title: 'Trace event 3',
    ...
    expect(mounted.ctx.tools.get('dsh_session_event_read')?.presentCall?.({ seq: 4 }))
  ...
  it('keeps generation-bound searches exclusive while exact observations remain parallel', async () => {
    ...
    const classifications = [
      ['dsh_session_search', { query: 'q' }, 'exclusive'],
      ['dsh_session_event_search', { query: 'q' }, 'exclusive'],
      ['dsh_session_trace', {}, 'parallel'],
      ['dsh_session_event_trace', { seq: 0 }, 'parallel'],
      ['dsh_session_event_read', { seq: 0 }, 'parallel'],
    ] as const
...
describe('input validation and translation', () => {
  it.each([
    ...
    const mounted = await mount()
    const result = await mounted.call('dsh_session_search', args)
  ...
  it('normalizes the query and compiles inclusive session/event filters with one parent OR clause', async () => {
    ...
    await mounted.call('dsh_session_search', {
      query: '  alpha   beta ',
      session_ids: ['a', 'b'],
  ...
  it('maps exact same-millisecond decimal bounds to adjacent numeric values without collapsing the interval', async () => {
    ...
    const result = await mounted.call('dsh_session_search', {
      query: 'q',
  ...
  it('compares unequal-length exact remainders with implicit trailing decimal zeroes', async () => {
    const mounted = await mount()
    const ordered = await mounted.call('dsh_session_search', {
      query: 'q',
    ...
    const reversed = await mounted.call('dsh_session_search', {
      query: 'q',
  ...
  it('maps fractional bounds correctly across zero and for negative pre-epoch milliseconds', async () => {
    ...
    await mounted.call('dsh_session_event_search', {
      query: 'q',
...
describe('workspace authority and lineage redaction', () => {
  it('fails closed without an agent and for direct cross-workspace targets', async () => {
    ...
    const missing = await mounted.ctx.tools.execute({
      name: 'dsh_session_trace',
      arguments: {},
      callId: CallId('missing-agent'),
      signal: new AbortController().signal,
    })
    expect(errorCode(missing)).toBe('SESSION_QUERY_TOOL_MISSING_AGENT')
    const denied = await mounted.call('dsh_session_event_read', { session_id: 'outside', seq: 0 })
  ...
  it('allows only self for a null-cwd caller and denies cross-session search', async () => {
    ...
    const own = await mounted.call('dsh_session_trace', {})
    expect(own.isError).toBe(false)
    expect(text(own)).toContain('Session caller')
    expect(errorCode(await mounted.call('dsh_session_search', { query: 'q' })))
      .toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
    createSession(mounted.ctx, 'other', undefined)
    expect(errorCode(await mounted.call('dsh_session_trace', { session_id: 'other' })))
      .toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
  })
  ...
  it('makes hidden and nonexistent parent guesses indistinguishable without calling search', async () => {
    ...
    const hidden = await mounted.call('dsh_session_search', {
      query: 'needle',
      parent_session_ids: [hiddenParent.id],
    })
    const missing = await mounted.call('dsh_session_search', {
      query: 'needle',
  ...
  it('sanitizes direct-target authorization failures before event search', async () => {
    ...
    const result = await mounted.call('dsh_session_event_search', {
  ...
  it('preserves parent-preauthorization cancellation and waits for cleanup without logging it', async () => {
    ...
    const pending = mounted.call('dsh_session_search', {
      query: 'needle',
      parent_session_ids: ['guessed-parent'],
  ...
  it('redacts an unauthorized ancestor and prunes unauthorized descendant subtrees without hidden ids', async () => {
    ...
    const result = await mounted.call('dsh_session_trace', { session_id: target.id })
  ...
  it.each([
    ...
    if (toolName === 'dsh_session_event_trace') {
  ...
  it.each([
    ...
    const args = toolName === 'dsh_session_trace'
      ? { session_id: target.id }
  ...
  it('renders branching descendants in source preorder with one indented marker per pruned subtree', async () => {
    ...
    const output = text(await mounted.call('dsh_session_trace', { session_id: target.id }))
  ...
  it('renders authorized ancestors and an unresolved lineage boundary without leaking it', async () => {
    ...
    const complete = text(await mounted.call('dsh_session_trace', { session_id: target.id }))
    ...
    const redacted = text(await mounted.call('dsh_session_trace', { session_id: incomplete.id }))
  ...
  it('rejects every payload observation whose target moved after pre-authorization', async () => {
    ...
    const search = await mounted.call('dsh_session_event_search', {
      session_id: target.id,
      query: 'secret',
    })
    ...
    expect(errorCode(await mounted.call('dsh_session_trace', { session_id: target.id })))
      .toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
    ...
    expect(errorCode(await mounted.call('dsh_session_event_trace', { session_id: target.id, seq: 0 })))
      .toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
    ...
    expect(errorCode(await mounted.call('dsh_session_event_read', { session_id: target.id, seq: 0 })))
      .toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
    ...
    const titled = await mounted.call('dsh_session_search', { query: 'safe' })
  ...
  it('rejects a default self read when its same-id observation moved after caller capture', async () => {
    ...
    const denied = await mounted.call('dsh_session_event_read', { seq: secret.seq })
    expect(errorCode(denied)).toBe('SESSION_QUERY_TOOL_UNAUTHORIZED')
...
describe('search paging, prior-history bounds, titles, and cancellation', () => {
  it('drains hidden internal pages to the authorized non-self cap and masks an unauthorized parent id', async () => {
    ...
    const result = await mounted.call('dsh_session_search', { query: 'needle' })
    const output = text(result)
  ...
  it('does not report a cap when only rejected hits remain after the authorized limit', async () => {
    ...
    const output = text(await mounted.call('dsh_session_search', { query: 'needle' }))
  ...
  it.each([
    ...
    {
      toolName: 'dsh_session_event_search',
      args: { query: 'needle' },
      secrets: [
    ...
    const mounted = await mount()
    if (toolName === 'dsh_session_search') {
  ...
  it('does not downgrade cancellation during title enrichment', async () => {
    ...
    const pending = mounted.call('dsh_session_search', { query: 'needle' }, { signal: controller.signal })
    await batchStarted
    controller.abort(cancellation)
    const result = await pending
...
describe('trace and exact read rendering', () => {
  ...
  it('renders every event relationship sequence and a UTC target timestamp', async () => {
    ...
    const result = await mounted.call('dsh_session_event_trace', { session_id: session.id, seq: 0 })
  ...
  it('renders unabridged fenced target JSON and readable semantic or log-only neighbor summaries', async () => {
    ...
    const result = await mounted.call('dsh_session_event_read', {
  ...
  it('renders empty event relationships and neighbors without semantic text', async () => {
    ...
    const trace = text(await mounted.call('dsh_session_event_trace', {
      session_id: session.id,
      seq: 0,
    }))
    ...
    const onlyAfter = text(await mounted.call('dsh_session_event_read', {
      session_id: session.id,
      seq: 0,
      after: 1,
    }))
    ...
    const onlyBefore = text(await mounted.call('dsh_session_event_read', {
      session_id: session.id,
      seq: 1,
      before: 1,
    }))
  ...
  it.each([
    ['dsh_session_event_trace', { seq: -1 }],
    ['dsh_session_event_read', { seq: Number.MAX_SAFE_INTEGER + 1 }],
    ['dsh_session_event_read', { seq: 0, before: -1 }],
    ['dsh_session_event_read', { seq: 0, after: 1.5 }, 'INVALID_ARGS'],
