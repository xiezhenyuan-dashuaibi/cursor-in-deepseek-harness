...
/**
 * Register the `dsh_todo_write` tool on `ctx.tools` and, when the session-projection seam is composed,
 * the `todos` unit.
  ...
  ctx.tools.register(defineTool({
    ...
    execute(args, exec) {
      ...
      if (!exec.agent) {
        ...
        throw new Error('dsh_todo_write requires an owning agent session')
      }
