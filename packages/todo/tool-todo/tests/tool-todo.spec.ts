 ...
 * and invokes the registered `dsh_todo_write` tool through `ctx.tools.execute`,
...
describe('dsh-tool-todo', () => {
  it('registers a `dsh_todo_write` tool whose schema is an array of {content,status}', async () => {
    const ctx = await setup(true)
    const schema = ctx.tools.schemas().find(s => s.name === 'dsh_todo_write')
  ...
  it('appends a todo/write event carrying the whole list to the calling session', async () => {
    ...
    if (result.isError) throw new Error('expected dsh_todo_write success')
  ...
  describe('allowParallelInProgress', () => {
    ...
    it('instructs the model to keep at most one active, while true instructs parallel', async () => {
      ...
      const singleDesc = single.tools.schemas().find(s => s.name === 'dsh_todo_write')!.description
      expect(singleDesc).toContain('Keep AT MOST ONE todo `in_progress`')
      expect(singleDesc).not.toContain('several at once')

      const parallelDesc = (await setup(true)).tools.schemas().find(s => s.name === 'dsh_todo_write')!.description
  ...
  it('presents the call with a stable title and the list as raw input', async () => {
    const ctx = await setup(true)
    const def = ctx.tools.get('dsh_todo_write')!
  ...
  it('unregisters the tool when its contributing fiber is disposed (HMR-safety)', async () => {
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_todo_write')).toBe(true)
    await fiber.dispose()
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_todo_write')).toBe(false)
  })
