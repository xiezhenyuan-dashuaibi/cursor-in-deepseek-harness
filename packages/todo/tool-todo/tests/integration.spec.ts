...
/**
 * Full-loop integration: a scripted mock model drives the REAL dsh_todo_write tool
...
describe('dsh_todo_write tool through the agent loop', () => {
  it('model calls dsh_todo_write: a tool/call, a non-error tool/result, and a todo/write snapshot land', async () => {
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'dsh_todo_write', {
        todos: [
    ...
    expect(findEvent(log, 'tool/call').data.name).toBe('dsh_todo_write')
  ...
  it('a second dsh_todo_write replaces the list (last-write-wins on the log)', async () => {
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'dsh_todo_write', { todos: [{ content: 'step one', status: 'in_progress' }] }),
      toolCallResponse('call-2', 'dsh_todo_write', {
        todos: [
