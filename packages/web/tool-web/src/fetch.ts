/**
 * The model-facing `dsh_web_fetch` tool. This module owns its schema, validation, and presentation;
 ...
 *
 * @param args - the schema-validated `dsh_web_fetch` arguments.
...
/**
 * The `dsh_web_fetch` tool's private `tool/result` `meta` payload: the fetch summary
...
/**
 * Project a validated `dsh_web_fetch` output value into its replayable presentation
 ...
 *
 * @param value - the canonical `dsh_web_fetch` output value (the seam's result shape).
...
/**
 * Register the `dsh_web_fetch` tool and its system-prompt guidance.
 *
...
export function applyWebFetchTool(ctx: Context, timeoutMs: number, maxOutputChars: number): void {
  ctx.systemPrompt.section({
    name: 'tool:dsh_web_fetch',
    order: 111,
    text: 'Use the dsh_web_fetch tool to retrieve the content of a specific HTTP(S) URL (for example a result from dsh_web_search). It returns the page content decoded to text. Cite the URL as a markdown link when you use its content.',
  })
