...
describe('dsh_web_fetch integration over the real backend', () => {
  it('fetches an html page and renders it to markdown', async () => {
    const out = await call('dsh_web_fetch', { url: base })
  ...
  it('surfaces WEB_INVALID_URL as a structured tool error', async () => {
    const out = await call('dsh_web_fetch', { url: 'ftp://example.com' })
...
describe('dsh_web_search integration over the real Exa provider', () => {
  it('runs dsh_web_search end-to-end and formats the provider result', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response(
      JSON.stringify({ results: [{ url: 'https://result.test', title: 'Result', highlights: ['a highlight'] }] }),
      { status: 200, headers: { 'content-type': 'application/json' } },
    )))
    const out = await call('dsh_web_search', { query: 'deepseek-official' })
    expect(out.isError).toBe(false)
...
describe('tool-call timeout policy over the migrated web tools', () => {
  it('neither model schema exposes a timeout parameter after the migration', () => {
    ...
    const fetchParams = byName.get('dsh_web_fetch')!.parameters as { properties: Record<string, unknown> }
    const searchParams = byName.get('dsh_web_search')!.parameters as { properties: Record<string, unknown> }
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    const out = await tctx.tools.execute({ signal: testToolSignal, callId: CallId('slow-1'), name: 'dsh_web_fetch', arguments: { url: slowBase } })
    expect(out.isError).toBe(true)
