...
describe('dsh_web_search presentation meta and result view', () => {
...
describe('dsh_web_fetch presentation meta and result view', () => {
  const NO_CAP = 1_000_000
...
describe('tool-web registration', () => {
  it('registers both tools by default', async () => {
    ...
    expect(names).toContain('dsh_web_fetch')
    expect(ctx.tools.executionMode({ signal: testToolSignal, callId: CallId('search-safe'), name: 'dsh_web_search', arguments: { query: 'q' } }))
      .toEqual({ kind: 'parallel' })
    expect(ctx.tools.executionMode({ signal: testToolSignal, callId: CallId('fetch-safe'), name: 'dsh_web_fetch', arguments: { url: 'https://a.test' } }))
      .toEqual({ kind: 'parallel' })
    await fiber.dispose()
  ...
  it('registers only enabled tools', async () => {
    ...
    const names = ctx.tools.schemas().map(s => s.name)
    expect(names).toContain('dsh_web_search')
    expect(names).not.toContain('dsh_web_fetch')
    await fiber.dispose()
  })

  it('registers only dsh_web_fetch when search is disabled', async () => {
  ...
  it('registers dsh_web_search even when no provider is available (schema follows enablement, not availability)', async () => {
    ...
    const out = await call('dsh_web_search', { query: 'q' })
  ...
  it('contributes prompt sections for the enabled tools', async () => {
    ...
    expect(text).toContain('Use the dsh_web_search tool to discover current information on the web. It returns an optional answer plus a list of source URLs. Follow up with dsh_web_fetch when you need the full content of a specific result, and cite the relevant URLs as markdown links.')
    expect(text).toContain('Use the dsh_web_fetch tool to retrieve the content of a specific HTTP(S) URL')
    await fiber.dispose()
  })

  it('does not advertise dsh_web_fetch in search-only prompt guidance', async () => {
    const { fiber, ctx } = await mountTools({ config: { search: true, fetch: false } })
    const prompt = await ctx.systemPrompt.assemble()
    const text = prompt.sections.map(s => s.text).join('\n')
    expect(text).toContain('Use the returned source snippets when available')
    expect(text).not.toContain('dsh_web_fetch')
    await fiber.dispose()
  })
})

describe('tool-web execution through the real registry', () => {
  it('executes dsh_web_search and formats the result', async () => {
  ...
  it('projects the search sources into the tool result meta and derives its web/search view', async () => {
    ...
    const view = ctx.tools.get('dsh_web_search')?.presentResult?.({ query: 'q' }, { content: out.content, isError: out.isError, ...out.meta !== undefined ? { meta: out.meta } : {} })
  ...
  it('projects the fetch summary into the tool result meta and derives its web/fetch view', async () => {
    ...
    const out = await call('dsh_web_fetch', { url: 'https://a.test' })
    expect(out.meta).toEqual({ url: 'https://a.test', statusCode: 200, truncated: true })
    const view = ctx.tools.get('dsh_web_fetch')?.presentResult?.({ url: 'https://a.test' }, { content: out.content, isError: out.isError, ...out.meta !== undefined ? { meta: out.meta } : {} })
  ...
  it('rejects invalid arguments with a structured INVALID_ARGS error', async () => {
    ...
    const out = await call('dsh_web_search', { query: 123 })
    expect(out.isError).toBe(true)
  ...
  it('executes dsh_web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    const out = await ctx.tools.execute({ callId: CallId('fetch-1'), name: 'dsh_web_fetch', arguments: { url: 'https://a.test' }, signal: controller.signal })
    expect(out.isError).toBe(false)
  ...
  it('forwards the required caller signal to dsh_web_fetch', async () => {
    ...
    const { ctx, fiber } = await mountTools({ webConfig: { fetchProvider: 'stub-fetch' }, fetchProvider })
    const out = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('fetch-2'), name: 'dsh_web_fetch', arguments: { url: 'https://a.test' } })
    expect(out.isError).toBe(false)
    expect(out.value).toEqual({
      url: 'https://a.test',
      statusCode: 200,
  ...
  it('executes dsh_web_search, forwarding the abort signal to the seam', async () => {
    ...
    const controller = new AbortController()
    await ctx.tools.execute({ callId: CallId('search-1'), name: 'dsh_web_search', arguments: { query: 'q' }, signal: controller.signal })
    expect(seen.signal).toBe(controller.signal)
    await fiber.dispose()
  })
})
...
describe('searchMaxResults is plugin config', () => {
  it('forwards the default cap to the seam when unconfigured', async () => {
    ...
    await call('dsh_web_search', { query: 'q' })
...
describe('tool-call timeout budget is plugin config', () => {
  it('attaches the default 30s budget to dsh_web_fetch and dsh_web_search', async () => {
    const { fiber, ctx } = await mountTools()
    expect(ctx.tools.get('dsh_web_fetch')?.timeoutMs).toBe(30_000)
  ...
  it('honors per-tool timeout overrides from config', async () => {
    ...
    expect(ctx.tools.get('dsh_web_fetch')?.timeoutMs).toBe(60_000)
...
describe('fetchMaxOutputChars is plugin config', () => {
  it('bounds the rendered output of the registered dsh_web_fetch tool', async () => {
    const fetchProvider = {
      id: 'stub-fetch',
      available: () => available,
      fetch: (request: { url: string }) => Promise.resolve({
