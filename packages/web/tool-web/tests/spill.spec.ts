/**
 * Showcase integration: the real `dsh_web_fetch` tool + the real spill stack
...
/** A dsh_web_fetch call carrying a session owner (so the policy can scope the spill). */
function fetchCall(): Promise<{ isError: boolean; content: { type: string; text?: string }[] }> {
  ...
  const exec = { callId: CallId('call-1'), name: 'dsh_web_fetch', arguments: { url: base }, agent, signal: testToolSignal } as unknown as ToolExecution
  return ctx.tools.execute(exec)
}

describe('dsh_web_fetch spill showcase', () => {
