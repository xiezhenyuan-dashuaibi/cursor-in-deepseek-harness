...
| `maxUses` | `5` | Positive-integer maximum `dsh_web_search` server-tool uses per request. |
...
A separate DeepSeek model receives exactly `Perform a web search for the query: <query>` as its user text and one native `dsh_web_search` server-tool definition. This request is not part of the conversation model's context.
