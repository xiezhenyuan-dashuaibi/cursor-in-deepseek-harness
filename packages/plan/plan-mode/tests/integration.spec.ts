...
describe('plan mode through the agent loop', () => {
  it('a pre-turn set() makes the FIRST header plan-shaped, and a non-shell call is guidance-constrained only', async () => {
    const adapter = new MockAdapter([
      toolCallResponse('call-1', 'dsh_write', {}, 'Writing during plan.'),
    ...
    expect(header.data.header.tools?.map(tool => tool.name)).toEqual(['dsh_exit_plan_mode', 'read', 'write'])
  ...
  it('a user flip between turns lands at the boundary: one notice and a changed header with stable tool schemas', async () => {
    ...
    expect(first.data.header.tools?.map(tool => tool.name)).toEqual(['dsh_exit_plan_mode', 'read', 'write'])
    ...
    expect(second.data.header.tools?.map(tool => tool.name)).toEqual(['dsh_exit_plan_mode', 'read', 'write'])
