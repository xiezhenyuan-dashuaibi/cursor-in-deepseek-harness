...
Logged, per-agent plan collaboration state with deployment-owned guidance, direct `/plan [message]` entry and `/plan off` exit commands, and the reviewed `dsh_exit_plan_mode` exit. Plan mode is soft guidance; sandbox mode and approval policy enforce restrictions independently and do not read or write plan state.
...
While active, `plan:policy` renders the configured `section`. The plugin always registers `dsh_exit_plan_mode`, keeping tool schemas stable across the transition; its execute path accepts only active plan mode and leaves it only after an exact user approval through `ctx.userQuestions`.
...
- id: plan-mode
  ...
  config:
    section: |
      ...
      plan through dsh_exit_plan_mode.
```
...
```markdown
You are in plan mode. Explore and design before presenting the complete plan through dsh_exit_plan_mode.
```
...
The [`dsh_exit_plan_mode` schema](../../../docs/tool-catalog.md#deepseek-aidsh-plan-mode) remains available in both states; execution outside plan mode fails, while an approved in-mode review returns the canonical `{ approved: true }` value and renders the existing confirmation text. Rejection remains a failed call carrying review feedback, and a dismissed review a failed call naming the user's takeover.
...
- A live child owned by another agent cannot open the `dsh_exit_plan_mode` review. The failed call tells the child to include the unresolved decision in its final result; durable fork lineage alone does not prevent a session resumed as a runtime root from opening the review.
