...
describe('outer Code Mode failure capture', () => {
  it('spills the bounded output-limit diagnostic through the ordinary outer-result policy', async () => {
    ...
    expect(saved[0]?.source.toolName).toBe('dsh_run_code')
    expect(saved[0]?.content).toContain('code run failed (output-limit)')
    expect(saved[0]?.content).toContain('HEAD-')
    expect(textOf(result.content)).toContain('Full formatted result stored at: /spill/dsh_run_code.txt')
    expect(events).toEqual([])
  })
})

describe('read skip', () => {
  it('never spills the dsh_read tool result (avoids a read → spill → read loop)', async () => {
