...
function request(content: string): SaveTextSpill {
  return {
    ...
    source: { toolName: 'dsh_web_fetch', callId: CallId('c1'), label: 'result' },
    suggestedName: 'dsh_web_fetch.txt',
    content,
  }
}
...
describe('spill seam', () => {
  it('registers as ctx.spillStore and saves text', async () => {
    ...
    expect(ref).toEqual({ locator: '/stub/dsh_web_fetch.txt', bytes: 5, retrievalHint: 'Use the stub reader.' })
