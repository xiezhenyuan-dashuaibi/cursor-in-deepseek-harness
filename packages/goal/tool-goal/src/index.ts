/**
 * Model-facing `dsh_get_goal`, `dsh_create_goal`, and `dsh_update_goal` tools over the
...
function guidance(blockedAfter: number): string {
  return 'Use goal tools for one long-running completion objective in the current session. '
    + 'dsh_create_goal may infer goal intent from a direct human request in any language; do not '
    + 'create a goal for routine single-turn work. Call dsh_get_goal before dsh_update_goal and copy its '
    + 'exact goal_id and revision. After session resume or fork, an active goal is disarmed: when '
    + 'a human asks to continue or resume in any wording or language, use dsh_update_goal action '
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      goal_id: { type: 'string', required: true, description: 'Exact id returned by dsh_get_goal.' },
      revision: { type: 'number', required: true, description: 'Exact positive revision returned by dsh_get_goal.' },
      action: {
        type: 'string',
        required: true,
        enum: UPDATE_ACTIONS,
