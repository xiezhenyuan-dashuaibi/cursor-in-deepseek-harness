...
describe('goal tool registration and presentation', () => {
  it('registers three exclusive tools plus configured guidance and disposes all contributions', async () => {
    ...
    expect(['dsh_create_goal', 'dsh_get_goal', 'dsh_update_goal'].map(name => ctx.tools.get(name)?.name))
      .toEqual(['dsh_create_goal', 'dsh_get_goal', 'dsh_update_goal'])
    for (const name of ['dsh_create_goal', 'dsh_get_goal', 'dsh_update_goal']) {
    ...
    await fiber.dispose()
    expect(ctx.tools.get('dsh_get_goal')).toBeUndefined()
  ...
  it('uses args-only generic render intent and soft-fails malformed replay args', async () => {
    ...
    expect(ctx.tools.get('dsh_get_goal')?.presentCall?.({})).toEqual({
      card: 'generic', title: 'Read current goal', kind: 'read',
    })
    expect(ctx.tools.get('dsh_create_goal')?.presentCall?.({ objective: 'ship' })).toEqual({
      card: 'generic', title: 'Create goal', kind: 'other', rawInput: 'ship',
    })
    expect(ctx.tools.get('dsh_update_goal')?.presentCall?.({
    ...
    expect(ctx.tools.get('dsh_update_goal')?.presentCall?.({ wrong: true })).toBeUndefined()
  })
...
describe('goal tool execution authority', () => {
  it('lets a root model infer create intent from its accepted human turn', async () => {
    ...
    const result = await execute(ctx, 'dsh_create_goal', {
  ...
  it('rejects agentless, driverless, non-human, and live-child creation', async () => {
    const { ctx, root } = await harness()
    const agentless = await execute(ctx, 'dsh_get_goal', {})
    ...
    const nonHuman = await execute(ctx, 'dsh_create_goal', { objective: 'forged' }, root.agent)
    ...
    const childResult = await execute(ctx, 'dsh_create_goal', { objective: 'child goal' }, child.agent)
  ...
  it('rejects stale agent objects and agents outside running status through the executor', async () => {
    ...
    const staleResult = await execute(ctx, 'dsh_get_goal', {}, stale, stale)
    expect(staleResult.error?.info?.code).toBe('GOAL_TOOL_DRIVER_REQUIRED')

    root.setStatus('idle')
    const idleResult = await execute(ctx, 'dsh_get_goal', {}, root.agent)
  ...
  it('treats a fork resumed as a runtime root as direct-human authority', async () => {
    ...
    const resumed = await execute(ctx, 'dsh_update_goal', {
  ...
  it('rejects calls before a turn and after its end boundary', async () => {
    const { ctx, root } = await harness()
    const before = await execute(ctx, 'dsh_get_goal', {}, root.agent)
    expect(before.error?.info?.code).toBe('GOAL_TOOL_DRIVER_REQUIRED')

    const turn = openTurn(root, { kind: 'user' })
    closeTurn(root, turn)
    const after = await execute(ctx, 'dsh_get_goal', {}, root.agent)
  ...
  it('accepts direct human steering in a goal-sourced root turn', async () => {
    ...
    const paused = await execute(ctx, 'dsh_update_goal', {
  ...
  it('rejects an initiator different from exec.agent', async () => {
    ...
    const result = await execute(ctx, 'dsh_get_goal', {}, other.agent, root.agent)
...
describe('goal tool state transitions', () => {
  it('reads null, then edits, pauses, and resumes by exact revision in one human turn', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    expect(resultJson(await execute(ctx, 'dsh_get_goal', {}, root.agent))).toEqual({ goal: null })
    let goal = resultGoal(await execute(ctx, 'dsh_create_goal', { objective: 'old' }, root.agent))
    goal = resultGoal(await execute(ctx, 'dsh_update_goal', {
  ...
  it('injects one wrap-up instruction for an autonomous completion but leaves a human pause interactive', async () => {
    ...
    const resumed = resultGoal(await execute(ctx, 'dsh_update_goal', {
    ...
    const complete = await execute(ctx, 'dsh_update_goal', {
  ...
  it('returns structured domain and conditional-argument failures', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    const invalidCreate = await execute(ctx, 'dsh_create_goal', { objective: ' ' }, root.agent)
    ...
    const replacement = await execute(ctx, 'dsh_update_goal', {
      goal_id: created.id,
    ...
    const terminalUpdate = await execute(ctx, 'dsh_update_goal', {
      goal_id: created.id,
      revision: created.revision,
      action: 'complete',
      max_goal_rounds: 2,
    ...
    const blockedWithoutReason = await execute(ctx, 'dsh_update_goal', {
    ...
    const blockedWithEmptyReason = await execute(ctx, 'dsh_update_goal', {
    ...
    const completeWithReason = await execute(ctx, 'dsh_update_goal', {
    ...
    const editWithReason = await execute(ctx, 'dsh_update_goal', {
      goal_id: created.id,
      revision: created.revision,
      action: 'edit',
    ...
    const malformedRef = await execute(ctx, 'dsh_update_goal', {
  ...
  it('accepts only empty fillers in fields unused by the selected action', async () => {
    ...
    const edited = await execute(ctx, 'dsh_update_goal', {
      goal_id: goal.id,
    ...
    const capped = await execute(ctx, 'dsh_update_goal', {
      goal_id: goal.id,
      revision: goal.revision,
      action: 'edit',
      objective: '',
    ...
    const blocked = await execute(ctx, 'dsh_update_goal', {
      goal_id: goal.id,
      revision: goal.revision,
      action: 'blocked',
      objective: '',
  ...
  it('allows exact goal rounds to complete but not edit or pause', async () => {
    ...
    const edit = await execute(ctx, 'dsh_update_goal', {
