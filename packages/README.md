...
| [`cursor/`](cursor/README.md) | Official Cursor terminal CLI (`agent`); local install under `cli/`, plus the web PTY gateway | Product — CLI binary + host plugin |
...
| [`lsp/`](lsp/README.md) | LSP capability family: seam, generic stdio provider, and the `dsh_lsp` tool | Product — stable API |
...
| [`workflow/`](workflow/README.md) | Workflow seam, worker-thread engine, and model-facing `workflow`/`dsh_ralph` tools | Product — stable API |
...
| [`todo/`](todo/README.md) | The model-facing `dsh_todo_write` tool | Product — stable API |
