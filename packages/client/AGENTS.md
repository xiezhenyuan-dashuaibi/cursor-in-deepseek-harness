...
## Overlay frontend on a live session

Pick the presentation form, then load its skill via `dsh_skill` before writing or running `pnpm overlay:live`: card window [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md) (canonical card [`ui-float-window`](ui-float-window/README.md); always insert that package first, a later insert adds a numbered card; new body page is `pnpm overlay:new-page <name>`, then edit only that package's `Page.tsx` / locales / CSS; do not copy the card tree; declare `dsh.client.overlayBody`; dual-face host+page is one Loader row so the rail can hide/unplug the whole card); arbitrary-shape floater [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md) (canonical board [`ui-overlay-shaped`](ui-overlay-shaped/README.md); always insert that package first; new occupant is `pnpm overlay:new-shaped <name>`, occupy `overlay-shaped.body`; many occupants coexist; edit only that package's `Occupant.tsx` / locales / CSS); desktop [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md) (canonical board [`ui-overlay-desktop`](ui-overlay-desktop/README.md); always insert that package first; new page is `pnpm overlay:new-desktop <name>`, occupy `overlay-desktop.body`; exclusive one occupant). Invent look from the product brief and the generator stub. Do not search, grep, or open another overlay occupant's `Page.tsx`, `Occupant.tsx`, `*.module.css`, locales, or page tests as reference. Do not copy the currently painted desktop, card, or floater. Open another occupant's page frontend only when the user explicitly asked to make something similar to that product ([visual independence](../../.agents/notes/implemented/process/2026-09-12-overlay-occupant-visual-independence.md)). If `dsh_skill` is missing, read that form's `SKILL.md` from disk and continue. Do not occupy `overlay-card.body` for a non-card outline. Live appearance is `overlay:live` (`insert` builds `lib/` when `tsdown.config.ts` is present). Confirm the package in `window.__DSH_BOOT__`; the rail lists yaml. Do not edit `packages/bundle/web-app/cordis.patch.yml` while this overlay session is running.

Plugin inventory is [README.md](README.md). Each package README owns that plugin's description. Agent Notes and skills name mechanisms (`overlay-card.body`, `overlay-desktop.body`, `overlay-shaped.body`, live insert, card module `ui-float-window`, desktop module `ui-overlay-desktop`, shaped module `ui-overlay-shaped`), not product occupants ([description registry](../../.agents/notes/implemented/process/2026-09-05-client-plugin-description-registry.md)). After adding a new overlay package, run `pnpm install --filter ./packages/client/<name>...` (or repo-root `pnpm install`) so `react` resolves; declare `react` in that package's `package.json`. `pnpm overlay:live remove <id>` without `--keep-files` deletes that omitted checkout occupant and its landing rows so an unload cannot leave a searchable empty record. It does not delete `ui-float-window`, `ui-overlay-desktop`, `ui-overlay-shaped`, or a web-app roster package.
...
1. **Package skeleton**: for an overlay **card-window page**, run `pnpm overlay:new-page <name>` instead of cloning another client package. For an overlay **desktop page**, run `pnpm overlay:new-desktop <name>`. For an overlay **shaped occupant**, run `pnpm overlay:new-shaped <name>`. That writer also adds the inventory row, aggregate tsconfig paths, Model Experience row, and web-app omit-list entry. It does not write a feature Agent Note. For other client plugins, copy `package.json` (`@deepseek-ai/dsh-client-<name>`, exports `.`/`./invariant`/`./client`/`./src/*`/`./package.json`, `dsh.client` manifest, `files` list), `tsconfig.json` (extends `tsconfig.base.client.json`, one `references` entry per workspace dependency plus `runtime-diagnostics/invariants`), `tsdown.config.ts` (`clientBundle(id, ['lib/types/index.js', 'lib/types/invariant.js'])`), `src/index.ts` (empty node-half apply), `src/invariant.ts` (companion with a real reason), `src/css-modules.d.ts` when using CSS Modules, `README.md` with the plugin description and the Model Experience section. Add a short inventory row to [README.md](README.md) (role and whether the default web-app roster mounts it, not product copy).
2. **Workspace install**: `pnpm install --filter ./packages/client/<name>...` (or repo-root `pnpm install`) so this package's `node_modules` links `react` and workspace peers. Declare `react` in `package.json` like other client UI packages. Do not junction another package's `node_modules`.
3. **Default-roster registration is not the live path.** A running overlay session inserts with `pnpm overlay:live` through the watched profile patch ([dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md), [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md), [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md)). Do not add a Loader row in `packages/bundle/web-app/cordis.patch.yml` or a `packages/bundle/web-app/package.json` dependency while that process is up. A **non-overlay** client plugin that should appear on the next `dsh web` boot still needs those two bundle surfaces plus a `tsconfig.client.json` aggregate reference. Overlay card pages get inventory, tsconfig, Model Experience, and omit-list rows from `overlay:new-page`; desktop pages get the same from `overlay:new-desktop`; shaped occupants get the same from `overlay:new-shaped`; they join the default roster only after this session ends, by adding the bundle patch and web-app dependency then. `pnpm-workspace.yaml` already globs `packages/*/*`.
4. **dsh.client manifest semantics**: `platform: 'web'` always; `immediately: true` only for stage-one-prefetch infrastructure rows. `inject` lists package-name dependency edges — they are **informational only** (preflight display, HMR diffing); they do not sequence entry activation or apply order. Activation order is cordis fiber inject waiting on *services*, nothing else.
5. **Registering into another package's slot**: apply order is unconstrained, and a business service is not a declaration barrier. Use `ctx.slots.inject(name, () => ctx.slots.register(...))`; it waits on the actual declaration, removes the contribution when that declaration collapses, reruns after redeclaration, and leaves with the caller's plugin fiber. Return a generator yielding each registration when several contributions must install and roll back atomically. A bare `slots.register` into an undeclared slot remains an error; keep service edges only for services the contribution actually reads.
6. Rebuild the bundle (`pnpm --filter <pkg> bundle`) before probing a live `dsh web` server — the registry serves `lib/client.js`, not sources. When the open page loaded a profile copy, run `pnpm overlay:live update packages/client/<name>` after bundling (or overwrite that copy's `lib/client.js`). Do not restart `dsh web`.
...
6. Non-trivial change? It needs an Agent Note in the same PR (repo-wide rule) — the GUI notes above are the precedents to extend. An overlay **page occupant** (card or desktop) does not get a feature Agent Note; its description lives in that package README ([description registry](../../.agents/notes/implemented/process/2026-09-05-client-plugin-description-registry.md)).
