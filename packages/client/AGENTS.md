...
Pick the presentation form, then load its skill via `dsh_skill` before writing or running `pnpm overlay:live`: card window [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md) (canonical card [`ui-float-window`](ui-float-window/README.md); always insert that package first, a later insert adds a numbered card; new body page is `pnpm overlay:new-page <name>`, then edit only that package's `Page.tsx` / locales / CSS; do not copy the card tree or another occupant; declare `dsh.client.overlayBody`; dual-face host+page is one Loader row so the rail can hide/unplug the whole card); arbitrary-shape floater [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md); full-page canvas [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md). Shaped and canvas HOW are reserved slots; do not invent them or occupy `overlay-card.body` for a non-card outline. Live appearance is `overlay:live`. Do not edit `packages/bundle/web-app/cordis.patch.yml` while this overlay session is running.

Plugin inventory is [README.md](README.md). Each package README owns that plugin's description. Agent Notes and skills name mechanisms (`overlay-card.body`, live insert, card module `ui-float-window`), not product occupants ([description registry](../../.agents/notes/implemented/process/2026-09-05-client-plugin-description-registry.md)). After adding a new overlay package, run `pnpm install --filter ./packages/client/<name>...` (or repo-root `pnpm install`) so `react` resolves; declare `react` in that package's `package.json`. `pnpm overlay:live remove <id>` without `--keep-files` deletes that omitted checkout occupant and its landing rows so an unload cannot leave a searchable empty record. It does not delete `ui-float-window` or a web-app roster package.
...
3. **Default-roster registration is not the live path.** A running overlay session inserts with `pnpm overlay:live` through the watched profile patch ([dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md)). Do not add a Loader row in `packages/bundle/web-app/cordis.patch.yml` or a `packages/bundle/web-app/package.json` dependency while that process is up. A **non-overlay** client plugin that should appear on the next `dsh web` boot still needs those two bundle surfaces plus a `tsconfig.client.json` aggregate reference. Overlay card pages get inventory, tsconfig, Model Experience, and omit-list rows from `overlay:new-page`; they join the default roster only after this session ends, by adding the bundle patch and web-app dependency then. `pnpm-workspace.yaml` already globs `packages/*/*`.
...
6. Non-trivial change? It needs an Agent Note in the same PR (repo-wide rule) — the GUI notes above are the precedents to extend. An overlay **page occupant** does not get a feature Agent Note; its description lives in that package README ([description registry](../../.agents/notes/implemented/process/2026-09-05-client-plugin-description-registry.md)).
