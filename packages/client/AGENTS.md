...
Pick the presentation form, then load its skill via `dsh_skill` before writing or running `pnpm overlay:live`: card window [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md) (canonical card [`ui-float-window`](ui-float-window/README.md); always insert that package first, a later insert adds a numbered card; new body page is `pnpm overlay:new-page <name>`, then edit only that package's `Page.tsx` / locales / CSS; do not copy the card tree; declare `dsh.client.overlayBody`; dual-face host+page is one Loader row so the rail can hide/unplug the whole card); arbitrary-shape floater [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md); desktop [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md) (canonical board [`ui-overlay-desktop`](ui-overlay-desktop/README.md); always insert that package first; new page is `pnpm overlay:new-desktop <name>`, occupy `overlay-desktop.body`; exclusive one occupant). Invent look from the product brief and the generator stub. Do not search, grep, or open another overlay occupant's `Page.tsx`, `Page.module.css`, locales, or page tests as reference. Do not copy the currently painted desktop or card. Open another occupant's page frontend only when the user explicitly asked to make something similar to that product ([visual independence](../../.agents/notes/implemented/process/2026-09-12-overlay-occupant-visual-independence.md)). If `dsh_skill` is missing, read that form's `SKILL.md` from disk and continue. Shaped HOW is a reserved slot; do not invent it or occupy `overlay-card.body` for a non-card outline. Live appearance is `overlay:live` (`insert` builds `lib/` when `tsdown.config.ts` is present). Confirm the package in `window.__DSH_BOOT__`; the rail lists yaml. Do not edit `packages/bundle/web-app/cordis.patch.yml` while this overlay session is running.

Plugin inventory is [README.md](README.md). Each package README owns that plugin's description. Agent Notes and skills name mechanisms (`overlay-card.body`, `overlay-desktop.body`, live insert, card module `ui-float-window`, desktop module `ui-overlay-desktop`), not product occupants ([description registry](../../.agents/notes/implemented/process/2026-09-05-client-plugin-description-registry.md)). After adding a new overlay package, run `pnpm install --filter ./packages/client/<name>...` (or repo-root `pnpm install`) so `react` resolves; declare `react` in that package's `package.json`. `pnpm overlay:live remove <id>` without `--keep-files` deletes that omitted checkout occupant and its landing rows so an unload cannot leave a searchable empty record. It does not delete `ui-float-window`, `ui-overlay-desktop`, or a web-app roster package.
...
3. **Default-roster registration is not the live path.** A running overlay session inserts with `pnpm overlay:live` through the watched profile patch ([dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md), [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md)). Do not add a Loader row in `packages/bundle/web-app/cordis.patch.yml` or a `packages/bundle/web-app/package.json` dependency while that process is up. A **non-overlay** client plugin that should appear on the next `dsh web` boot still needs those two bundle surfaces plus a `tsconfig.client.json` aggregate reference. Overlay card pages get inventory, tsconfig, Model Experience, and omit-list rows from `overlay:new-page`; desktop pages get the same from `overlay:new-desktop`; they join the default roster only after this session ends, by adding the bundle patch and web-app dependency then. `pnpm-workspace.yaml` already globs `packages/*/*`.
