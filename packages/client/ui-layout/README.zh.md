...
`/client` 导出表层包含插件主体（`apply`／`inject`）、`LayoutController`、`OverlayStackController`、overlay-stack id／z-index 辅助函数，以及四个 owner-share 接口。AppFrame、面板 store 与让步求解器仍属于包内部。
...
- **Overlay 层按外框裁剪**：`shell.overlay` 不滚动。探出盒子的占用者会被裁掉。全幅裁剪板若不能抢走兄弟命中，需设置 `data-overlay-board` 和 `pointer-events: none`。
- **Overlay 叠放只有两个占用者**：`ctx.overlayStack` 把 Cursor 窗对 overlay-card 桌面作为整体排序；卡片之间的顺序仍在桌面 store 里。
