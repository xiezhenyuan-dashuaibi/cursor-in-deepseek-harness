...
The `/client` exports are the plugin body (`apply`/`inject`), `LayoutController`, `OverlayStackController`, overlay-stack ids / z-index helper, and the four owner-share interfaces. AppFrame, the panel store, and the concession solver remain package-internal.
...
- **Overlay layer clips to the frame** — `shell.overlay` does not scroll. Occupants that hang past the box are clipped. A full-size clip board that must not steal sibling hits sets `data-overlay-board` and `pointer-events: none`.
- **Overlay stack is two occupants** — `ctx.overlayStack` orders the Cursor window against the overlay-card desk as a unit; per-card order stays inside the desk store.
