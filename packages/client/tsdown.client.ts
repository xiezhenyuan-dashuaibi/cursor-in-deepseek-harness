 ...
 * A non-module `.css` import (vendor sheets such as xterm.js) injects the same
 * way without a class map. The virtual loader registers each real stylesheet
 * as a watch dependency.
 */
...
const CSS_GLOBAL_PREFIX = '\0dsh-css-global:'
  ...
  return {
    ...
    plugins: [{
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        const abs = source.startsWith('.') && importer !== undefined
          ? sourceAssetPath(source, importer)
          : createRequire(importer ?? fileURLToPath(import.meta.url)).resolve(source)
        return CSS_GLOBAL_PREFIX + abs + CSS_VIRTUAL_SUFFIX
      },
      async load(virtualId: string) {
        if (!virtualId.startsWith(CSS_GLOBAL_PREFIX)) return null
        const fileId = virtualId.slice(CSS_GLOBAL_PREFIX.length, -CSS_VIRTUAL_SUFFIX.length)
        this.addWatchFile(fileId)
        const source = await readFile(fileId)
