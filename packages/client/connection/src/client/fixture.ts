 ...
 * which is the dsh_bash tool's own job and not something to reimplement here.
 */
...
const SEARCH_MATCHES_FIXTURE: { path: string; matches: { lineNumber: number; line: string }[] }[] = [
  ...
  },
  {
    ...
    matches: [
      ...
      { lineNumber: 78, line: "      yield ctx.slots.register({ name: 'tool.call.toolview', key: 'dsh_grep', locale: NS }, SearchRow)" },
    ],
  },
]
 ...
 * fixture is client-side and cannot import the dsh_read tool, so the structured
...
/** The `dsh_web_fetch` result view for the web-fetch turn, authored inline for the same reason. */
...
function buildAlphaLog(): SessionEvent[] {
  ...
  toolTurn(62, 'dsh_edit', '{"file_path":"notes/demo.txt","old_string":"hello","new_string":"hello fixture"}', '已编辑')
  toolTurn(63, 'dsh_write', '{"file_path":"notes/new-demo.txt","content":"hello fixture\\n"}', '已写入')
  // Turn 64: a multi-hunk edit — two scattered replacements in one file. Named
  // `dsh_edit` so it lands on the keyed FileMutationRow (the resident diff card the
  ...
  toolTurn(64, 'dsh_edit', '{"file_path":"src/config.ts","old_string":"const timeout = 30","new_string":"const timeout = 60"}', '已编辑')
  // Turn 65: one dsh_run_code turn with three logged sub-dispatches — the Code
  ...
  // keyed registration a top-level bash row uses).
  {
    const turn = 65
    const callId = `fx-call-${turn}`
    const program = 'const listing = await tools.dsh_bash({ command: "ls notes", description: "List notes" })\n'
      + 'const demo = await tools.dsh_read({ file_path: "notes/demo.txt" })\n'
      + 'await tools.dsh_read({ file_path: "notes/missing.txt" }).catch(() => "tolerated")\n'
      + 'return { listing, demo }'
    const args = JSON.stringify({ code: program, description: 'Read the notes files and summarize' })
    push({ type: 'turn/start', data: { turn } })
    push({ type: 'user/message', surfaceOp: 'append', data: userMessage(text(`问题 ${turn}：dsh_run_code 样本。`)) })
    push({ type: 'step/start', data: { turn, step: 0 } })
    push({
      type: 'assistant/message', surfaceOp: 'append',
      data: { turn, step: 0, message: assistantMessage([{ type: 'tool-call', id: callId, name: 'dsh_run_code', arguments: args } as ContentBlock]) },
    })
    push({ type: 'tool/call', data: { turn, step: 0, callId, name: 'dsh_run_code', arguments: args } })
  ...
  // Turn 74: dsh_todo_write sample — the TodoRow toolview in the flow plus the
  ...
  // path) is covered by the turn 65 dsh_run_code read sub-dispatches, which
  // session.ts folds with resultView: null; the fallback-row + read-CARD
  // combination is pinned by the dsh_web_fetch case in read-card.spec.tsx, not by
  ...
  // Turns 70-71: the web render intent — a dsh_web_search whose result view carries
  ...
  // dsh_web_fetch whose result view carries the fetched URL and its HTTP status.
  ...
  toolTurn(70, 'dsh_web_search', '{"query":"deepseek harness architecture"}', 'Search results for deepseek harness architecture.')
  toolTurn(71, 'dsh_web_fetch', '{"url":"https://www.deepseek.com/blog/harness-architecture"}', '# Harness architecture\n\nEverything is a plugin.')
  ...
  toolTurn(74, 'dsh_todo_write', todoArgs, 'Updated todo list: 1 pending, 2 in progress, 1 completed.')
