...
const toolResult = (seq: number, callId: string, name = 'dsh_bash'): ToolResultNode => ({
  kind: 'tool-result', seq, time: seq * 1_000, callId,
  call: { name, argsRaw: `{"command":"cmd-${callId}","description":"run ${callId}"}` },
  callTime: seq * 1_000 - 500,
  content: [], isError: false, callView: null, resultView: null, subCalls: [],
})
const runningCall = (callId: string, name = 'dsh_bash'): RunningToolCall => ({
...
describe('Chat node rendering', () => {
  ...
  it('threads the injected file-mention vocabulary into the closing prose only', () => {
    const wrote = (seq: number, callId: string, path: string): ToolResultNode => ({
      ...toolResult(seq, callId, 'dsh_write'),
      callView: {
...
describe('ChatView', () => {
  ...
  it('renders the fixture main line as independently keyed business nodes', () => {
    ...
    expect(view.getByTestId('tool-seat-a').textContent).toBe('dsh_bash:a')
    expect(view.getByTestId('tool-seat-b').textContent).toBe('dsh_bash:b')
  ...
  it('updates the selected call id handed to the Tool seat', () => {
    ...
    act(() => { h.setSelection({ turnSeq: 3, callId: 'a', toolName: 'dsh_bash' }) })
    ...
    const h = makeHarness({
      pending: [
        new PendingWait('approval', RpcId('r1'), SID,
          { approvalId: 'ap1', toolName: 'dsh_bash' } as PendingWait<'approval'>['payload'], vi.fn()),
