...
describe('render branch tails', () => {
  ...
  it('DetailsPanel resolves a nested dsh_run_code leaf to its full logged args and output', () => {
    localStorage.clear()
    const snap = snapshotBase()
    const longText = 'x'.repeat(1_000)
    snap.runningCalls = [{
      callId: 'p1', name: 'dsh_run_code', argsRaw: '{}', turn: 1, step: 1,
      time: 7_000, callView: null, subCalls: [{
        kind: 'tool-result', seq: 8, time: 8_000, callId: 'p1:code:1',
        call: { name: 'dsh_run_code', argsRaw: '{"code":"return 1"}' },
        callTime: 8_000,
        content: [], isError: false, callView: null, resultView: null,
        subCalls: [{
          kind: 'tool-result', seq: 9, time: 9_000, callId: 'p1:code:1:code:1',
          call: { name: 'dsh_read', argsRaw: '{"path":"notes/demo.txt"}' },
          callTime: 8_500,
    ...
    const chat = createChatStore().create()
    chat.actions.select({ turnSeq: 9, callId: 'p1:code:1:code:1', toolName: 'dsh_read' } satisfies SelectionTarget)
    const emptyList = createSnapshotStore<SessionListState>(
      { ids: [], byId: {}, current: undefined, phase: 'ready', subagentsByParent: {}, jobsBySession: {}, currentAddress: undefined })
    const emptyWorkspaces = createSnapshotStore<WorkspaceListState>({
      items: [], archivedSessionIds: [], state: 'idle', phase: 'ready', error: null,
