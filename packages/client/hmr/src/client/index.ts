 ...
 * fiber in place. On a `graph` frame it diffs the host roster against the
 * live loader tree: adopt/drop module-table rows, `loader.create` names
 * the page does not yet have, and `loader.remove` names the host dropped
 * (never the kernel entries: modules, this driver, app-shell). Every graph
 * entry is a plugin bundle — `immediately` rows differ only in stage-one
 * prefetch (a boot optimization), so all rostered plugin packages share
 * these reload semantics; normal packages (react family, cordis, shell,
 * pure libs) are not entries and shell changes still mean a page reload.
 * Cascade is zero-touch: downstream fibers key their activation epoch on
 * provider fiber uids (vendor/cordis/src/fiber.ts `_refresh`), so replacing
 * a provider fiber re-cascades natively — reloading a data-layer plugin
 * (connection/runtime) cascades into its UI dependents with no HMR-side
 * bookkeeping.
 *
 ...
 * Full unload uses `loader.remove` (the Loader-owned dispose path) instead
 * of that swap.
 *
 ...
 * acceptable for the dev channel, the next rebuild renotifies. A graph
 * frame never removes this driver, the modules kernel, or app-shell.
 *
...
/** Loader names the shell kernel created; a host graph frame must not unload them. */
const KERNEL_ENTRY_NAMES = new Set([
 ...
 * rebuilt entries, and apply host roster add/remove from `graph` frames.
...
export function apply(ctx: Context): void {
  ...
  async function applyGraph(wire: unknown): Promise<void> {
    if (typeof wire !== 'object' || wire === null || !('entries' in wire) || !Array.isArray(wire.entries)) {
      throw new Error('client-hmr: graph frame is not a boot graph')
    }
    const wanted = new Set<string>()
    for (const entry of wire.entries) {
      if (typeof entry !== 'object' || entry === null) {
        throw new Error('client-hmr: graph frame entry must carry string id/url/rev')
      }
      const row = entry as { id?: unknown; url?: unknown; rev?: unknown }
      if (typeof row.id !== 'string' || typeof row.url !== 'string' || typeof row.rev !== 'string') {
        throw new Error('client-hmr: graph frame entry must carry string id/url/rev')
      }
      wanted.add(row.id)
      modLoader.adoptRow({ id: row.id, url: row.url, rev: row.rev })
    }
    for (const loaderEntry of [...loader.entries()]) {
      const packageName = loaderEntry.options.name
      if (KERNEL_ENTRY_NAMES.has(packageName) || wanted.has(packageName)) continue
      await loader.remove(loaderEntry.id)
      modLoader.dropRow(packageName)
      removeOwnedStyles(packageName)
    }
    for (const id of wanted) {
      if (KERNEL_ENTRY_NAMES.has(id) || findEntry(loader, id) !== undefined) continue
      await modLoader.prefetch(id).catch(() => {
        // Import during create reloads and reports this loudly; swallowing
        // here keeps one failing prefetch from blocking later roster rows.
      })
      const treeId = await loader.create({ name: id })
      if (loader.resolve(treeId).fiber === undefined) {
        ctx.logger.warn(`client-hmr: graph add "${id}" imported without a fiber`)
      } else {
        await loader.resolve(treeId).fiber?.await()
      }
    }
  }

  // Serialize reloads and roster diffs: frames can arrive faster than a swap
  // completes, and interleaved dispose/execute chains would corrupt the
  // single-slot handoff.
    ...
    switch (frame.type) {
      ...
      case 'graph':
        queue = queue.then(() => applyGraph(frame.graph)).catch((error: unknown) => {
          ctx.logger.error('client-hmr: graph apply failed')
          ctx.logger.error(error)
        })
        break
      default:
