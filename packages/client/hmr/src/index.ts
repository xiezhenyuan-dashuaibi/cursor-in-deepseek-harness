 ...
 * Graph frames are sent on connect and whenever the host roster changes so
 * the open page can add or remove entries without a refresh. The web bundle
 * mounts this row unconditionally: without a rebuild watcher rewriting
 * client bundles, content polls stay idle, but roster frames still fire
 * when Loader entries appear or disappear.
 */
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.effect(() => {
    ...
    // Broadcast the roster before syncWatches so a new row's graph frame
    // reaches the browser before the immediate re-hash can emit `rebuilt`.
    syncWatches()
