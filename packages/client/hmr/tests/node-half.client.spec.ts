...
describe('hmr node half', () => {
  ...
  it('pushes a graph frame to open SSE clients when the roster changes', async () => {
    const bundle = join(dir, 'a.js')
    writeFileSync(bundle, 'v1')
    const clientModuleHost = fakeClientModuleHost(new Map([['pkg-a', bundle]]))
    const routes: WebRoute[] = []
    ...
    if (handler === undefined) throw new Error('expected /plugins/events route')
    void handler({ method: 'GET' } as never, response as never)
    const before = chunks.length
    expect(chunks.some(chunk => chunk.includes('"type":"graph"'))).toBe(true)
    clientModuleHost.fireGraphChanged()
    expect(chunks.slice(before).some(chunk => chunk.includes('"type":"graph"'))).toBe(true)
    await fiber.dispose()
  })
})
