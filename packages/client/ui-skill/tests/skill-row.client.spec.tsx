...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'dsh_skill', argsRaw: '{"name":"dsh-manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"dsh-manage-issues"}'): RunningToolCall {
  return {
    callId: 'call-skill', name: 'dsh_skill', argsRaw, turn: 1, step: 1, time: 2_000, callView: null, subCalls: [],
  }
}
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    expect(view.container.querySelector('[data-tool="dsh_skill"]')?.getAttribute('data-state')).toBe('ok')
    expect(view.container.querySelector('[data-tool="dsh_skill"] svg')?.getAttribute('width')).toBe('14')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    const row = view.container.querySelector('[data-tool="dsh_skill"] > div')!
  ...
  it('uses the first failure line in the summary and exposes the full error', () => {
    ...
    expect(view.container.querySelector('[data-tool="dsh_skill"]')?.getAttribute('data-state')).toBe('error')
