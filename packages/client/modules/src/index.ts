 ...
 * steady state share one implementation. A successful metadata resolve and a
 * "not a web client" verdict stay cached; an unresolvable specifier is
...
export class ClientModuleRegistry extends Service {
  ...
  // `unresolvable` retries on the next flush (a package can appear on disk
  // after the first miss). `not-client` and a successful resolve stay cached.
  private readonly pkgMeta = new Map<string, PkgMeta | 'unresolvable' | 'not-client'>()
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    if (cached === 'not-client') return null
    if (cached !== undefined && cached !== 'unresolvable') return cached
    let pkgPath: string
    try {
      pkgPath = this.resolvePkgJson(pkgName)
    } catch {
      // Loader builtins (`cordis:include`) and subpath rows stay unresolvable;
      // a later flush retries in case this name has become a real package.
      this.pkgMeta.set(pkgName, 'unresolvable')
      return null
    }
    ...
    if (decl === undefined || decl.platform !== 'web') {
      this.pkgMeta.set(pkgName, 'not-client')
      return null
    }
