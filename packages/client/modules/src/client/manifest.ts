...
export interface ClientModuleLoader {
  ...
  /**
   * Insert or replace one graph row so later prefetch/import can load it.
   * Live roster updates from HMR `graph` frames use this; it does not
   * materialize or create a loader entry.
   * @param row - module-table view of the host graph row.
   */
  adoptRow(row: BootModuleRow): void
  /**
   * Drop one graph row and invalidate its factory and record. A later
   * prefetch of the same id is loud until {@link adoptRow} runs again.
   * @param id - entry name to drop.
   */
