...
describe('HMR reset', () => {
  ...
  it('adoptRow lets a later prefetch load a row that was not in the boot graph', async () => {
    const b = bench([], { late: () => ({ marker: 'late' }) })
    b.loader.adoptRow(row('late'))
    await b.loader.prefetch('late')
    const exports = await b.loader.import('late', '', {})
    expect((exports as { marker: string }).marker).toBe('late')
  })

  it('dropRow removes the row and invalidates so a later prefetch is loud', async () => {
    const b = bench([row('a')], { a: () => ({}) })
    await b.loader.prefetch('a')
    b.loader.dropRow('a')
    await expect(b.loader.prefetch('a')).rejects.toThrow('prefetch("a") — not a graph entry')
    expect(b.loader.loadCache.has('a')).toBe(false)
  })
})
