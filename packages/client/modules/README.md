...
Resolution branch order (`import(specifier)`): platform seed word → shell instance; memoized record → surface; shell-own static registry (`registerStatic`, app-shell) → module; registered factory → materialize; graph row (`window.__DSH_BOOT__` or a later HMR `adoptRow`) → load its external classic script + materialize; anything else throws — the runtime mirror of the build-time bundle purity gate. The synchronous `require` handed to factories walks the same order minus the asynchronous load branch and records observed edges into the module record. `prefetch` is the stage-one arrival hook (script load and factory registration only; concurrent calls share one in-flight task); `invalidate` drops the factory and materialized record so the next prefetch/import reloads the script (the HMR hook); `adoptRow` / `dropRow` insert or remove a graph row for live roster updates without rebuilding the page.

The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins`. Boot-graph admission is `platform: 'web'` plus that client export. An unknown string `dsh.client.overlayBody` is ignored so a newer slot name still joins the graph; a non-string value throws. An unresolvable specifier is retried on the next scan so a package that appears on disk after the first miss can still join; a successful resolve and a "not a web client" verdict stay cached. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
