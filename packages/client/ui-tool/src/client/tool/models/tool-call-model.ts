 ...
 *
 * `dsh_cordis_define` is deliberately absent: ui-cordis registers a keyed
...
const TOOL_VARIANTS: Record<string, ToolRowVariant> = {
  ...
  // no program to show (that is `dsh_cordis_define`'s card) and no file to open. The
  // id lands in the summary slot, and the titles below name the act.
  dsh_cordis_run: 'others',
  dsh_cordis_stop: 'others',
  dsh_cordis_undefine: 'others',
}
...
const TOOL_TITLES: Record<string, string> = {
  ...
  dsh_cordis_run: 'Run Cordis Plugin',
  dsh_cordis_stop: 'Stop Cordis Plugin',
  dsh_cordis_undefine: 'Remove Cordis Plugin',
  dsh_pwsh: 'Pwsh',
}
...
/** Path keys only — never `url` (dsh_web_fetch lands on the read variant). */
