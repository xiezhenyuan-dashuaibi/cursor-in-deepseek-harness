...
// nested dsh_run_code sub-dispatch, a legacy generic result) surfaces its
...
export function SearchRow({ toolName, block, inspect, t }: SearchRowProps) {
  ...
  return (
    <ToolRow
      ...
      body={null}
      // A settled call with no search card (errored search, nested dsh_run_code
...
export const searchToolview = {
   ...
   * the `grep` and `dsh_glob` tool names.
   * @param ctx - registrant context (disposal rides ctx.effect inside slots.register).
   */
  apply(ctx: Context): void {
    ctx.slots.inject('tool.call.toolview', function* () {
      yield ctx.slots.register({ name: 'tool.call.toolview', key: 'dsh_grep', locale: NS }, SearchRow)
      yield ctx.slots.register({ name: 'tool.call.toolview', key: 'dsh_glob', locale: NS }, SearchRow)
    })
  },
}
