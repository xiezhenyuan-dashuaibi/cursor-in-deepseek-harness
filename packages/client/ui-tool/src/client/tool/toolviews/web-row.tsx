...
// `dsh_web_fetch` tools. Registered under BOTH, since both declare the one `web`
...
/** dsh_web_fetch reads one URL; dsh_web_search queries. Titles are figma literals. */
...
export function WebRow({ toolName, block, inspect, t }: WebRowProps) {
  ...
  const icon = toolName === 'dsh_web_fetch' ? <IconBrowseOutline16 size={14} /> : <IconGlobeOutline14 size={14} />
  return (
    <ToolRow
      t={t}
...
export const webToolview = {
  ...
  apply(ctx: Context): void {
    ctx.slots.inject('tool.call.toolview', function* () {
      ...
      yield ctx.slots.register({ name: 'tool.call.toolview', key: 'dsh_web_fetch', locale: NS }, WebRow)
    })
  },
}
