...
// WebRow (registered under both dsh_web_search and dsh_web_fetch), the GenericToolCard
...
/** A dsh_web_search result view; overrides tune the sources / answer / truncation. */
...
const settledFetch = (over?: Partial<ToolResultNode>): ToolResultNode => ({
  ...
  call: { name: 'dsh_web_fetch', argsRaw: FETCH_ARGS },
  callTime: 1_000,
...
describe('chat row web body', () => {
  ...
  it('the WebRow expands to the fetch card, titled Fetch', () => {
    const view = render(<WebRow {...rowProps(settledFetch(), 'dsh_web_fetch')} />)
...
describe('DetailsPanel web Output section', () => {
  ...
  it('renders the fetch card and keeps the fetched body below it', () => {
    const view = mount(snapshot({ nodes: [settledFetch()] }), { turnSeq: 11, callId: 'c2', toolName: 'dsh_web_fetch' })
    const card = view.container.querySelector('[data-web="fetch"]')
    expect(card?.querySelector('a')?.getAttribute('href')).toBe('https://example.com/page')
    expect(view.getByText('HTTP 200')).toBeTruthy()
...
describe('web toolview registration', () => {
  it('registers one WebRow under both dsh_web_search and dsh_web_fetch', () => {
    ...
    expect(registered.map(r => r.key)).toEqual(['dsh_web_search', 'dsh_web_fetch'])
