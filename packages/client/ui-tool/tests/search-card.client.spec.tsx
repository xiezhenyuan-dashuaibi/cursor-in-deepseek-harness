...
const runningGrep = (over?: Partial<RunningToolCall>): RunningToolCall => ({
  callId: 'c1', name: 'dsh_grep', argsRaw: GREP_ARGS,
...
const settledGrep = (over?: Partial<ToolResultNode>): ToolResultNode => ({
  ...
  call: { name: 'dsh_grep', argsRaw: GREP_ARGS },
  callTime: 1_000,
...
const settledGlob = (over?: Partial<ToolResultNode>): ToolResultNode => ({
  ...
  call: { name: 'dsh_glob', argsRaw: GLOB_ARGS },
  callTime: 1_000,
...
describe('searchCardModel', () => {
  ...
  it('returns null for every non-search call: running, no views, generic, terminal, unknown cards', () => {
    ...
    // or a nested dsh_run_code dispatch), which keeps the generic path.
...
describe('chat row search body (GenericToolCard fallback)', () => {
  ...
  it('the expanded body is the grouped matches, capped tighter than the panel', () => {
    ...
    const view = render(<GenericToolCard {...ownerProps(settledGrep(), 'dsh_grep')} />)
  ...
  it('the glob fallback expands to the flat path card', () => {
    const view = render(<GenericToolCard {...ownerProps(settledGlob(), 'dsh_glob')} />)
    toggleRow(view)
...
describe('SearchRow keyed card', () => {
  ...
  it('collapses to the summary row; expanding reveals the grep card', () => {
    const view = render(<SearchRow {...rowProps(settledGrep(), 'dsh_grep')} />)
  ...
  it('expands to the glob path card', () => {
    const view = render(<SearchRow {...rowProps(settledGlob(), 'dsh_glob')} />)
  ...
  it('agrees with the summary row about the run state', () => {
    const runningView = render(<SearchRow {...rowProps(runningGrep(), 'dsh_grep')} />)
  ...
  it('surfaces the result text for a settled non-error call with no card once expanded', () => {
    // A successful nested dsh_run_code sub-dispatch (backend computes no
    ...
    const view = render(<SearchRow {...rowProps(settledGrep({
      ...
      content: [{ type: 'text', text: 'nested dsh_run_code output line' }],
    }), 'dsh_grep')} />)
    ...
    expect(view.queryByText('nested dsh_run_code output line')).toBeNull()
    toggleRow(view)
    expect(view.getByText('nested dsh_run_code output line')).toBeTruthy()
  })
  ...
  it('registers the one row component under both grep and glob keys', () => {
    ...
    expect(registered.map(r => r.key)).toEqual(['dsh_grep', 'dsh_glob'])
...
describe('DetailsPanel Output section (search)', () => {
  ...
  const grepTarget: SelectionTarget = { turnSeq: 10, callId: 'c1', toolName: 'dsh_grep' }
  const globTarget: SelectionTarget = { turnSeq: 11, callId: 'c2', toolName: 'dsh_glob' }
