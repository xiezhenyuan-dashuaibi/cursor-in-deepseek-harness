...
describe('chat row terminal body', () => {
  const ownerProps = (block: RunningToolCall | ToolResultNode): GenericToolCardProps => ({
    callId: 'c1', toolName: 'dsh_bash', block, openFile: vi.fn(), t,
  })
...
describe('BashRow terminal card', () => {
  ...
  const rowProps = (block: RunningToolCall | ToolResultNode): BashRowProps => ({
    callId: 'c1', toolName: 'dsh_bash', block, openFile: vi.fn(),
  ...
  it('shows the terminal presenter\'s description instead of the args summary', () => {
    // `dsh_terminal_send`-style presenters author a description the args do not
...
describe('DetailsPanel Output section', () => {
  ...
  const target: SelectionTarget = { turnSeq: 10, callId: 'c1', toolName: 'dsh_bash' }
  ...
  it('resets the card\'s expand state when the selected call changes', () => {
    ...
    }), { turnSeq: 10, callId: 'c2', toolName: 'dsh_bash' })
  ...
  it('a dsh_run_code sub-dispatch resolves to its own terminal card once views reach it', () => {
  ...
  it('a running dsh_run_code sub-dispatch resolves through the running material', () => {
    const view = mount(snapshot({
