...
describe('ToolCallTree', () => {
  ...
  it('recursively renders a selected leaf without selecting its ancestors', () => {
    const leaf = root('parent:code:1:code:1', { name: 'dsh_read', argsRaw: '{"path":"a.ts"}' })
    const child = {
      ...root('parent:code:1', { name: 'dsh_run_code', argsRaw: '{"code":"return 1"}' }),
      subCalls: [leaf],
    }
    const block = {
      ...root('parent', { name: 'dsh_run_code', argsRaw: '{"code":"return 1"}' }),
      subCalls: [child],
    }
