...
/** dsh_todo_write atomic Tool presentation and its plan-summary model. */
...
describe('TodoRow', () => {
  ...
  it('omits the active clause when no item is in progress and reads running-call args', () => {
    ...
    render(<TodoRow {...rowProps({ callId: 'c1', name: 'dsh_todo_write', argsRaw: args, turn: 1, step: 1, time: 1_000, callView: null })} />)
  ...
  it('keeps non-ok execution states visible through the shared row states', () => {
    const args = JSON.stringify({ todos: LIST })
    const running = render(<TodoRow {...rowProps({ callId: 'c1', name: 'dsh_todo_write', argsRaw: args, turn: 1, step: 1, time: 1_000, callView: null, subCalls: [] })} />)
  ...
  it('falls back to the generic summary on malformed args and marks the error state', () => {
    ...
    expect(screen.getByText('dsh_todo_write · not json')).toBeTruthy()
  })

  it('falls back when parsed args carry no todos array', () => {
    render(<TodoRow {...rowProps(resultNode('{"other":1}'))} />)
    expect(screen.getByText('dsh_todo_write · {"other":1}')).toBeTruthy()
  })
  ...
  it.each([
    ...
    expect(screen.getByText(`dsh_todo_write · ${argsRaw}`)).toBeTruthy()
  })

  it('window-truncated result falls back to the callId summary', () => {
    render(<TodoRow {...rowProps(resultNode('', { call: null }))} />)
    expect(screen.getByText('dsh_todo_write · c1')).toBeTruthy()
  })
  ...
  it('injects the keyed toolview declaration directly', () => {
    ...
    expect(register).toHaveBeenCalledWith({ name: 'tool.call.toolview', key: 'dsh_todo_write', locale: NS }, TodoRow)
  })
})
