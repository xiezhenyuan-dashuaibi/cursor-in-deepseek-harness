...
/**
 * dsh_ask_user_question toolview acceptance: `waiting` summary while running,
...
const resultNode = (argsRaw: string, resultText: string | null, over?: Partial<ToolResultNode>): ToolResultNode => ({
  ...
  call: { name: 'dsh_ask_user_question', argsRaw },
...
const runningCall = (argsRaw: string) =>
  ({ callId: 'c1', name: 'dsh_ask_user_question', argsRaw, turn: 1, step: 1, time: 1_000, callView: null, subCalls: [] })
...
function rowProps(block: unknown): Parameters<typeof AskQuestionRow>[0] {
  return {
    callId: 'c1', toolName: 'dsh_ask_user_question', block, t,
    openFile: vi.fn(),
    sessionId: 's1',
...
describe('AskQuestionRow', () => {
  ...
  it.each([
    ...
    expect(screen.getByText(`dsh_ask_user_question · ${ARGS}`)).toBeTruthy()
  })

  it('user cancellation names the verdict instead of the generic failed shape', () => {
    // ASK_CANCELLED: the apiproxy dsh_ask_user_question handler's cancel error.
  ...
  it('window-truncated result (call head lost) falls back to the callId summary', () => {
    ...
    expect(screen.getByText('dsh_ask_user_question · c1')).toBeTruthy()
  })
  ...
  it('askQuestionToolview injects the toolview declaration directly', () => {
    ...
    expect(register).toHaveBeenCalledWith(
      { name: 'tool.call.toolview', key: 'dsh_ask_user_question', locale: 'conversation' },
      AskQuestionRow,
    )
  })
})
