...
const running = (over?: Partial<RunningToolCall>): RunningToolCall => ({
  callId: 'c1', name: 'dsh_bash', argsRaw: '{"command":"ls -la","description":"List files"}',
  turn: 1, step: 1, time: 1_000, callView: null, subCalls: [], ...over,
})

const result = (over?: Partial<ToolResultNode>): ToolResultNode => ({
  kind: 'tool-result', seq: 10, time: 2_000, callId: 'c1',
  call: { name: 'dsh_bash', argsRaw: '{"command":"ls -la","description":"List files"}' },
  callTime: 1_000,
...
describe('tool-call-model', () => {
  it('classifies known tools and falls back to others', () => {
    expect(classifyTool('dsh_bash')).toBe('bash')
    expect(classifyTool('dsh_pwsh')).toBe('bash')
    expect(classifyTool('dsh_read')).toBe('read')
    expect(classifyTool('dsh_web_fetch')).toBe('read')
    expect(classifyTool('dsh_web_search')).toBe('search')
    expect(classifyTool('dsh_grep')).toBe('search')
    expect(classifyTool('dsh_write')).toBe('write')
    expect(classifyTool('dsh_edit')).toBe('edit')
    ...
    expect(classifyTool('dsh_cordis_run')).toBe('others')
    expect(classifyTool('dsh_cordis_stop')).toBe('others')
    expect(classifyTool('dsh_cordis_undefine')).toBe('others')
    expect(classifyTool('dsh_todo_write')).toBe('others')
  })
  ...
  it('names each cordis verb instead of leaving it a bare tool call', () => {
    ...
    // generic "Tool call · dsh_cordis_run · dyn-1" fallback is user-visible slop.
    const titleOf = (name: string) => toolRowModel(name, running({ name, argsRaw: '{"id":"dyn-1"}' }))
    expect(titleOf('dsh_cordis_run').title).toBe('Run Cordis Plugin')
    expect(titleOf('dsh_cordis_stop').title).toBe('Stop Cordis Plugin')
    expect(titleOf('dsh_cordis_undefine').title).toBe('Remove Cordis Plugin')
    // An owned title takes the tool name out of the summary slot, leaving the
    // package id as the only mutable text.
    expect(titleOf('dsh_cordis_run').summary).toBe('dyn-1')
  })

  it('leaves dsh_cordis_define to its own keyed toolview', () => {
    // ui-cordis registers a keyed `tool.call.toolview` entry for dsh_cordis_define,
    ...
    const model = toolRowModel('dsh_cordis_define', running({ name: 'dsh_cordis_define', argsRaw: '{"name":"clock"}' }))
  ...
  it('gives the pwsh shell row the bash family treatment with its own title', () => {
    const m = toolRowModel('dsh_pwsh', running())
    expect(m.variant).toBe('bash')
    expect(m.title).toBe('Pwsh')
  })

  it('derives state across running/ok/error/interrupted', () => {
    expect(toolRowModel('dsh_bash', running()).state).toBe('running')
    expect(toolRowModel('dsh_bash', result()).state).toBe('ok')
    expect(toolRowModel('dsh_bash', result({ isError: true })).state).toBe('error')
    expect(toolRowModel('dsh_bash', result({ isError: true, error: { name: 'E', code: 'interrupted' } })).state).toBe('stopped')
  })

  it('derives the bash summary from description over command', () => {
    const m = toolRowModel('dsh_bash', running())
    expect(m.title).toBe('Bash')
    expect(m.summary).toBe('List files')
    expect(toolRowModel('dsh_bash', running({ argsRaw: '{"command":"pwd"}' })).summary).toBe('pwd')
  })

  it('keeps summaries single-line and falls back for opaque args', () => {
    expect(toolRowModel('dsh_bash', running({ argsRaw: '{"command":"a\\nb"}' })).summary).toBe('a')
    expect(toolRowModel('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"/tmp/x.ts"}' })).summary).toBe('/tmp/x.ts')
    expect(toolRowModel('dsh_write', running({ name: 'dsh_write', argsRaw: '{"file_path":"src/x.ts"}' })).summary).toBe('src/x.ts')
    expect(toolRowModel('dsh_edit', running({ name: 'dsh_edit', argsRaw: '{"file_path":"src/x.ts"}' })).summary).toBe('src/x.ts')
  ...
  it('exposes filePath for path/file_path args and skips URL-only reads', () => {
    expect(toolRowModel('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"src/a.ts"}' })).filePath).toBe('src/a.ts')
    expect(toolRowModel('dsh_write', running({ name: 'dsh_write', argsRaw: '{"file_path":"src/a.ts"}' })).filePath).toBe('src/a.ts')
    expect(toolRowModel('dsh_edit', running({ name: 'dsh_edit', argsRaw: '{"file_path":"src/a.ts"}' })).filePath).toBe('src/a.ts')
    expect(toolRowModel('dsh_web_fetch', running({ name: 'dsh_web_fetch', argsRaw: '{"url":"https://example.com"}' })).filePath)
      .toBeUndefined()
    expect(toolRowModel('dsh_bash', running()).filePath).toBeUndefined()
  })
  ...
  it('displays workspace-rooted paths relative to the session cwd', () => {
    ...
    expect(toolRowModel('dsh_edit', running({ name: 'dsh_edit', argsRaw: '{"file_path":"/Users/u/ws/src/x.ts"}' }), cwd).summary).toBe('src/x.ts')
    expect(toolRowModel('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"/Users/u/ws/a.md"}' }), cwd).summary).toBe('a.md')
    // Paths outside the workspace (and non-path summaries) stay verbatim.
    expect(toolRowModel('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"/etc/hosts"}' }), cwd).summary).toBe('/etc/hosts')
    expect(toolRowModel('dsh_bash', running({ argsRaw: '{"command":"pwd"}' }), cwd).summary).toBe('pwd')
    expect(toolRowModel('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"/Users/u/ws/a.md"}' }), '').summary).toBe('/Users/u/ws/a.md')
  })

  it('body pretty-prints JSON args, keeps raw non-JSON, null when empty', () => {
    expect(toolRowModel('dsh_bash', running({ argsRaw: '{"a":1}' })).body).toBe('{\n  "a": 1\n}')
    expect(toolRowModel('dsh_bash', running({ argsRaw: 'raw' })).body).toBe('raw')
    expect(toolRowModel('dsh_bash', running({ argsRaw: '' })).body).toBeNull()
    expect(toolRowModel('dsh_bash', result({ call: null })).body).toBeNull()
  })

  it('a code row with an empty program falls back to the args JSON envelope', () => {
    expect(toolRowModel('dsh_run_code', running({ name: 'dsh_run_code', argsRaw: '{"code":""}' })).body)
      .toBe('{\n  "code": ""\n}')
  })
  ...
  it('derives output from the settled result and null while running or blank', () => {
    expect(toolRowModel('dsh_bash', result({ content: [{ type: 'text', text: 'out' }] })).output).toBe('out')
    expect(toolRowModel('dsh_bash', running()).output).toBeNull()
    expect(toolRowModel('dsh_bash', result({ content: [] })).output).toBeNull()
  })

  it('derives errorSummary as the first output line on error rows only', () => {
    const failed = result({ content: [{ type: 'text', text: 'boom\ndetail' }], isError: true })
    expect(toolRowModel('dsh_bash', failed).errorSummary).toBe('boom')
    expect(toolRowModel('dsh_bash', result({ content: [{ type: 'text', text: 'boom' }] })).errorSummary).toBeNull()
    expect(toolRowModel('dsh_bash', result({ content: [], isError: true })).errorSummary).toBeNull()
    expect(toolRowModel('dsh_bash', running()).errorSummary).toBeNull()
  })
  ...
  it('gives Cordis lifecycle tools action titles over their generic variants', () => {
    ...
    expect(toolRowModel('dsh_cordis_run', running({
      name: 'dsh_cordis_run',
      argsRaw: '{"id":"dyn-2"}',
    }))).toMatchObject({
      variant: 'others',
    ...
    expect(toolRowModel('dsh_cordis_undefine', result({
      call: { name: 'dsh_cordis_undefine', argsRaw: '{"id":"dyn-2"}' },
    }))).toMatchObject({
      variant: 'others',
...
describe('GenericToolCard', () => {
  ...
  it('renders the classified variant row from the frozen slice', () => {
    const view = render(<GenericToolCard {...props('dsh_bash', result())} />)
    expect(view.getByText('Bash')).toBeTruthy()
    expect(view.getByText('List files')).toBeTruthy()
  ...
  it('unknown tools land on the others variant titled Tool call', () => {
    const view = render(
      <GenericToolCard {...props('dsh_todo_write', running({ name: 'dsh_todo_write', argsRaw: '{"note":"x"}' }))} />,
    )
  ...
  it('renders edit with its dedicated title, icon variant, and path summary', () => {
    const view = render(
      <GenericToolCard {...props('dsh_edit', running({
        name: 'dsh_edit',
  ...
  it('renders write with its dedicated title, icon variant, and path summary', () => {
    const view = render(
      <GenericToolCard {...props('dsh_write', running({
        name: 'dsh_write',
  ...
  it('passes the owner inspect callback through to the expanded row pill', () => {
    const inspect = vi.fn()
    const view = render(<GenericToolCard {...props('dsh_bash', result())} inspect={inspect} />)
    fireEvent.click(view.getByRole('button', { name: /Bash/ }))
  ...
  it('file-path summary click reaches openFile; bash summary does not', () => {
    const file = props('dsh_read', running({ name: 'dsh_read', argsRaw: '{"path":"src/x.ts"}' }))
    ...
    const bash = props('dsh_bash', result())
