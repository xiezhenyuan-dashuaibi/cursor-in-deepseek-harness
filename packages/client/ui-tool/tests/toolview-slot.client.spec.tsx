...
describe('keyed toolview hole through the real machinery', () => {
  ...
  it('renders top-level Cordis calls with lifecycle titles over the generic variants', async () => {
    const b = await bench([
      ...
      toolResult(4, 'cordis-2', 'dsh_cordis_run', '{"id":"dyn-2"}'),
      toolResult(5, 'cordis-3', 'dsh_cordis_stop', '{"id":"dyn-2"}'),
      toolResult(6, 'cordis-4', 'dsh_cordis_undefine', '{"id":"dyn-2"}'),
    ])
    const view = b.runtime.renderRoot()
    ...
    expect(rowText('dsh_cordis_run')).toContain('Run Cordis Plugindyn-2')
    expect(rowText('dsh_cordis_stop')).toContain('Stop Cordis Plugindyn-2')
    expect(rowText('dsh_cordis_undefine')).toContain('Remove Cordis Plugindyn-2')
    // No run-control verb is a code row; the program is dsh_cordis_define's, and its
  ...
  it('file-path clicks travel owner openFile → chat inject → workspaces.openPath', async () => {
    const b = await bench([toolResult(3, 'c1', 'dsh_read', '{"path":"src/a.ts"}')])
    const view = b.runtime.renderRoot()
  ...
  it('bash summary clicks do not open details or host paths', async () => {
    const b = await bench([toolResult(3, 'c1', 'dsh_bash')])
    const view = b.runtime.renderRoot()
  ...
  it('a duplicate key registration fails loud at load', async () => {
    ...
    expect(() => b.slots.register(
      { name: 'tool.call.toolview', key: 'dsh_bash' },
      () => null,
    )).toThrow(/key "dsh_bash"/)
    await b.runtime.dispose()
  })
...
describe('registrant declaration injection', () => {
  it('runs a registrant before ui-tool and waits on the actual toolview declaration', async () => {
    ...
    expect(runtime.slots.entries('tool.call.toolview').map(e => e.options.key))
      .toEqual(expect.arrayContaining(['dsh_bash', 'late']))
