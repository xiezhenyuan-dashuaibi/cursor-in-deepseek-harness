...
const todoResult = (seq: number): ToolResultNode => ({
  ...
  call: { name: 'dsh_todo_write', argsRaw: JSON.stringify({ todos: TODOS }) },
...
describe('dsh_todo_write assembly (product registrations, no outlet twins)', () => {
  it('reaches the keyed toolview row and the dock plan strip, and the strip follows projection retirement', async () => {
    ...
    const row = view.container.querySelector('[data-tool="dsh_todo_write"]')
    ...
    expect(view.container.querySelector('[data-tool="dsh_todo_write"]')).not.toBeNull()
