...
// use it so the row exercises a production-shaped call. `dsh_web_fetch` (below) has
...
describe('GenericToolCard read body', () => {
  const ownerProps = (block: RunningToolCall | ToolResultNode): GenericToolCardProps => ({
    callId: 'c1', toolName: 'dsh_web_fetch', block, openFile: vi.fn(), t,
  })
  ...
  it('expands to the read card, capped tighter than the panel', () => {
    ...
    // dsh_web_fetch lands on the read variant without its own keyed row, so the
    // fallback card owns the read block once expanded.
    const view = render(<GenericToolCard {...ownerProps(settled({ call: { name: 'dsh_web_fetch', argsRaw: WEB_FETCH_ARGS } }))} />)
  ...
  it('a running read renders the summary row alone (no result view yet)', () => {
    const view = render(<GenericToolCard {...ownerProps(running({ name: 'dsh_web_fetch' }))} />)
    expect(view.container.querySelector('[data-read]')).toBeNull()
  })
})
...
describe('ReadRow keyed toolview', () => {
  ...
  const rowProps = (block: RunningToolCall | ToolResultNode): Parameters<typeof ReadRow>[0] => ({
    callId: 'c1', toolName: 'dsh_read', block, openFile: vi.fn(),
  ...
  it('registers under the read key of the keyed toolview slot', () => {
    ...
    expect(registered).toEqual([{ name: 'tool.call.toolview', key: 'dsh_read', locale: 'conversation' }])
...
describe('DetailsPanel Output section (read)', () => {
  ...
  const target: SelectionTarget = { turnSeq: 10, callId: 'c1', toolName: 'dsh_read' }
