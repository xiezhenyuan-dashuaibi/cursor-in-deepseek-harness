...
// chat-toolview-slot.spec): a dsh_run_code result renders the 'code' variant row
...
const codeResult = (seq: number, callId: string): ToolResultNode => ({
  ...
  call: { name: 'dsh_run_code', argsRaw: RUN_CODE_ARGS },
...
const runningCode = (callId: string): RunningToolCall => ({
  callId, name: 'dsh_run_code', argsRaw: RUN_CODE_ARGS, turn: 9, step: 0, time: 9_000, callView: null,
  subCalls: [],
})
...
describe('dsh_run_code sub-calls through the real chat machinery', () => {
  it('renders the code-variant parent row with the description summary and nested sub-rows', async () => {
    const parent = 'call-64'
    const subCalls = [
      subCall(11, parent, 1, 'dsh_bash', { command: 'ls notes', description: 'List notes' }, 'demo.txt'),
  ...
  it('renders Cordis sub-calls with lifecycle titles over the generic variants', async () => {
    ...
    const subCalls = [
      ...
      subCall(12, parent, 2, 'dsh_cordis_run', { id: 'dyn-2' }, 'Dynamic package dyn-2 is running'),
      subCall(13, parent, 3, 'dsh_cordis_undefine', { id: 'dyn-2' }, 'Dynamic package dyn-2 was discarded.'),
    ]
    const b = await bench(snapshotWith([codeResult(10, parent)], subCalls))
    const view = mountApp(b.slots)
    const nest = view.container.querySelector('[data-subcalls]')!
    ...
    // owned titles all three would read "Tool call · dsh_cordis_run · dyn-2".
    expect(nest.querySelector('[data-tool="cordis_runtime_inspect"]')?.textContent).toContain('Inspect')
    expect(nest.querySelector('[data-tool="dsh_cordis_run"]')?.textContent).toContain('Run Cordis Plugindyn-2')
    expect(nest.querySelector('[data-tool="dsh_cordis_undefine"]')?.textContent).toContain('Remove Cordis Plugindyn-2')
    // None of them is a code row: the program belongs to dsh_cordis_define, whose
  ...
  it('expanding the code row reveals the program body verbatim (shiki-tokenized)', async () => {
    ...
    expect(pre!.textContent).toContain('const listing = await tools.dsh_bash')
  ...
  it('a file sub-row click opens the host path; bash sub-rows do not open details', async () => {
    const parent = 'call-64'
    const subCalls = [
      subCall(11, parent, 1, 'dsh_read', { path: 'notes/demo.txt' }, 'ok'),
      subCall(12, parent, 2, 'dsh_bash', { command: 'ls notes', description: 'List notes' }, 'demo.txt'),
    ]
    const b = await bench(snapshotWith([codeResult(10, parent)], subCalls))
    const view = mountApp(b.slots)
  ...
  it('a RUNNING dsh_run_code call nests its so-far dispatches under the spinner row', async () => {
    const parent = 'call-live'
    const subCalls = [
      subCall(21, parent, 1, 'dsh_bash', { command: 'ls notes', description: 'List notes' }, 'demo.txt'),
    ]
  ...
  it('a started-but-unsettled sub-call renders the running state exactly like a native in-flight row', async () => {
    ...
    const runningSub: ToolCallBlock = {
      callId: `${parent}:code:1`, name: 'dsh_grep', argsRaw: '{"pattern":"todo"}',
