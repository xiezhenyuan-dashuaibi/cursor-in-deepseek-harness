 ...
 * - `FAKE_REASON_KIND`: the `turn/end` reason kind (default `completed`; `none` emits `{ kind: 'none' }`).
 * - `FAKE_ERROR_MESSAGE`: `turn/end` `error.message` when `FAKE_REASON_KIND` is `error` (default `scripted turn failure`).
...
function runTurn(sessionId: string): void {
  ...
  const reason = reasonKind === 'error'
    ? {
      kind: 'error',
      error: { message: env.FAKE_ERROR_MESSAGE ?? 'scripted turn failure', code: 'UNKNOWN' },
    }
    : { kind: reasonKind }
