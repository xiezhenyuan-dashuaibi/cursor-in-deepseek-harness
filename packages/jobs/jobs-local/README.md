...
At capacity, `start()` fails before producer execution and id allocation with an error that names the limit and tells the model to use `dsh_job_kill`, wait for the job to finish stopping, and retry. The registry does not queue, preempt, or maintain a second mutable counter.
