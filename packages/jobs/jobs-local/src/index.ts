...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    ...
    if (active >= this.maxConcurrentJobsPerOwner) {
      throw new Error(
        `background job limit reached for this owner (limit: ${this.maxConcurrentJobsPerOwner}); use dsh_job_kill to stop an unneeded job, wait for it to finish, then retry`,
      )
    }
