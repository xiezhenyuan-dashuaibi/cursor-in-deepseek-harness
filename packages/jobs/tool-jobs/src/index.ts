/**
 * Model-facing `dsh_job_output`, `dsh_job_list`, and `dsh_job_kill` tools over
...
function fitCompletionNotice(snapshot: JobSnapshot): string {
  ...
  const action = '\nDone; dsh_job_output.'
  const complete = `${prefix}${detail}. Read its output with dsh_job_output.`
...
function visibleOutputLimit(ctx: Context, exec: ToolExecution): number | undefined {
  if (exec.name !== 'dsh_job_output' && exec.name !== 'dsh_job_kill') return undefined
...
export function apply(ctx: Context, config: Config): void {
  ...
  const finalizeTaskContent: NonNullable<ToolDefinition['finalizeContent']> = (exec, result) => {
    ...
    if (exec.name === 'dsh_job_output' && !result.isError) {
