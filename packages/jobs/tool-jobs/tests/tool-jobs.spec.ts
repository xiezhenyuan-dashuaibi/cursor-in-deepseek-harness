...
describe('tool-jobs setup', () => {
  ...
  it('applies the built-in wait bounds when apply() receives a bare config', async () => {
    ...
    await ctx.plugin(LocalJobRegistry)
    ToolTasks.apply(ctx, {})
    expect(ctx.tools.get('dsh_job_output')).toBeDefined()
    expect(() => ctx.jobs.start(producer().spec)).not.toThrow()
  })
})

describe('dsh_job_output', () => {
  it('reads a consuming delta with a trailing status line', async () => {
    ...
    const first = await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })
    if (first.isError) throw new Error('expected dsh_job_output success')
    ...
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'bash-1' }))).toBe('(no new output)\n[status: running]')
  })
  ...
  it('returns the final output of a settled final-output job', async () => {
    ...
    ctx.jobs.start(p.spec)
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'subagent-1' }))).toBe('(no new output)\n[status: running]')

    p.settle({ status: 'completed', detail: 'completed', output: 'the answer' })
    await tick()
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'subagent-1' }))).toBe('the answer\n[status: completed, completed]')
  })
  ...
  it('applies a producer limit to the complete body and status result', async () => {
    ...
    const output = text(await call(ctx, 'dsh_job_output', { job_id: 'bash-1' }))
  ...
  it('preserves empty and newline-terminated output under a producer limit', async () => {
    ...
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })))
      .toBe('(no new output)\n[status: running]')
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })))
      .toBe('line\n[status: running]')
  })
  ...
  it('bounds post-policy output without restoring the canonical status rendering', async () => {
    ...
    ctx.on('tools/post-execute', (exec, _result, next) => {
      if (exec.name !== 'dsh_job_output') return next()
      return Promise.resolve({ kind: 'accept', content: [{ type: 'text', text: 'p'.repeat(1_000) }] })
    })

    const result = await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })
  ...
  it('bounds pre-, around-, and post-execute policy outcomes and failures', async () => {
    ...
    const denied = await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })
    expect(denied.isError).toBe(true)
    expect(Buffer.byteLength(text(denied))).toBeLessThanOrEqual(64)
    expect(text(denied)).toContain('[result truncated]')

    const shortCircuited = await call(ctx, 'dsh_job_output', { job_id: 'bash-2' })
    ...
    const failures = [
      await call(ctx, 'dsh_job_output', { job_id: 'bash-3' }),
      await call(ctx, 'dsh_job_output', { job_id: 'bash-4' }),
      await call(ctx, 'dsh_job_output', { job_id: 'bash-5' }),
    ]
  ...
  it('wait: true blocks until settlement and reports the terminal state', async () => {
    ...
    const pending = call(ctx, 'dsh_job_output', { job_id: 'subagent-1', wait: true })
  ...
  it('wait: true times out against the configured cap and leaves the job alive', async () => {
    ...
    const result = await call(ctx, 'dsh_job_output', { job_id: 'bash-1', wait: true, timeout_ms: 600_000 })
    expect(text(result)).toBe('(no new output)\n[status: running]')
  })

  it('rejects an empty or unknown job id as an errored result', async () => {
    const { ctx } = await setup()
    expect((await call(ctx, 'dsh_job_output', { job_id: '' })).isError).toBe(true)
    const unknown = await call(ctx, 'dsh_job_output', { job_id: 'bash-99' })
    expect(unknown.isError).toBe(true)
    expect(text(unknown)).toContain('unknown job bash-99')
  })
})

describe('dsh_job_list', () => {
  it('lists caller-visible jobs and renders the empty case', async () => {
    const { ctx } = await setup()
    expect(text(await call(ctx, 'dsh_job_list', {}))).toBe('(no background jobs)')
    ...
    const listed = await call(ctx, 'dsh_job_list', {}, alice)
    if (listed.isError) throw new Error('expected dsh_job_list success')
    ...
    expect(text(await call(ctx, 'dsh_job_list', {}, bob))).toBe('subagent-1 [subagent] running — open research')
  })
})

describe('dsh_job_kill', () => {
  it('requests cancellation with the forwarded reason', async () => {
    const { ctx } = await setup()
    const p = producer()
    ctx.jobs.start(p.spec)

    const result = await call(ctx, 'dsh_job_kill', { job_id: 'bash-1', reason: 'superseded' })
    if (result.isError) throw new Error('expected dsh_job_kill success')
  ...
  it('applies the producer output limit to a cancellation acknowledgement', async () => {
    ...
    const result = await call(ctx, 'dsh_job_kill', { job_id: 'bash-1' })
  ...
  it('bounds single-text post policy while preserving structured policy results', async () => {
    const { ctx } = await setup()
    ctx.on('tools/post-execute', (exec, _result, next) => {
      if (exec.name !== 'dsh_job_kill') return next()
  ...
  it('reports an already-finished job without consuming its pending delta', async () => {
    ...
    if (killed.isError) throw new Error('expected dsh_job_kill success')
    ...
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'bash-1' }))).toBe('unread tail\n[status: completed, exit code: 0]')
  })

  it('rejects an empty job id as an errored result', async () => {
    const { ctx } = await setup()
    expect((await call(ctx, 'dsh_job_kill', { job_id: '' })).isError).toBe(true)
  })
})
...
describe('tool-owned UI presentation (presentCall)', () => {
  it('renders generic cards for all three control tools', async () => {
    const { ctx } = await setup()
    expect(ctx.tools.get('dsh_job_output')?.presentCall?.({ job_id: 'bash-1' }))
      .toEqual({ card: 'generic', title: 'Read output from background job bash-1', kind: 'read', rawInput: 'bash-1' })
    expect(ctx.tools.get('dsh_job_list')?.presentCall?.({}))
      .toEqual({ card: 'generic', title: 'List background jobs', kind: 'read' })
    expect(ctx.tools.get('dsh_job_kill')?.presentCall?.({ job_id: 'subagent-2' }))
...
describe('completion notices', () => {
  it('injects a notice into the owning agent when an unreported job settles', async () => {
    ...
    expect(inject).toHaveBeenCalledWith({
      ...
      role: 'user',
      content: [{ type: 'text', text: 'background job bash-1 (bash: pnpm test) finished [status: completed, exit code: 0]. Read its output with dsh_job_output.' }],
      source: {
        kind: 'plugin',
        plugin: 'tool-jobs',
        form: 'notice',
  ...
  it('preserves job ids and collection guidance in bounded completion notices', async () => {
    ...
    expect(notice).toContain('background job subagent-2 (subagent: xx')
  ...
  it('reserves the collection-action tail when a producer supplies a smaller budget', async () => {
    ...
    expect(shortNotice).toBe('background\nDone; dsh_job_output.')
  })
  ...
  it('suppresses the notice for a job the model already killed', async () => {
    ...
    await call(ctx, 'dsh_job_kill', { job_id: 'bash-1' }, owner)
  ...
  it('suppresses the notice when a wait returned the terminal state', async () => {
    ...
    const pending = call(ctx, 'dsh_job_output', { job_id: 'subagent-1', wait: true }, owner)
