...
- id: repeat-tool-reminder
  ...
  config:
    ...
    exclude: [dsh_todo_write]        # tool-name patterns transparent to the chain
...
- **不受跟踪的调用对链透明。** 被 `include`／`exclude` 排除的调用既不递增计数器，也不重置计数器；因此，`grep X → dsh_todo_write → grep X` 仍算作连续两次 `grep X`，即使 `dsh_todo_write` 已被排除。这正是排除机制的价值：循环中穿插的记录类工具不能掩盖循环。
