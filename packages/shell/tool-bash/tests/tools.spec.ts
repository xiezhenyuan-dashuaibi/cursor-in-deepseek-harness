...
/** Full harness: the generic job runtime + its controller, then the dsh_bash tool. */
...
describe('bash tool', () => {
  it('returns stdout for a successful command', async () => {
    ...
    const result = await call(ctx, 'dsh_bash', { command: 'echo hello', description: 'test command' })
  ...
  it('reports (no output) for silent commands', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'true', description: 'test command' })
    expect(text(result)).toBe('(no output)')
  })

  it('marks stderr sections', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'echo out; echo err >&2', description: 'test command' })
  ...
  it('reports non-zero exits without isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'echo failing; exit 3', description: 'test command' })
    expect(result.isError).toBe(false)
  ...
  it('reports timeout kills with both markers (timeout first)', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'sleep 60', description: 'test command', timeoutMs: 100 })
    expect(result.isError).toBe(false)
  ...
  it('reports a timeout even when the command traps the signal and exits 0', async () => {
    ...
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'trap "exit 0" TERM; sleep 60', description: 'test command', timeoutMs: 100 })
    expect(result.isError).toBe(false)
  ...
  it('reports truncation with the spill path', async () => {
    ...
    await ctx.plugin(BashEnvPlugin)
    await ctx.plugin(ToolBash)
    const result = await call(ctx, 'dsh_bash', { command: 'for i in $(seq 1 100); do printf "line-%04d\\n" $i; done', description: 'test command' })
  ...
  it('honors workdir', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'test command', workdir: '/tmp' })
    expect(text(result).trim()).toMatch(/\/tmp$/)
  })

  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'true', description: 'test command', workdir: '/nonexistent-dsh' })
  ...
  it.each([
    ...
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', args)
    expect(result.isError).toBe(true)
  ...
  it('rejects a non-JSON numeric argument before tool-specific validation', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', {
  ...
  it('registers the bash schema with run_in_background exposed by default', async () => {
    ...
    expect(schemas.map(schema => schema.name)).toEqual(['dsh_bash'])
    ...
    expect(bashSchema.description).toContain('dsh_job_output')
  })
...
describe('background execution through the job runtime', () => {
  it('run_in_background acks with the job id, readable through the REAL dsh_job_output tool', async () => {
    const ctx = await setupWithTasks()
    const started = await call(ctx, 'dsh_bash', { command: 'echo bg-ok', description: 'test command', run_in_background: true })
    ...
    const read = await callUntilText(ctx, 'dsh_job_output', { job_id: 'bash-1' }, 'bg-ok')
    expect(text(read)).toContain('bg-ok')
    // A later read reports the terminal outcome in the generic status line.
    const final = await callUntilText(ctx, 'dsh_job_output', { job_id: 'bash-1' }, '[status: completed, exit code: 0]')
    expect(final.isError).toBe(false)
  })

  it('a running background job is killable through the REAL dsh_job_kill tool', async () => {
    const ctx = await setupWithTasks()
    await call(ctx, 'dsh_bash', { command: 'sleep 60', description: 'test command', run_in_background: true })

    const killed = await call(ctx, 'dsh_job_kill', { job_id: 'bash-1' })
    ...
    const final = await call(ctx, 'dsh_job_output', { job_id: 'bash-1', wait: true })
    expect(text(final)).toContain('[status: killed, signal: SIGTERM]')
  })

  it('a self-signal background exit is reported as killed through the REAL dsh_job_output tool', async () => {
    const ctx = await setupWithTasks()
    await call(ctx, 'dsh_bash', { command: 'kill -TERM $$', description: 'test command', run_in_background: true })
  ...
  it('a background job started by an agent is registered with that agent as owner', async () => {
    ...
    const started = await call(ctx, 'dsh_bash', { command: 'sleep 60', description: 'test command', run_in_background: true }, agent)
    expect(text(started)).toBe('started background job bash-1')

    const anon = await call(ctx, 'dsh_job_output', { job_id: 'bash-1' })
    expect(anon.isError).toBe(true)
    expect(text(anon)).toMatch(/belongs to another session/)

    const killed = await call(ctx, 'dsh_job_kill', { job_id: 'bash-1' }, agent)
    expect(killed.isError).toBe(false)
    await call(ctx, 'dsh_job_output', { job_id: 'bash-1', wait: true }, agent) // await settlement — no orphan
  })

  it('fails loud when the job runtime is not loaded', async () => {
    const ctx = await setup() // no LocalJobRegistry / ToolTasks
    const result = await call(ctx, 'dsh_bash', { command: 'sleep 60', description: 'test command', run_in_background: true })
    expect(result.isError).toBe(true)
  ...
  it('enableRunInBackground: false removes the parameter and flips the description', async () => {
    ...
    const schema = ctx.tools.schemas().find(s => s.name === 'dsh_bash')!
    expect(Object.keys(schema.parameters.properties as Record<string, unknown>))
    ...
    const parameters = ctx.tools.get('dsh_bash')!.parameters as { properties: Record<string, unknown> }
    ...
    const forced = await call(ctx, 'dsh_bash', { command: 'echo hi', description: 'test command', run_in_background: true })
    expect(forced.isError).toBe(true)
    expect(text(forced)).toContain('run_in_background is disabled for this deployment')
    const foreground = await call(ctx, 'dsh_bash', { command: 'echo hi', description: 'test command' })
...
describe('sandbox escalation through the generic task producer', () => {
  ...
  it('advertises the sandbox fields and validates their pairing', async () => {
    ...
    const schema = ctx.tools.schemas().find(item => item.name === 'dsh_bash')!
    ...
    for (const args of [
      ...
      expect((await call(ctx, 'dsh_bash', args)).isError).toBe(true)
    }
  })

  it('rejects injected escalation without a sandbox and non-widening escalation without prompting', async () => {
    const plain = await setup()
    expect(text(await call(plain.ctx, 'dsh_bash', escalate))).toContain('not available in this composition')

    const { ctx } = await setupSandboxed(true)
    const prompted = vi.fn()
    ctx.on('approval/request', () => { prompted(); return Promise.resolve<ApprovalOutcome>('allowed-once') })
    const result = await call(ctx, 'dsh_bash', { ...escalate, sandbox_permissions: 'workspace-write' }, sandboxAgent('workspace-write'))
    ...
    expect(text(await call(ctx, 'dsh_bash', escalate, malformed))).toContain('not strictly wider')
  })

  it('fails closed when approval cannot be routed', async () => {
    const withoutService = await setupSandboxed()
    expect(text(await call(withoutService.ctx, 'dsh_bash', escalate, sandboxAgent()))).toContain('no approval service')

    const withService = await setupSandboxed(true)
    expect(text(await call(withService.ctx, 'dsh_bash', escalate))).toContain('no agent to route')
    expect(text(await call(withService.ctx, 'dsh_bash', escalate, sandboxAgent()))).toContain('no approval channel')
  })
  ...
  it.each([
    ...
    const result = await call(ctx, 'dsh_bash', escalate, sandboxAgent())
  ...
  it('runs a granted foreground or background call under the approved mode', async () => {
    ...
    expect(foreground.isError).toBe(false)
    const background = await call(ctx, 'dsh_bash', { ...escalate, run_in_background: true }, agent)
  ...
  it('uses the session override for ordinary calls and evaluates widening against it', async () => {
    ...
    await call(ctx, 'dsh_bash', { command: 'true', description: 'ordinary' }, agent)
    ctx.on('approval/request', () => Promise.resolve<ApprovalOutcome>('allowed-once'))
    await call(ctx, 'dsh_bash', { ...escalate, sandbox_permissions: 'danger-full-access' }, agent)
...
describe('session-cwd routing (per-session workdir)', () => {
  ...
  it('defaults bash to the agent\'s session cwd (not the server launch dir)', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'pwd' }, agentInCwd('/tmp'))
    expect(text(result).trim()).toMatch(/\/tmp$/)
  })

  it('an explicit absolute workdir overrides the session cwd', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'pwd', workdir: '/tmp' }, agentInCwd('/'))
    expect(text(result).trim()).toMatch(/\/tmp$/)
  })
  ...
  it('a relative workdir is resolved against the session cwd', async () => {
    ...
    const result = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'pwd', workdir: 'bin' }, agentInCwd('/usr'))
    expect(text(result).trim()).toMatch(/\/usr\/bin$/)
  })

  it('two sessions with different cwds each run bash in their own dir', async () => {
    const ctx = await setup()
    const inUsr = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'pwd' }, agentInCwd('/usr'))
    const inTmp = await call(ctx, 'dsh_bash', { command: 'pwd', description: 'pwd' }, agentInCwd('/tmp'))
  ...
  it('falls back to the executor default when the agent has no session cwd', async () => {
    ...
    const result = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('cwd-noagent'), name: 'dsh_bash', arguments: { command: 'pwd', description: 'pwd' } })
    expect(result.isError).toBe(false)
...
describe('tool-owned UI presentation (presentCall / presentResult)', () => {
  it('bash presentCall: a foreground run is a terminal card (command title, description, workdir → cwd absolute or relative)', async () => {
    ...
    expect(ctx.tools.get('dsh_bash')?.presentCall?.({ command: 'ls -la src', description: 'List files in src' }))
    ...
    expect(ctx.tools.get('dsh_bash')?.presentCall?.({ command: 'pwd', description: 'Print dir', workdir: '/tmp/x' }))
    ...
    expect(ctx.tools.get('dsh_bash')?.presentCall?.({ command: 'pwd', description: 'Print dir', workdir: 'sub' }))
  ...
  it('bash presentResult: a terminal result carries RAW output (newlines intact) + parsed exit code', async () => {
    const ctx = await setup()
    const present = ctx.tools.get('dsh_bash')!.presentResult!(
      { command: 'printf "hi\\n\\n"', description: 'echo' },
  ...
  it('bash presentResult: a non-zero exit and a signal kill parse into exitCode / signal', async () => {
    ...
    const nonzero = ctx.tools.get('dsh_bash')!.presentResult!(args, { content: [{ type: 'text', text: 'oops\n[exit code: 3]' }], isError: false })
    expect(nonzero).toEqual({ card: 'terminal', output: 'oops', exitCode: 3 })
    const killed = ctx.tools.get('dsh_bash')!.presentResult!(args, { content: [{ type: 'text', text: 'gone\n[killed by signal: SIGKILL]' }], isError: false })
  ...
  it('bash presentResult: markers a pill CANNOT show (timeout, sandbox denial) stay in the terminal output', async () => {
    const ctx = await setup()
    const args = { command: 'x', description: 'x' }
    const timedOut = ctx.tools.get('dsh_bash')!.presentResult!(
      args,
  ...
  it('bash presentResult exit parse is the inverse of renderResult markers (round-trip)', async () => {
    const ctx = await setup()
    const present = ctx.tools.get('dsh_bash')!
  ...
  it('bash presentResult: a clean exit-0 whose output ENDS in marker-like text is NOT read as a failure', async () => {
    ...
    const out = ctx.tools.get('dsh_bash')!.presentResult!(args, { content: [{ type: 'text', text: '[exit code: 5]' }], isError: false })
    ...
    const sig = ctx.tools.get('dsh_bash')!.presentResult!(args, { content: [{ type: 'text', text: '[killed by signal: SIGKILL]' }], isError: false })
  ...
  it('bash presentCall/presentResult: a run_in_background call is a generic card and its ack carries no exit pill', async () => {
    ...
    const call = ctx.tools.get('dsh_bash')!.presentCall!({ command: 'sleep 100', description: 'wait', run_in_background: true })
    ...
    const result = ctx.tools.get('dsh_bash')!.presentResult!(
  ...
  it('bash presentResult: an isError result is a generic card (no real process exit to report)', async () => {
    ...
    const out = ctx.tools.get('dsh_bash')!.presentResult!(
  ...
  it('bash presentResult: a result that is not exactly one block → undefined (no single text to fence)', async () => {
    ...
    expect(ctx.tools.get('dsh_bash')!.presentResult!(args, { content: [], isError: false })).toBeUndefined()
    expect(ctx.tools.get('dsh_bash')!.presentResult!(args, {
  ...
  it('presentCall validates softly: malformed args (missing required description) return undefined, never throw', async () => {
    ...
    expect(ctx.tools.get('dsh_bash')?.presentCall?.({ command: 'ls' })).toBeUndefined()
  })
})
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    const description = ctx.tools.get('dsh_bash')?.description ?? ''
