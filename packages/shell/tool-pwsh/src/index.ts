 ...
 * presentation mirrors the dsh_bash tool's too: a completed foreground call is
...
/** Runtime configuration schema for the dsh_pwsh tool plugin. */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  const background = backgroundEnabled
    ? 'Set `run_in_background: true` for long-running commands: the call returns a job id immediately; read its output with `dsh_job_output` and stop it with `dsh_job_kill`.'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      ...backgroundEnabled ? {
        run_in_background: { type: 'boolean' as const, description: 'Run in the background and return a job id immediately (collect with dsh_job_output, stop with dsh_job_kill). No timeout applies.' },
      } : {},
    ...
    presentCall: (args: PwshToolArgs): TerminalCallView | GenericCallView => {
      ...
      // card mirrors the dsh_bash tool's background presentation.
      if (args.run_in_background === true) {
        return {
          card: 'generic',
          title: args.command,
