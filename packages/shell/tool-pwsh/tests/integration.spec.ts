 ...
 * `dsh_pwsh` tool, exercised through `ctx.tools.execute()` with a real PowerShell
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  ...
  it('runs a command and returns stdout with no marker on a clean exit', async () => {
    const result = await call('dsh_pwsh', { command: 'Write-Output hi', description: 'say hi' }, agent())
  ...
  it('returns stderr in a marked section and a nonzero exit as a marker, not an error', async () => {
    const result = await call('dsh_pwsh', {
  ...
  it('a background run settles through the REAL dsh_job_output tool', async () => {
    const started = await call('dsh_pwsh', {
    ...
    while (Date.now() < deadline) {
      const read = await call('dsh_job_output', { job_id: jobId })
      output += text(read)
