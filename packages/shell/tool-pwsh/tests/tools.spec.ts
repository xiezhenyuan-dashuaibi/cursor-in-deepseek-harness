/**
 * Consumer-surface tests for the `dsh_pwsh` tool over a FAKE bash executor,
...
/** A running background handle whose kill() settles it as killed (like a real dsh_job_kill). */
...
/** Full harness: the generic job runtime + its controller, then the dsh_pwsh tool. */
 ...
 * like the dsh_bash tool — the per-session sandbox-policy regression surface.
...
/** Sandboxed composition: the shared policy service + a confining executor + the dsh_pwsh tool (+ optional approval). */
...
describe('registration', () => {
  it('registers the dsh_pwsh tool with its prompt section and schema', async () => {
    const { ctx } = await setup()
    const schema = ctx.tools.schemas().find(s => s.name === 'dsh_pwsh')
...
describe('argument validation', () => {
  it('rejects a blank command or description and a non-positive timeoutMs', async () => {
    const { ctx } = await setup()
    expect(text(await call(ctx, 'dsh_pwsh', { command: '  ', description: 'd' }))).toContain('expected a non-empty string')
    expect(text(await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: ' ' }))).toContain('expected a non-empty string')
    expect(text(await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'd', timeoutMs: -1 })))
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed DSH_* environment', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', {
  ...
  it('resolves a relative workdir against the session cwd, absolute ones verbatim', async () => {
    ...
    Object.assign(agent.session.header, { cwd: '/sessions/s1' })
    await call(ctx, 'dsh_pwsh', { command: 'pwd', description: 'cwd', workdir: 'sub/dir' }, agent)
    expect(bash.requests[0]?.workdir).toBe(resolvePath('/sessions/s1', 'sub/dir'))
    await call(ctx, 'dsh_pwsh', { command: 'pwd', description: 'cwd', workdir: resolvePath('/abs/path') }, agent)
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    const { ctx, bash } = await setup()
    bash.handler = () => runResult('ok\n')
    await call(ctx, 'dsh_pwsh', { command: 'Write-Output ok', description: 'ok' })
  ...
  it('projects the canonical foreground result with stdout, stderr, and exit facts', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', { command: 'failing', description: 'fail' })
    expect(result.isError).toBe(false)
  ...
  it('renders a clean exit without a marker and an empty body as (no output)', async () => {
    const { ctx, bash } = await setup()
    bash.handler = () => runResult('hi\n')
    const clean = await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'say hi' })
    expect(text(clean)).toBe('hi\n')

    bash.handler = () => runResult('')
    const empty = await call(ctx, 'dsh_pwsh', { command: 'Write-Output -NoNewline ""', description: 'nothing' })
  ...
  it('renders stderr-only output without a stdout prefix', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', { command: 'fail', description: 'fail' })
  ...
  it('renders the truncation notice with the spill path, then markers', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', { command: 'noisy', description: 'noise' })
    ...
    const timedOut = await call(ctx, 'dsh_pwsh', { command: 'slow', description: 'slow' })
    // A timeout kill carries both facts, mirroring the dsh_bash tool's markers.
  ...
  it('translates an aborted run into the TOOL_ABORTED HarnessError', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', { command: 'Start-Sleep -Seconds 60', description: 'sleep' })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'say hi' }, agent)
    expect(result.isError).toBe(false)
  ...
  it('falls back to the deployment policy without an agent, and omits the field entirely without a confining executor', async () => {
    const { ctx, bash } = await setupSandboxed()
    await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'say hi' })
    expect(bash.requests[0]?.sandboxPolicy).toEqual({
      mode: 'read-only',
    ...
    await call(plain.ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'say hi' })
...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it('advertises the sandbox fields, the escalation clause, and the confined-mode contracts', async () => {
    ...
    const schema = ctx.tools.schemas().find(item => item.name === 'dsh_pwsh')!
    ...
    for (const args of [
      ...
      expect((await call(ctx, 'dsh_pwsh', args)).isError).toBe(true)
    }
  })
  ...
  it('rejects injected escalation without a sandbox and non-widening escalation without prompting', async () => {
    const plain = await setup()
    expect(text(await call(plain.ctx, 'dsh_pwsh', escalate))).toContain('not available in this composition')

    const { ctx } = await setupSandboxed(true)
    const prompted = vi.fn()
    ctx.on('approval/request', () => { prompted(); return Promise.resolve<ApprovalOutcome>('allowed-once') })
    const result = await call(ctx, 'dsh_pwsh', { ...escalate, sandbox_permissions: 'workspace-write' }, sandboxAgent('workspace-write'))
    ...
    expect(text(await call(ctx, 'dsh_pwsh', escalate, malformed))).toContain('not strictly wider')
  })

  it('fails closed when approval cannot be routed', async () => {
    const withoutService = await setupSandboxed()
    expect(text(await call(withoutService.ctx, 'dsh_pwsh', escalate, sandboxAgent()))).toContain('no approval service')

    const withService = await setupSandboxed(true)
    expect(text(await call(withService.ctx, 'dsh_pwsh', escalate))).toContain('no agent to route')
    expect(text(await call(withService.ctx, 'dsh_pwsh', escalate, sandboxAgent()))).toContain('no approval channel')
  })
  ...
  it.each([
    ...
    const result = await call(ctx, 'dsh_pwsh', escalate, sandboxAgent())
  ...
  it('runs a granted foreground or background call under the approved mode', async () => {
    ...
    const background = await call(ctx, 'dsh_pwsh', { ...escalate, run_in_background: true }, agent)
  ...
  it('uses the session override for ordinary calls and evaluates widening against it', async () => {
    ...
    await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'ordinary' }, agent)
    ctx.on('approval/request', () => Promise.resolve<ApprovalOutcome>('allowed-once'))
    await call(ctx, 'dsh_pwsh', { ...escalate, sandbox_permissions: 'danger-full-access' }, agent)
...
describe('background execution through the job runtime', () => {
  it('run_in_background acks with the job id, readable through the REAL dsh_job_output tool', async () => {
    ...
    const started = await call(ctx, 'dsh_pwsh', { command: 'Write-Output bg-ok', description: 'test command', run_in_background: true })
    ...
    const read = await callUntilText(ctx, 'dsh_job_output', { job_id: 'pwsh-1' }, 'bg-ok')
    expect(text(read)).toContain('bg-ok')
    // A later read reports the terminal outcome in the generic status line.
    const final = await callUntilText(ctx, 'dsh_job_output', { job_id: 'pwsh-1' }, '[status: completed, exit code: 0]')
  ...
  it('a running background job is killable through the REAL dsh_job_kill tool', async () => {
    ...
    await call(ctx, 'dsh_pwsh', { command: 'Start-Sleep -Seconds 60', description: 'test command', run_in_background: true })

    const killed = await call(ctx, 'dsh_job_kill', { job_id: 'pwsh-1' })
    ...
    const final = await call(ctx, 'dsh_job_output', { job_id: 'pwsh-1', wait: true })
  ...
  it('a background job started by an agent is registered with that agent as owner', async () => {
    ...
    const started = await call(ctx, 'dsh_pwsh', { command: 'Start-Sleep -Seconds 60', description: 'test command', run_in_background: true }, agent)
    expect(text(started)).toBe('started background job pwsh-1')

    const anon = await call(ctx, 'dsh_job_output', { job_id: 'pwsh-1' })
    expect(anon.isError).toBe(true)
    expect(text(anon)).toMatch(/belongs to another session/)

    const killed = await call(ctx, 'dsh_job_kill', { job_id: 'pwsh-1' }, agent)
    expect(killed.isError).toBe(false)
    await call(ctx, 'dsh_job_output', { job_id: 'pwsh-1', wait: true }, agent) // await settlement — no orphan
  })

  it('fails loud when the job runtime is not loaded', async () => {
    const { ctx } = await setup() // no LocalJobRegistry / ToolTasks
    const result = await call(ctx, 'dsh_pwsh', { command: 'Start-Sleep -Seconds 60', description: 'test command', run_in_background: true })
    expect(result.isError).toBe(true)
  ...
  it('enableRunInBackground: false removes the parameter and flips the description', async () => {
    ...
    const forced = await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'test command', run_in_background: true })
    expect(forced.isError).toBe(true)
    expect(text(forced)).toContain('run_in_background is disabled for this deployment')
    const foreground = await call(ctx, 'dsh_pwsh', { command: 'Write-Output hi', description: 'test command' })
    expect(foreground.isError).toBe(false)
  })
  ...
  it('applies the built-in background default when apply() receives a bare config', async () => {
    ...
    expect(schema.description).toContain('dsh_job_output')
  })
})
...
describe('UI presentation', () => {
  it('a real execute presents a completed foreground run as a terminal card with the parsed exit pill', async () => {
    ...
    const result = await call(ctx, 'dsh_pwsh', args)
    const view = ctx.tools.get('dsh_pwsh')?.presentResult?.(args, result)
    ...
    // raw output with a clean exit-0 pill, mirroring the dsh_bash tool.
  ...
  it('the pending call view is a terminal card carrying command, description, and optional cwd', async () => {
    const { ctx } = await setup()
    const definition = ctx.tools.get('dsh_pwsh')
  ...
  it('a background pending call renders the generic card like the dsh_bash tool', async () => {
    const { ctx } = await setup()
    const definition = ctx.tools.get('dsh_pwsh')
  ...
  it('presentResult: a non-zero exit and a signal kill parse into exitCode / signal', async () => {
    const { ctx } = await setup()
    const present = ctx.tools.get('dsh_pwsh')
  ...
  it('presentResult: markers a pill CANNOT show (timeout) stay in the terminal output', async () => {
    const { ctx } = await setup()
    const args = { command: 'x', description: 'x' }
    expect(ctx.tools.get('dsh_pwsh')?.presentResult?.(
      args,
  ...
  it('presentResult: a clean exit-0 whose output ENDS in marker-like text is NOT read as a failure', async () => {
    ...
    const out = ctx.tools.get('dsh_pwsh')!.presentResult!(args, { content: [{ type: 'text', text: '[exit code: 5]' }], isError: false })
    expect(out).toEqual({ card: 'terminal', output: '[exit code: 5]', exitCode: 0 })
    // Same for a fake signal marker with no leading newline.
    const sig = ctx.tools.get('dsh_pwsh')!.presentResult!(args, { content: [{ type: 'text', text: '[killed by signal: SIGKILL]' }], isError: false })
  ...
  it('presentResult: a run_in_background ack is a generic card and carries no exit pill', async () => {
    const { ctx } = await setup()
    const result = ctx.tools.get('dsh_pwsh')!.presentResult!(
  ...
  it('presentResult: an isError result is a generic card (no real process exit to report)', async () => {
    const { ctx } = await setup()
    const out = ctx.tools.get('dsh_pwsh')!.presentResult!(
