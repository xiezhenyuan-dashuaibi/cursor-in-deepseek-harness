...
describe('tool-bash-persistent', () => {
  it('registers a configurable schema and reuses one owner shell', async () => {
    ...
    expect(ctx.tools.schemas().map(item => item.name)).toEqual(['dsh_bash'])
    expect(schema?.description).toBe('deployment-specific persistent shell')
    expect(schema?.parameters).toMatchObject({
      required: ['command'],
      properties: { command: { type: 'string' } },
    })
    expect(ctx.tools.get('dsh_bash')?.presentCall?.({ command: 'pwd' }))
    ...
    expect(ctx.tools.get('dsh_bash')).toBeUndefined()
  })
