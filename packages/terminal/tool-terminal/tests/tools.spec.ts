...
let callNumber = 0
const TOOL_NAMES = ['dsh_terminal_open', 'dsh_terminal_send', 'dsh_terminal_read', 'dsh_terminal_signal', 'dsh_terminal_close', 'dsh_terminal_list'] as const
...
describe('tool-terminal foreground API', () => {
  it('registers exactly six schemas and drives the full owner-scoped lifecycle', async () => {
    ...
    const spawned = await call(ctx, 'dsh_terminal_open', { type: 'stub', name: 'main' }, agent)
    ...
    const listed = await call(ctx, 'dsh_terminal_list', {}, agent)
    ...
    const read = await call(ctx, 'dsh_terminal_read', { sessionId: 'pty-1' }, agent)
    ...
    const signalled = await call(ctx, 'dsh_terminal_signal', { sessionId: 'pty-1', signal: 'SIGINT' }, agent)
    ...
    const sent = await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'echo hi' }, agent)
    ...
    const closed = await call(ctx, 'dsh_terminal_close', { sessionId: 'pty-1' }, agent)
    ...
    const empty = await call(ctx, 'dsh_terminal_list', {}, agent)
  ...
  it('fails without an initiating agent and rejects background before writing', async () => {
    ...
    expect((await call(ctx, 'dsh_terminal_open', { type: 'stub' })).isError).toBe(true)
    await call(ctx, 'dsh_terminal_open', { type: 'stub' }, agent)
    const result = await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'sleep 1', run_in_background: true }, agent)
  ...
  it('validates required values and forwards optional spawn/read arguments', async () => {
    const { ctx, agent } = await setup(false)
    expect((await call(ctx, 'dsh_terminal_open', { type: '' }, agent)).isError).toBe(true)
    expect((await call(ctx, 'dsh_terminal_send', { sessionId: '', text: 'x' }, agent)).isError).toBe(true)
    expect((await call(ctx, 'dsh_terminal_send', { sessionId: 1, text: 'x' }, agent)).isError).toBe(true)
    expect((await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 1 }, agent)).isError).toBe(true)
    await call(ctx, 'dsh_terminal_open', { type: 'stub', name: 'named', cwd: '/tmp' }, agent)
    expect(text(await call(ctx, 'dsh_terminal_read', { sessionId: 'pty-1', offset: 2, count: 3 }, agent))).toContain('history')
  })

  it('declares terminal presentation only for foreground sends', async () => {
    const { ctx } = await setup(false)
    const definition = ctx.tools.get('dsh_terminal_send')
    ...
    expect(ctx.tools.get('dsh_terminal_open')?.presentCall?.({ type: 'stub' })).toMatchObject({ card: 'generic', title: 'Open terminal stub' })
    expect(ctx.tools.get('dsh_terminal_open')?.presentCall?.({ type: 'stub', name: 'main' })).toMatchObject({ card: 'generic', title: 'Open terminal main' })
    expect(ctx.tools.get('dsh_terminal_read')?.presentCall?.({ sessionId: 'pty-1' })).toMatchObject({ card: 'generic', title: 'Read terminal pty-1' })
    expect(ctx.tools.get('dsh_terminal_signal')?.presentCall?.({ sessionId: 'pty-1', signal: 'SIGINT' })).toMatchObject({ card: 'generic', title: 'Signal terminal pty-1' })
    expect(ctx.tools.get('dsh_terminal_close')?.presentCall?.({ sessionId: 'pty-1' })).toMatchObject({ card: 'generic', title: 'Close terminal pty-1' })
    expect(ctx.tools.get('dsh_terminal_list')?.presentCall?.({})).toMatchObject({ card: 'generic', title: 'List terminal sessions' })
  })
  ...
  it('configuration-gates background sends and validates the final result bound', async () => {
    ...
    const definition = disabled.ctx.tools.get('dsh_terminal_send')
    ...
    await call(disabled.ctx, 'dsh_terminal_open', { type: 'stub' }, disabled.agent)
    expect((await call(disabled.ctx, 'dsh_terminal_send', {
    ...
    expect(defaults.ctx.tools.get('dsh_terminal_send')?.parameters).toHaveProperty('properties.run_in_background')
  ...
  it('bounds terminal results after policy decisions and pipeline failures', async () => {
    ...
    ctx.on('tools/pre-execute', async (exec, next) => {
      if (exec.name === 'dsh_terminal_list') return { kind: 'deny', reason: 'd'.repeat(1_000) }
      if (exec.name === 'dsh_terminal_signal') throw new Error(`pre failed: ${'p'.repeat(1_000)}`)
      return next()
    })
    ctx.on('tools/execute', async (exec, next) => {
      if (exec.name === 'dsh_terminal_close') throw new Error(`around failed: ${'e'.repeat(1_000)}`)
      return next()
    })
    ctx.on('tools/post-execute', async (exec, _result, next) => {
      if (exec.name === 'dsh_terminal_open') {
        return { kind: 'accept', content: [{ type: 'text', text: 'a'.repeat(1_000) }] }
      }
      if (exec.name === 'dsh_terminal_read') {
        return { kind: 'block', feedback: [{ type: 'text', text: 'b'.repeat(1_000) }] }
      }
      if (exec.name === 'dsh_terminal_send') throw new Error(`post failed: ${'o'.repeat(1_000)}`)
      return next()
    })

    const denied = await call(ctx, 'dsh_terminal_list', {}, agent)
    expect(denied.isError).toBe(true)
    expect(Buffer.byteLength(text(denied))).toBeLessThanOrEqual(64)
    expect(text(denied)).toContain('[output truncated]')

    const replaced = await call(ctx, 'dsh_terminal_open', { type: 'stub' }, agent)
    expect(replaced.isError).toBe(false)
    expect(Buffer.byteLength(text(replaced))).toBeLessThanOrEqual(64)
    expect(text(replaced)).toContain('[output truncated]')

    const blocked = await call(ctx, 'dsh_terminal_read', { sessionId: 'pty-1' }, agent)
    expect(blocked.isError).toBe(true)
    expect(Buffer.byteLength(text(blocked))).toBeLessThanOrEqual(64)
    expect(text(blocked)).toContain('[output truncated]')

    const failures = [
      await call(ctx, 'dsh_terminal_signal', { sessionId: 'pty-1', signal: 'SIGINT' }, agent),
      await call(ctx, 'dsh_terminal_close', { sessionId: 'pty-1' }, agent),
      await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'work' }, agent),
    ]
  ...
  it('leaves a structured around-dispatch failure unchanged', async () => {
    const { ctx, agent } = await setup(false, { maxResultBytes: 64 })
    ctx.on('tools/execute', async (exec, next) => exec.name === 'dsh_terminal_list'
      ? { content: [], isError: true, error: { message: 'structured failure' } }
      : next())
    const result = await call(ctx, 'dsh_terminal_list', {}, agent)
    expect(result.isError).toBe(true)
...
describe('tool-terminal task integration', () => {
  it('registers a generic task and exposes incremental output', async () => {
    ...
    await call(ctx, 'dsh_terminal_open', { type: 'stub' }, agent)
    const started = await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'build', run_in_background: true }, agent)
    ...
    const output = await call(ctx, 'dsh_job_output', { job_id: 'pty-send-1', wait: true }, agent)
  ...
  it('bounds foreground and background results after terminal and task metadata', async () => {
    ...
    const foreground = await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'foreground' }, agent)
    ...
    await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'background', run_in_background: true }, agent)
    const background = await call(ctx, 'dsh_job_output', { job_id: 'pty-send-1', wait: true }, agent)
    expect(Buffer.byteLength(text(background))).toBeLessThanOrEqual(64)
  ...
  it('rejects pre-aborted background calls, maps job cancellation, and contains operation failure', async () => {
    ...
    controller.abort()
    expect((await callWithSignal(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'x', run_in_background: true }, agent, controller.signal)).isError).toBe(true)

    stub.sessions[0]!.autoSettle = false
    expect(text(await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: '', run_in_background: true }, agent))).toContain('pty-send-1')
    expect(text(await call(ctx, 'dsh_job_kill', { job_id: 'pty-send-1' }, agent))).toContain('requested cancellation')
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'pty-send-1' }, agent))).toContain('[status: killed')

    stub.sessions[0]!.rejectOperation = true
    stub.sessions[0]!.autoSettle = false
    expect(text(await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'bad', run_in_background: true }, agent))).toContain('pty-send-2')
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(text(await call(ctx, 'dsh_job_output', { job_id: 'pty-send-2' }, agent))).toContain('[status: failed')
  })
  ...
  it('reports foreground cancellation after the terminal operation settles', async () => {
    ...
    stub.sessions[0]!.autoSettle = false
    const controller = new AbortController()
    const pending = callWithSignal(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'sleep' }, agent, controller.signal)
  ...
  it('renders the already-closing kill result', async () => {
    ...
    const second = call(ctx, 'dsh_terminal_close', { sessionId: 'pty-1' }, agent)
  ...
  it('renders an exited session detail for background completion', async () => {
    ...
    await call(ctx, 'dsh_terminal_send', { sessionId: 'pty-1', text: 'exit', run_in_background: true }, agent)
    const output = await call(ctx, 'dsh_job_output', { job_id: 'pty-send-1', wait: true }, agent)
