...
Six model-facing tools over `ctx.terminals`: `dsh_terminal_open`, `dsh_terminal_send`, `dsh_terminal_read`, `dsh_terminal_signal`, `dsh_terminal_close`, and `dsh_terminal_list`. Every operation requires the exact initiating `Agent`, so a model cannot address another agent's terminal even if it learns the id.

`dsh_terminal_send(run_in_background: true)` reuses `ctx.jobs`; job preflight and the PTY service's exclusive per-session send reservation occur before the job id is returned, completion is collected with `dsh_job_output`, and `dsh_job_kill` delivers `SIGINT` to the foreground process group. Foreground sends use terminal call/result cards. Background sends use a generic execute card; open, read, signal, close, and list use generic `execute`, `read`, `execute`, `delete`, and `read` cards respectively. None declares source locations.
