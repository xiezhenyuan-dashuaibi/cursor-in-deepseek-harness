...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Send text to a persistent terminal. By default Enter is submitted and the call waits for a prompt, stdin wait, output silence, timeout, or session exit.'
      + (enableRunInBackground ? ' Background mode returns a job id for dsh_job_output/dsh_job_kill.' : ''),
    parameters: {
      sessionId: { type: 'string', required: true, description: 'Terminal session id returned by dsh_terminal_open or dsh_terminal_list.' },
      ...
      ...enableRunInBackground
        ? { run_in_background: { type: 'boolean' as const, description: 'Return a job id immediately; collect with dsh_job_output or stop with dsh_job_kill.' } }
        : {},
    },
    finalizeContent,
    output: {
  ...
  ctx.tools.register(defineTool({
    name: 'dsh_terminal_signal',
    description: 'Send an allowed signal to the current foreground process group of a persistent terminal.',
    parameters: {
      sessionId: { type: 'string', required: true, description: 'Terminal session id.' },
      signal: { type: 'string', required: true, enum: ['SIGINT', 'SIGTERM', 'SIGKILL', 'SIGTSTP', 'SIGHUP'], description: 'Signal to deliver. Shell-targeted SIGKILL is rejected; use dsh_terminal_close.' },
    },
    finalizeContent,
    output: {
      schema: {
