...
describe('scrubRequestHeaders', () => {
  ...
  it('replaces header system and tools with tokens, keeping config and reason', () => {
    const ev = headerEvent({
      ...
      tools: [{ name: 'dsh_read', description: 'Read a file.', parameters: { type: 'object' } }],
    })
  ...
  it('scrubs only system prompt payloads while keeping tools verbatim', () => {
    const header = JSON.stringify({
      ...
      data: {
        header: {
          system: 'full prompt',
          tools: [{ name: 'dsh_read', description: 'full schema' }],
        },
        reason: 'initial',
      },
    })
    const changed = JSON.stringify({
      ...
      data: {
        header: {
          system: 'new prompt',
          tools: [{ name: 'dsh_read', description: 'changed schema' }],
        },
        reason: 'change',
      },
    })
    const toolsOnly = JSON.stringify({
      ...
      data: { header: { tools: [{ name: 'dsh_read', description: 'schema only' }] }, reason: 'resume' },
    })
  ...
  it('scrubs only tool-schema payloads while keeping prompts verbatim', () => {
    const header = JSON.stringify({
      ...
      data: {
        header: {
          system: 'full prompt',
          tools: [{ name: 'dsh_read', description: 'full schema', parameters: { type: 'object' } }],
        },
        reason: 'initial',
      },
    })
    const changed = JSON.stringify({
      ...
      data: {
        header: {
          system: 'new prompt',
          tools: [{ name: 'dsh_grep', description: 'new schema' }],
        },
        reason: 'change',
      },
    })
