  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Register globally or in the calling agent scope. Scoped tools shadow globals; duplicates within one layer and the reserved `dsh_run_code` name fail.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Allow a listener to replace content in the DURABLE LOG COPY of one `dsh_run_code` sub-dispatch outcome before the bridge appends its `tool/code-dispatch` event.',
    description: 'Allow a listener to replace content in the DURABLE LOG COPY of one `dsh_run_code` sub-dispatch outcome before the bridge appends its `tool/code-dispatch` event. `next()` keeps the content unchanged; a listener may return replacement blocks (e.g. the spill policy\'s preview + locator for an oversized text result). Only the logged copy is affected — the program already received the complete value, and the model sees neither. A throwing listener is contained: the bridge falls back to logging the original settled content. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent\'s dispatches.',
