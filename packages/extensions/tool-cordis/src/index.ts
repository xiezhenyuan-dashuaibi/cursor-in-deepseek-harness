...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'method for dsh_cordis_inspect_query from its result. Do not guess names or treat an Inspect method as a business '
  ...
  ctx.tools.register(defineTool({
    name: 'dsh_cordis_inspect_query',
    description:
      'Run a read-only query explicitly declared by an Inspect Provider. platform, provider, and method must come '
      + 'from dsh_cordis_inspect_list, and input must satisfy that method\'s schema. Use this Tool before dsh_cordis_define '
    ...
    parameters: {
      ...
      provider: { type: 'string', required: true, description: 'Exact Provider ID returned by dsh_cordis_inspect_list.' },
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      pluginId: { type: 'string', description: 'Stable Plugin ID returned by dsh_cordis_define or injected by @pluginId; omit it to list every current Plugin.' },
    ...
    execute(args, exec): Promise<JsonValue> {
      ...
      if (args.packageId !== undefined && args.pluginId === undefined) {
        throw new Error('dsh_cordis_inspect_self packageId requires pluginId')
      }
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'does not request approval, execute apply, or change currentPackageId. On success, call dsh_cordis_run with the '
      + 'returned IDs.',
    parameters: {
      plugin: {
        required: true,
    ...
    output: {
      ...
      render: (_args, value) => [{
        ...
        text: `Defined ${value.pluginId}/${value.packageId} (${value.name}); it is not running yet. `
          + 'Use dsh_cordis_run to activate this Package.',
      }],
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'dsh_cordis_inspect_self, correct the same Plugin, and retry autonomously. Do not request approval again after '
      + 'the user rejects it.',
    parameters: {
      pluginId: { type: 'string', required: true, description: 'Stable Plugin ID returned by dsh_cordis_define.' },
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'temporarily; use dsh_cordis_undefine for permanent removal.',
    parameters: {
...
function renderReference(reference: ReturnType<Context['dynamicCordisRunner']['reference']> & {}): string {
  ...
  return [
    ...
    `Before modifying it, call dsh_cordis_inspect_self with pluginId="${reference.pluginId}" and packageId="${reference.packageId}" to read the exact metadata and source.`,
    `Use dsh_cordis_define with plugin.kind="existing" and the original pluginId="${reference.pluginId}" to append an immutable Package.`,
    `Do not create a new Plugin for this request. After dsh_cordis_define succeeds, call dsh_cordis_run mode="${mode}" with the returned packageId.`,
    '</cordis_dynamic_plugin_context>',
  ].join('\n')
}
