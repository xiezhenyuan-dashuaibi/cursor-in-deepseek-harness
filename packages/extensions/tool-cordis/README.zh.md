...
- `dsh_cordis_inspect_list`：Host 与 Client 当前已知的 Inspect Provider，以及各自的只读方法与 schema。在 `dsh_cordis_inspect_query` 之前调用。
- `dsh_cordis_inspect_query`：执行一项已声明的检查方法（`Service.listService`、`Event.listEvents`、`Builtin.listBuiltins`、`Tool.listTools`、Client `Slots` 等）。精确的 `service`／`event`／槽 `root` 输入会把紧凑目录收窄为完整约定。
- `dsh_cordis_inspect_self`：本会话的动态 Plugin 与 Package；同时给出 `pluginId` 与 `packageId` 时返回源码与诊断，包括最后一次浏览器半装载上报。
- `dsh_cordis_define`：在语法预检两个半之后登记一个包（`name`、`purpose`，以及 host 半 `code` 和／或浏览器半 `client`）。此时不运行任何东西；用户会在会话里看到它的卡片和一个启动控件。铸出的 `dyn-<n>` 标识同时进入结果 value **与**持久的呈现元数据，卡片正是靠后者在 replay 中寻址运行动词。
- `dsh_cordis_run`：在沙箱中求值 host 半，并把浏览器半投递给每个打开的网页。对已在运行的包再次运行不会失败，而是重新投递当前版本——这正是被刷新过的页面把包取回来的方式。
- `dsh_cordis_stop`：把 host 半 dispose 到完全停稳，并从各页面撤回浏览器半；定义存续，可以再次运行。
- `dsh_cordis_undefine`：必要时先停止该包，再忘掉定义；它的卡片作为一条已卸载记录留在会话里。

面向模型的确切 schema 见[生成的工具目录](../../../docs/tool-catalog.md)。

动态包只存在于共享 DSH 进程内存中。它可跨后续轮次保持活跃，也可能影响同一进程中的其他会话，但会在 `dsh_cordis_stop`／`dsh_cordis_undefine`、工具集卸载或 DSH 重启后消失。它不会创建插件文件、安装任何包、修改 `cordis.yml` 或个人／项目配置、跨重启存续，也不能自动转为正式插件。若要保留实验结果，应让 agent（智能体）通过常规开发流程实现普通的本地、项目或仓库插件。每个动词都以会话为界：一个包只在定义它的那个会话里可见、可控。
...
`dsh_cordis_inspect_query` 对 Host `Service.listService`／`Event.listEvents` 渲染的是 `src/api-catalog.ts`，即工作区 Cordis 声明的生成投影：渲染好的方法签名、源码 JSDoc、带分发模式的 harness 事件，以及这些签名引用到的类型形状——全部由与 `docs/subsystems` 同一次 AST 遍历产出，因此模型读到的数据与渲染出的文档不可能彼此偏离。它是关于**仓库**的编译期事实，所以用 `pnpm run gen-cordis-api` 重新生成、用 `pnpm run verify-cordis-api` 守它的新鲜度。
...
每个工具都渲染 `generic` 卡片（`read`／`execute`／`delete`）；`dsh_cordis_define` 以 `rawInput` 携带提交的两个半，并用标签与用途作为卡片标题。presenter 是 args 的纯函数，结果保留默认文本渲染。Web 客户端注册自己的 keyed `dsh_cordis_define` 行（`@deepseek-ai/dsh-client-ui-cordis`），从调用参数与结果元数据里取标签、用途和铸出的标识；没有该注册的界面则退回到这张 generic 卡片。
...
该插件可见时，会话模型会看到生成的 [`dsh_cordis_inspect_list`、`dsh_cordis_inspect_query`、`dsh_cordis_inspect_self`、`dsh_cordis_define`、`dsh_cordis_run`、`dsh_cordis_stop` 和 `dsh_cordis_undefine` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-cordis)。
...
检查工具返回 JSON。`dsh_cordis_inspect_list` 枚举 Provider；`dsh_cordis_inspect_query` 返回一项方法的数据（紧凑目录省略 JSDoc；精确的 `service`、`event` 或槽 `root` 返回完整约定）。`dsh_cordis_inspect_self` 列出 Plugin 摘要，或某一个 Plugin 的 Package，或——在两个 id 都给出时——源码与诊断（含最后一次浏览器半装载上报）。空的 Plugin 列表说明定义只存在于本进程内存中。define 回答该包已定义、尚未运行，并给出用于运行的标识；run 报告版本号、host 半提供或等待什么，以及是否有页面确认了浏览器半；stop 与 undefine 各以一行确认。每一次拒绝都是携带 runner 教学文案的工具错误。提交的程序保留在 assistant 工具调用历史中。
...
### dsh_cordis_run 后的后续请求

#### 模型看到的内容

运行中的包可以注册工具、提示词贡献或监听器，改变其目标 scope 的后续请求；`dsh_cordis_stop` 与 `dsh_cordis_undefine` 会在完全停稳后移除这些贡献。
