...
describe('versioned Cordis card models', () => {
  it('reads symmetric Host and Client source fields from dsh_cordis_define', () => {
  ...
  it('reads exact activation metadata from a successful dsh_cordis_run result', () => {
    const card = cordisRunCard({
      kind: 'tool-result',
      seq: 9,
      time: 2,
      callId: 'call-2',
      call: { name: 'dsh_cordis_run', argsRaw: JSON.stringify({ pluginId: PLUGIN, packageId: PACKAGE, mode: 'run' }) },
      callTime: 1,
  ...
  it('keeps the target identities while dsh_cordis_run waits for approval', () => {
    const card = cordisRunCard({
      callId: 'call-3',
      name: 'dsh_cordis_run',
