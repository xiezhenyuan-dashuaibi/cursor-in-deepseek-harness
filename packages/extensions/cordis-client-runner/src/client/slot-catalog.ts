...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:63',
  },
  {
    key: 'overlay-desktop.body',
    kind: 'single',
    scope: 'root',
    summary: 'Full-viewport desktop product under cards and Cursor.',
    doc: 'Full-viewport desktop product under cards and Cursor. Occupied by one\npage plugin. Declared by `overlay-desktop`; do not occupy `root` or\n`overlay-card.body`.',
    registerOptions: [],
    ownerProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'shell.overlay\' (client-ui-overlay-desktop), so it exists while that entry is mounted',
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-desktop.body\', () => ctx.slots.register(\n      { name: \'overlay-desktop.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-overlay-desktop/src/client/contract/slots.ts:14',
  },
  {
    key: 'overlay-shaped.body',
    kind: 'list',
    scope: 'root',
    summary: 'Concurrent arbitrary-shape overlay occupants.',
    doc: 'Concurrent arbitrary-shape overlay occupants. Occupied by many page\nplugins at once. Declared by `overlay-shaped`; do not occupy `root`,\n`overlay-card.body`, or `overlay-desktop.body`.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
