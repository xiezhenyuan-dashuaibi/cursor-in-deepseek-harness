...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    occupants: [
      'client-ui-petshop Page',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-2.body\', () => ctx.slots.register(\n      { name: \'overlay-card-2.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:50',
  },
  {
    key: 'overlay-card-2.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 2.',
    doc: 'Trailing chrome list of overlay card seat 2. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-2.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-2.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:92',
  },
  {
    key: 'overlay-card-3.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 3.',
    doc: 'Body of overlay card seat 3. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    declaredBy: 'the runtime itself (built in; always present)',
    occupants: [
      'client-ui-petshop-hub Page',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-3.body\', () => ctx.slots.register(\n      { name: \'overlay-card-3.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:55',
  },
  {
    key: 'overlay-card-3.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 3.',
    doc: 'Trailing chrome list of overlay card seat 3. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-3.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-3.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:97',
  },
  {
    key: 'overlay-card-4.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 4.',
    doc: 'Body of overlay card seat 4. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    declaredBy: 'the runtime itself (built in; always present)',
    occupants: [
      'client-ui-petshop-store Page',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-4.body\', () => ctx.slots.register(\n      { name: \'overlay-card-4.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:60',
  },
  {
    key: 'overlay-card-4.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 4.',
    doc: 'Trailing chrome list of overlay card seat 4. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-4.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-4.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:102',
  },
  {
    key: 'overlay-card-5.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 5.',
    doc: 'Body of overlay card seat 5. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-5.body\', () => ctx.slots.register(\n      { name: \'overlay-card-5.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:65',
  },
  {
    key: 'overlay-card-5.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 5.',
    doc: 'Trailing chrome list of overlay card seat 5. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-5.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-5.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:107',
  },
  {
    key: 'overlay-card-6.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 6.',
    doc: 'Body of overlay card seat 6. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-6.body\', () => ctx.slots.register(\n      { name: \'overlay-card-6.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:70',
  },
  {
    key: 'overlay-card-6.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 6.',
    doc: 'Trailing chrome list of overlay card seat 6. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-6.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-6.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:112',
  },
  {
    key: 'overlay-card-7.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 7.',
    doc: 'Body of overlay card seat 7. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-7.body\', () => ctx.slots.register(\n      { name: \'overlay-card-7.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:75',
  },
  {
    key: 'overlay-card-7.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 7.',
    doc: 'Trailing chrome list of overlay card seat 7. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-7.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-7.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:117',
  },
  {
    key: 'overlay-card-8.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of overlay card seat 8.',
    doc: 'Body of overlay card seat 8. Same owner share as `overlay-card.body`.\nExists while that numbered card is mounted.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-8.body\', () => ctx.slots.register(\n      { name: \'overlay-card-8.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:80',
  },
  {
    key: 'overlay-card-8.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing chrome list of overlay card seat 8.',
    doc: 'Trailing chrome list of overlay card seat 8. Same contract as\n`overlay-card.chrome.trailing`. Exists while that numbered card is mounted.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card-8.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card-8.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:122',
  },
  {
    key: 'overlay-card.body',
    kind: 'single',
    scope: 'root',
    summary: 'Body of the movable overlay card.',
    doc: 'Body of the movable overlay card. Occupied by a page plugin.\nThe page is a designed webpage; call `preferFrame` on mount for opening size.\nDeclared by `overlay-card`; do not occupy `root` or `cursor-agent`.',
    registerOptions: [],
    ownerProps: [
      '/** Owner share rendered into a card body slot. */\nexport type OverlayCardBodyOwner = {\n  /**\n   * Ask this card to adopt this opening frame. Call once on mount.\n   * Does not move other cards. Later user drag/resize is kept until this occupant unmounts.\n   * @param frame - preferred size, and optionally origin.\n   */\n  preferFrame: (frame: OverlayCardPreferredFrame) => void\n}',
    ],
    ...
    declaredBy: 'the runtime itself (built in; always present)',
    occupants: [
      'client-ui-barber Page',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card.body\', () => ctx.slots.register(\n      { name: \'overlay-card.body\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:45',
  },
  {
    key: 'overlay-card.chrome.trailing',
    kind: 'list',
    scope: 'root',
    summary: 'Trailing controls on the default compact title bar (minimize and similar).',
    doc: 'Trailing controls on the default compact title bar (minimize and similar).\nPointer events here do not start a card drag; they still raise the window.\nAbsent occupants leave the region blank. A tall branded title bar is an\nedit of OverlayCard, not this slot. Declared by `overlay-card`.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'overlay-card.chrome.trailing\', () => ctx.slots.register(\n      { name: \'overlay-card.chrome.trailing\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-float-window/src/client/contract/slots.ts:87',
  },
  {
    key: 'root',
    kind: 'single',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar.settings\' (client-ui-settings-general), so it exists while that entry is mounted',
    occupants: [
      'client-ui-cursor-agent SkipOnboarding',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-cursor-agent CursorPanel id \'cursor-agent\'',
  ...
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: dsh_ask_user_question, dsh_bash, dsh_cordis_define, dsh_cordis_run, dsh_cordis_stop, dsh_cordis_undefine, dsh_edit, dsh_glob, dsh_grep, dsh_read, dsh_skill, dsh_todo_write, dsh_web_fetch, dsh_web_search, dsh_write',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.chat.node\' (client-ui-tool), so it exists while that entry is mounted',
    occupants: [
      'client-ui-skill SkillRow key \'dsh_skill\'',
      'client-ui-tool AskQuestionRow key \'dsh_ask_user_question\'',
      'client-ui-tool BashRow key \'dsh_bash\'',
      'client-ui-tool FileMutationRow key \'dsh_edit\'',
      'client-ui-tool FileMutationRow key \'dsh_write\'',
      'client-ui-tool ReadRow key \'dsh_read\'',
      'client-ui-tool SearchRow key \'dsh_grep\'',
      'client-ui-tool SearchRow key \'dsh_glob\'',
      'client-ui-tool TodoRow key \'dsh_todo_write\'',
      'client-ui-tool WebRow key \'dsh_web_search\'',
      'client-ui-tool WebRow key \'dsh_web_fetch\'',
      'client-ui-cordis CordisDefineRow key \'dsh_cordis_define\'',
      'client-ui-cordis CordisRunRow key \'dsh_cordis_run\'',
      'client-ui-cordis CordisActionRow key \'dsh_cordis_stop\'',
      'client-ui-cordis CordisActionRow key \'dsh_cordis_undefine\'',
    ],
    replaceRisk: 'shadows-shipped-ui',
  ...
  },
  {
    ...
    kind: 'keyed',
    scope: 'session',
    summary: 'Interactive Package-owned region rendered inside the latest eligible `dsh_cordis_run` card in the conversation flow.',
