...
const NODE_API_REDIRECTS: Record<string, string> = {
  require:
    ...
    + '[\'web\'] for HTTP, [\'bash\'] for processes; query Service.listService with dsh_cordis_inspect_query first.',
  ...
  fetch:
    ...
    + '(query Host Service.listService with dsh_cordis_inspect_query for its methods).',
}
