...
export class DynamicCordisRunnerService extends TypertRemoteService {
  ...
  define(request: DynamicCordisDefineRequest): DynamicCordisDefineReceipt {
    ...
    if (name.length === 0) throw new Error('dsh_cordis_define needs a non-empty `name`')
    if (purpose.length === 0) throw new Error('dsh_cordis_define needs a non-empty `purpose`')
    if (request.code.host === undefined && request.code.client === undefined) {
      throw new Error('dsh_cordis_define needs `code.host`, `code.client`, or both')
    }
    ...
    if (request.plugin.kind === 'new') {
      ...
      if (!/^[a-z]{3,6}$/.test(prefix)) {
        throw new Error('dsh_cordis_define `plugin.idPrefix` must contain 3–6 lowercase English letters')
      }
  ...
  private steerRunOutcome(
    ...
    if (settled.ok) {
      ...
      text = `Cordis ${pending.mode} ${identity} failed after dsh_cordis_run returned ${returnedStatus}: `
    ...
    agent.steer(createUserMessage({
      content: [{
        ...
        text: `Cordis Client UI ${plugin.pluginId}/${definition.packageId} (${pluginRunId}) failed while rendering `
          ...
          + 'activate that Package autonomously with dsh_cordis_run mode:"update".',
      }],
      source: { kind: 'plugin', plugin: 'cordis-host-runner' },
    }))
  }
    ...
    agent.steer(createUserMessage({
      content: [{
        ...
        text: `Cordis Host handler ${plugin.pluginId}/${run.packageId} (${run.pluginRunId}) failed when the Client called `
          ...
          + 'the new Package autonomously with dsh_cordis_run mode:"update". If the handler needs a Service, either declare '
    ...
    agent.steer(createUserMessage({
      content: [{
        ...
        text: `Cordis ${platform} guard rejected runtime code in ${plugin.pluginId}/${run.packageId} `
          ...
          + 'activate it autonomously with dsh_cordis_run mode:"update".',
      }],
      source: { kind: 'plugin', plugin: 'cordis-host-runner' },
    }))
  }
