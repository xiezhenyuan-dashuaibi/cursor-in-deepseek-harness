...
| 包 | 角色 | ctx 键 |
|---|---|---|
| [`tool-cordis/`](tool-cordis/README.md) | `dsh_cordis_inspect_list`／`dsh_cordis_inspect_query`／`dsh_cordis_inspect_self`／`dsh_cordis_define`／`dsh_cordis_run`／`dsh_cordis_stop`／`dsh_cordis_undefine` 工具：读取当前进程运行时，并在一个自有分组 fiber 下管理内存中的动态包 | 注册到 `ctx.tools` |
