...
@deepseek-ai/dsh-tool-jobs       dsh_job_output/dsh_job_list/dsh_job_kill schemas + completion notices
...
- **non-local skill providers** — the bundle ships the skill registry, the local filesystem provider, and the `dsh_skill` tool; deployments can add other providers such as embedded or remote catalogs as siblings.
