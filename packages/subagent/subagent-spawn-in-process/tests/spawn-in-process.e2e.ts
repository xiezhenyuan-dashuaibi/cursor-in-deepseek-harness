...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('spawn backend with-key smoke', () => {
  it('a parent delegates to a child that writes a file on disk', async () => {
    ...
    parent.followup(createUserMessage({
      ...
      'Use the dsh_subagent tool to delegate this exact task: "Use the dsh_bash tool to write the text '
    ...
    // The parent's log records the dsh_subagent tool/call + its result (not the
    // child's internal steps).
    const events = [...parent.session.events]
    const subagentCalls = events.filter(e => e.type === 'tool/call' && e.data.name === 'dsh_subagent')
