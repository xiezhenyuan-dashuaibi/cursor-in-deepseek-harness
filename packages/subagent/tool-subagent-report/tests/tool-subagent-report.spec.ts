...
describe('dsh-tool-subagent-report', () => {
  it('registers report only in continuable child scopes', async () => {
    ...
    const schemas = ctx.tools.schemas(child).filter(schema => schema.name === 'dsh_report')
  ...
  it('does not imply parent controls and survives a global-tool allow-list', async () => {
    const { ctx, parent } = await setup()
    expect(ctx.tools.schemas().map(schema => schema.name)).not.toContain('dsh_send_message')
    await ctx.plugin(control)
    expect(ctx.tools.schemas().map(schema => schema.name)).toContain('dsh_send_message')
    ...
    expect(names).not.toContain('dsh_send_message')
  })
  ...
  it('scopes the report guidance to the child that owns it', async () => {
    ...
    expect(guidance?.text).toContain('Deliver your result with the dsh_report tool before you finish')
  ...
  it('rolls back materialization when a setup contribution revokes itself', async () => {
    ...
    // ghost that `dsh_list_agents` surfaces and `dsh_send_message` can resurrect.
