/**
 * The child-scoped `dsh_report` tool and its usage guidance, installed into every
...
/**
 * Install `dsh_report` and its usage guidance into one continuable child's scope.
...
export function installReportTool(
  ...
  const disposeSection = childCtx.systemPrompt.section({
    ...
    text: 'Deliver your result with the dsh_report tool before you finish: call it once with a self-contained '
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError(
        [error, rollbackError],
        'failed to register the dsh_report tool and roll back its prompt guidance',
      )
    }
    throw error
  }
