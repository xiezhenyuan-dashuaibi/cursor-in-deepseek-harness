...
import { AssistantOutputFold, diagnosticFromTurnEnd, settleRunResult, subprocessRunHandle } from '@deepseek-ai/dsh-subagent'
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const result: Promise<SubagentResult> = settleRunResult({
    attempt: async () => {
      ...
      const diagnostic = diagnosticFromTurnEnd(reason)
      return {
        output: collectOutput(),
        stopReason: sdkStopReason(reason),
