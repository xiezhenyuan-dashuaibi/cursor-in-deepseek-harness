...
describe('dsh-subagent-dsh-sdk provider', () => {
  ...
  it('flattens a child turn error into stopReason error and keeps partial text', async () => {
    const ctx = await setup({
      ...
      FAKE_STATUS: 'error',
      FAKE_TEXT: 'partial answer',
      FAKE_ERROR_MESSAGE: 'no API key for provider route "deepseek-official"',
    })
    const run = await ctx.subagents.start('dsh-sdk', request())
    const result = await run.result
    expect(result.stopReason).toBe('error')
    expect(result.diagnostic).toBe('no API key for provider route "deepseek-official"')
