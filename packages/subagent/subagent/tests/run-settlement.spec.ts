...
describe('outcome mapping helpers', () => {
  ...
  it('copies an error diagnostic onto the failed Task outcome and leaves assistant text off the job', async () => {
    await expect(settleRun({
      ...
      result: Promise.resolve({
        ...
        diagnostic: 'no API key for provider route "deepseek-official"',
      }),
      dispose: () => Promise.resolve(),
    })).resolves.toEqual({
      status: 'failed',
      detail: 'no API key for provider route "deepseek-official"',
      output: 'no API key for provider route "deepseek-official"',
    })
  })
})
