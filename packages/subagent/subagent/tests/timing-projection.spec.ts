...
import { subagentIdentityProjectionDefinition, subagentTimingProjectionDefinition } from '../src/projection.ts'
...
describe('subagent timing projection', () => {
  it('registers with the optional session projection registry', async () => {
    ...
    // serializable null sentinel. The map key is `subagent`, not the tool name
    // `dsh_subagent` — listing reads `values.subagent`.
    expect(subagentIdentityProjectionDefinition.key).toBe('subagent')
