 ...
 * aborted is killed, and every other reason is failed. An `error` diagnostic
 * becomes both `detail` and `output` so `dsh_job_output` shows the turn/end
 * message instead of the bare `error` tag; assistant `output` stays off the
 * job so a partial answer is not reported as the task result.
...
function runOutcome(result: SubagentResult): JobOutcome {
  switch (result.stopReason) {
    ...
    case 'error': {
      const detail = result.diagnostic ?? result.stopReason
      return {
        status: 'failed',
        detail,
        ...result.diagnostic !== undefined ? { output: result.diagnostic } : {},
      }
    }
    case 'max-tokens':
    case 'refusal':
