 ...
 * boundary, and invokes the registered `dsh_subagent` tool through
...
describe('dsh-tool-subagent', () => {
  ...
  it('registers a `dsh_subagent` tool that delegates to the configured provider and returns its output', async () => {
  ...
  it('exposes description + prompt + run_in_background to the model (no provider/type parameter)', async () => {
    ...
    const schema = ctx.tools.schemas().find(s => s.name === 'dsh_subagent')
  ...
  it('omits run_in_background entirely when the instance disables it (schema and capability never disagree)', async () => {
    ...
    expect(schema!.description).not.toContain('dsh_job_output')
  })
  ...
  it('uses a child error diagnostic as the foreground headline', async () => {
    const ctx = await setup(
      { provider: 'mock' },
      { stopReason: 'error', diagnostic: 'no API key for provider route "deepseek-official"' },
    )
    const result = await callSubagent(ctx, { description: 'd', prompt: 'p' })
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('no API key for provider route "deepseek-official"')
    expect(text(result)).not.toContain('subagent run failed')
  })
  ...
  it('registers under a configurable toolName so multiple providers can coexist', async () => {
    ...
    await ctx.plugin(tool, { provider: 'spawn', toolName: 'dsh_subagent' })
    await ctx.plugin(tool, { provider: 'acp', toolName: 'subagent_acp' })

    const names = ctx.tools.schemas().map(s => s.name).sort()
    expect(names).toEqual(['dsh_subagent', 'subagent_acp'])

    const viaSpawn = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c-spawn'), name: 'dsh_subagent', arguments: { description: 'd', prompt: 'p' }, agent: fakeAgent() })
  ...
  it('defaults toolName and omits agentOptions when apply() is called directly (schema bypass)', async () => {
    // `ctx.plugin` validates+defaults config first (toolName→'dsh_subagent', the
    // agentOptions object→{}), so the runtime `?? 'dsh_subagent'` fallback and the
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_subagent')).toBe(true)
    await callSubagent(ctx, { description: 'd', prompt: 'p' })
  ...
  it('registers when the provider appears LATER — no load-order requirement (Loader starts siblings concurrently)', async () => {
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_subagent')).toBe(false)
  ...
  it('keeps continuable guidance empty while its provider is absent', async () => {
    ...
    expect(assembly.sections.find(section => section.name === 'tool:dsh_subagent')?.text).toBe('')
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_subagent')).toBe(false)
  })
  ...
  it('mirrors the provider lifecycle: gone on backend dispose, re-derived wording on re-registration', async () => {
    ...
    expect(ctx.tools.schemas().find(s => s.name === 'dsh_subagent')!.description).toContain('does not see this conversation')
    ...
    expect(ctx.tools.schemas().find(s => s.name === 'dsh_subagent')!.description).toContain('inherits this conversation')
  })
  ...
  it('the tool PLUGIN fiber owns its lifecycle listeners: disposal unmounts, and a disposed fiber never zombie-mounts', async () => {
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_subagent')).toBe(true)
    expect((await ctx.systemPrompt.assemble()).sections.some(s => s.name === 'tool:dsh_subagent')).toBe(true)
    await mounted.dispose()
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_subagent')).toBe(false)
    expect((await ctx.systemPrompt.assemble()).sections.some(s => s.name === 'tool:dsh_subagent')).toBe(false)
  ...
  it('ignores lifecycle events for OTHER providers', async () => {
    ...
    expect(ctx.tools.schemas().filter(s => s.name === 'dsh_subagent')).toHaveLength(1)
    expect(ctx.tools.schemas().find(s => s.name === 'dsh_subagent')!.description).toContain('does not see this conversation')
    await other.dispose()
    expect(ctx.tools.schemas().some(s => s.name === 'dsh_subagent')).toBe(true)
  })
  ...
  it('derives inherited-context wording from a seeded-conversation provider', async () => {
    const ctx = await setup({ provider: 'mock', toolName: 'dsh_subagent' }, { inheritsParentContext: true })
    const schema = ctx.tools.schemas().find(s => s.name === 'dsh_subagent')!
  ...
  it('tools depend on the service: no `dsh_subagent` tool without ctx.subagents', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    ...
    const present = ctx.get('tools')?.schemas().some(s => s.name === 'dsh_subagent') ?? false
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    await ctx.plugin(tool, {
      ...
      toolFilter: { deny: ['dsh_subagent'] },
      maxDepth: 2,
    })

    await callSubagent(ctx, { description: 'd', prompt: 'p' })
    expect(seen?.persona).toBe('You are the child.')
    expect(seen?.toolFilter).toMatchObject({ deny: ['dsh_subagent'] })
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    await ctx.plugin(tool, { provider: 'capture3', toolFilter: { deny: ['dsh_subagent'] }, maxDepth: 'provider-managed' })
    await callSubagent(ctx, { description: 'd', prompt: 'p' })
    expect(seen?.toolFilter).toEqual({ deny: ['dsh_subagent'] })
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('returns a job id immediately and the answer is collected through dsh_job_output', async () => {
  ...
  it('copies a child error diagnostic onto dsh_job_output', async () => {
    ...
    const started = await callSubagent(
      ctx,
      { description: 'fork smoke', prompt: 'p', run_in_background: true },
      { agent: parent },
    )
    expect(text(started)).toBe('started background subagent task subagent-1')
    const output = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('diagnostic-output'),
      name: 'dsh_job_output',
      arguments: { job_id: 'subagent-1', wait: true },
      agent: parent,
    })
    expect(text(output)).toBe(
      'no API key for provider route "deepseek-official"\n[status: failed, no API key for provider route "deepseek-official"]',
    )
  })
  ...
  it('forwards dsh_job_kill reasons through the run signal (and defaults one when absent)', async () => {
    ...
    const withReason = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('k1'), name: 'dsh_job_kill', arguments: { job_id: 'subagent-1', reason: 'superseded' }, agent: parent })
    const withoutReason = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('k2'), name: 'dsh_job_kill', arguments: { job_id: 'subagent-2' }, agent: parent })
    ...
    const killed = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('w1'), name: 'dsh_job_output', arguments: { job_id: 'subagent-1', wait: true }, agent: parent })
...
describe('dsh-tool-subagent continuable background mode', () => {
  ...
  it('defaults continuable delegation to background and returns only its durable id', async () => {
    ...
    expect(schema.description).not.toContain('dsh_job_output')
    expect(schema.description).not.toContain('dsh_job_kill')
    expect(schema.description).toContain('dsh_send_message')
    ...
    const guidance = assembly.sections.find(section => section.name === 'tool:dsh_subagent')
    expect(guidance?.text).toContain('Use dsh_subagent in the background by default')
  ...
  it('hides continuable guidance when the current agent cannot see the tool', async () => {
    const { ctx, parent } = await continuableSetup()
    parent.ctx.tools.restrict({ deny: ['dsh_subagent'] })

    expect(ctx.tools.get('dsh_subagent', parent)).toBeUndefined()
    const assembly = await ctx.systemPrompt.assemble(assembleContextFor(parent))
    expect(assembly.sections.find(section => section.name === 'tool:dsh_subagent')?.text).toBe('')
  })
