...
export const Config: z<Config> = z.object({
  ...
  toolName: z.string().default('dsh_subagent'),
...
function stopReasonError(result: SubagentResult): string | undefined {
  switch (result.stopReason) {
    ...
    case 'error':
      return result.diagnostic ?? 'subagent run failed'
    case 'max-tokens':
...
export function apply(ctx: Context, config: Config): void {
  ...
  const toolName = config.toolName ?? 'dsh_subagent'
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      description: wording.description + (backgroundEnabled
        ...
        ? continuable
          ...
          : ' This call waits for the result by default. Set `run_in_background: true` to return a job id; collect with `dsh_job_output` and stop with `dsh_job_kill`.'
        ...
        ...backgroundEnabled ? {
          run_in_background: {
            ...
            description: continuable
              ...
              : 'Whether to run as a background job and return its id. Defaults to false; collect with dsh_job_output or stop with dsh_job_kill.',
          },
        } : {},
      },
      output: {
  ...
  if (present !== undefined) {
    ...
    ctx.logger.info(`subagent provider "${config.provider}" not registered yet; the "${config.toolName ?? 'dsh_subagent'}" tool will register when it appears`)
  }
