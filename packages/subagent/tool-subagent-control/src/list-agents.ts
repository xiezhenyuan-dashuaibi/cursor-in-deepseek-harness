/**
 * The globally named `dsh_list_agents` tool: a thin model-facing adapter over
 ...
 * loadable from the root `dsh_send_message` plugin so a deployment can register
...
function project(
  ...
  // One-shot children cannot be continued by dsh_send_message, so the model
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'terminal, and not a result waiting to be collected; a `dsh_send_message` starts a new turn on the same '
      + 'conversation, and a direct child remains a `dsh_send_message` candidate in every status. The snapshot is not a delivery '
      + 'promise — `dsh_send_message` performs the authoritative check and may still fail. Children that could '
      ...
      + 'session id and depth. You may use `dsh_send_message` only for depth-1 entries; deeper entries are '
      + 'candidates for `dsh_interrupt_agent` only.',
    parameters: {
      scope: {
        type: 'string',
    ...
    async execute(args, exec) {
      ...
      if (!parent) {
        ...
        throw new Error('dsh_list_agents requires a calling agent (exec.agent was undefined)')
      }
      const request = resolveListAgentsRequest(args)
      ...
      switch (request.scope) {
        ...
        default:
          return assertNever(request.scope, 'dsh_list_agents scope')
      }
    },
  }))
}
