...
describe('dsh-tool-subagent-control/list-agents', () => {
  it('registers dsh_list_agents once, globally, with only the optional scope parameter', async () => {
    const { ctx } = await setup([])
    const schemas = ctx.tools.schemas().filter(schema => schema.name === 'dsh_list_agents')
    ...
    expect(schemas[0]!.description).toContain('dsh_interrupt_agent')
  })

  it('renders the empty result as (no subagents)', async () => {
    const { ctx, parent } = await setup([])
    await ctx.sessions.flush(parent.session)
    const result = await callTool(ctx, 'dsh_list_agents', {}, parent)
  ...
  it('resolves omitted scope to children and forwards the tool cancellation signal', async () => {
    ...
    const result = await callTool(ctx, 'dsh_list_agents', {}, parent, signal)
  ...
  it('describes ready as resumable and pins the status vocabulary', async () => {
    const { ctx } = await setup([])
    const schema = ctx.tools.schemas().find(candidate => candidate.name === 'dsh_list_agents')
    ...
    const variants = ctx.tools.get('dsh_list_agents')?.output.schema.items?.oneOf ?? []
  ...
  it('fails loud when invoked without a calling agent', async () => {
    const { ctx } = await setup([])
    const result = await callTool(ctx, 'dsh_list_agents', {})
  ...
  it('unregisters with its plugin fiber (HMR safety)', async () => {
    ...
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_list_agents')).toBe(true)
    await fiber.dispose()
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_list_agents')).toBe(false)
  })
  ...
  it('walks the complete descendant tree in pre-order with parent and depth annotations', async () => {
    ...
    const result = await callTool(ctx, 'dsh_list_agents', { scope: 'descendants' }, parent)
    expect(result.isError).toBe(false)
    expect(text(result)).toBe(
  ...
  it('preserves explicit descendants scope and forwards the tool cancellation signal', async () => {
    ...
    const result = await callTool(ctx, 'dsh_list_agents', { scope: 'descendants' }, parent, signal)
