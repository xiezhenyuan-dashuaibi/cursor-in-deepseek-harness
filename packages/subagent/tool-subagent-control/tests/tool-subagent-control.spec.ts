...
describe('dsh-tool-subagent-control', () => {
  it('registers dsh_send_message once, globally, with the two required parameters', async () => {
    const { ctx } = await setup([])
    const schemas = ctx.tools.schemas().filter(schema => schema.name === 'dsh_send_message')
    ...
    expect(schemas[0]!.description).not.toContain('dsh_job_output')
  ...
  it('cold-resumes a settled child and reports the queued next turn', async () => {
    ...
    const result = await callTool(ctx, 'dsh_send_message', {
  ...
  it('fails loud when invoked without a calling agent', async () => {
    const { ctx } = await setup([])
    const result = await callTool(ctx, 'dsh_send_message', { subagent_id: 'x', message: 'y' })
    expect(result.isError).toBe(true)
  ...
  it('unregisters with its plugin fiber (HMR safety)', async () => {
    ...
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_send_message')).toBe(true)
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_interrupt_agent')).toBe(true)
    await fiber.dispose()
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_send_message')).toBe(false)
    expect(ctx.tools.schemas().some(schema => schema.name === 'dsh_interrupt_agent')).toBe(false)
  })
...
describe('dsh-tool-subagent-control dsh_interrupt_agent', () => {
  it('registers dsh_interrupt_agent with the single agent_id parameter and current-turn wording', async () => {
    const { ctx } = await setup([])
    const schemas = ctx.tools.schemas().filter(schema => schema.name === 'dsh_interrupt_agent')
    expect(schemas).toHaveLength(1)
    const props = (schemas[0]!.parameters as { properties?: Record<string, unknown> }).properties ?? {}
    expect(Object.keys(props)).toEqual(['agent_id'])
    expect(schemas[0]!.description).toContain('current turn')
    expect(schemas[0]!.description).toContain('dsh_send_message')
  })
  ...
  it('interrupts a running direct child with the parent cause, parking its queue', async () => {
    ...
    const queued = await callTool(ctx, 'dsh_send_message', {
      subagent_id: started.childId,
    ...
    const result = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: started.childId }, parent)
    ...
    const waking = await callTool(ctx, 'dsh_send_message', {
      subagent_id: started.childId,
      message: 'wake up',
    }, parent)
  ...
  it('lets a deep live ancestor interrupt a descendant it did not directly create', async () => {
    ...
    const result = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: grandchild.childId }, parent)
  ...
  it('rejects self, sibling, and unrelated callers without touching the target', async () => {
    ...
    const self = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: target.childId }, targetAgent)
    expect(self.isError).toBe(true)
    expect(text(self)).toContain('cannot interrupt itself')
    const fromSibling = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: target.childId }, siblingAgent)
    expect(fromSibling.isError).toBe(true)
    expect(text(fromSibling)).toContain('not a live descendant')
    const fromStranger = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: target.childId }, stranger)
  ...
  it('accepts an absent target as a no-op without cold-resuming it', async () => {
    ...
    const settled = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: started.childId }, parent)
    expect(settled.isError).toBe(false)
    expect(text(settled)).toBe(`interrupt requested for agent ${started.childId}`)
    const unknown = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: 'no-such-agent' }, parent)
  ...
  it('fails loud when invoked without a calling agent', async () => {
    const { ctx } = await setup([])
    const result = await callTool(ctx, 'dsh_interrupt_agent', { agent_id: 'x' })
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('requires a calling agent')
  })
})
