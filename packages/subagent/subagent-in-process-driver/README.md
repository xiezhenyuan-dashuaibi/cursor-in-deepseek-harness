...
- A `tools/result` observer commits a staged value only after that execution's authoritative final tool result succeeds, including the enclosing `dsh_run_code` result for Code Mode sub-dispatch.
