...
describe('in-process structured output', () => {
  ...
  it('discards a nested capture when the enclosing dsh_run_code execution fails', async () => {
    const { ctx, parent, adapter } = await setup([
  ...
  it('discards a nested capture when post-policy blocks the enclosing dsh_run_code result', async () => {
    const { ctx, parent, adapter } = await setup([
      toolCallResponse('c1', RUN_CODE_NAME, { code: 'return await tools.structured_output({ answer: 12 })', description: 'Capture the structured answer' }),
