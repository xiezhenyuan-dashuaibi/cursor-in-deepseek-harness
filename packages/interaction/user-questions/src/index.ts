...
export class UserQuestionService extends Service {
  ...
  async ask(request: AskUserQuestionRequest): Promise<AskUserQuestionAnswer> {
    if (request.signal?.aborted) {
      throw new UserQuestionError('dsh_ask_user_question was aborted before the user answered', 'ASK_ABORTED')
    }
    if (request.questions.length === 0) {
      throw new UserQuestionError('dsh_ask_user_question requires at least one question', 'EMPTY_QUESTIONS')
    }
