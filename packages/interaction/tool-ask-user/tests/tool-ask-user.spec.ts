...
describe('dsh_ask_user_question tool', () => {
  it('registers a model-facing tool schema', async () => {
    const ctx = await setup()
    const schema = ctx.tools.schemas().find(tool => tool.name === 'dsh_ask_user_question')

    expect(schema).toMatchObject({
      name: 'dsh_ask_user_question',
      parameters: {
        type: 'object',
        properties: {
  ...
  it('projects custom answers and multi-select choices', async () => {
    ...
    if (result.isError) throw new Error('expected dsh_ask_user_question success')
  ...
  it('unregisters the tool when its plugin fiber is disposed', async () => {
    ...
    expect(ctx.tools.get('dsh_ask_user_question')).toBeDefined()

    await fiber.dispose()

    expect(ctx.tools.get('dsh_ask_user_question')).toBeUndefined()
  })
})
