...
export type { SkillCatalogEntry } from './catalog-render.ts'
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  // catalog and the `dsh_skill` tool below never see them.
  ...
  ctx.on('agent/pre-step', async (
    ...
    const entries = toSkillCatalogEntries(skills, catalogDescriptionMaxLength)
  ...
  return createUserMessage({
    content: [{
      ...
      text: [
        ...
        '',
        '<available_skills>',
        ...renderSkillCatalogLines(entries),
        '</available_skills>',
        '',
        SKILL_CATALOG_LOAD_GUIDANCE,
...
function renderCatalogUpdate(entries: SkillCatalogSource['entries']): UserMessage {
  const availability = entries.length === 0
    ? [
      SKILL_CATALOG_EMPTY,
      'A user may still invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the `dsh_skill` tool for it.',
    ]
    : [
      'Use only names in this replacement catalog. If the user names a listed skill, or the task clearly matches its description, call the `dsh_skill` tool with the exact name before acting.',
      'A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the `dsh_skill` tool again for that skill.',
    ]
  return createUserMessage({
    content: [{
