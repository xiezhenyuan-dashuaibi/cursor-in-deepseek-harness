...
describe('dsh-tool-skill', () => {
  it('exports catalog entry formatting for MCP and native publishers', () => {
    expect(toolSkill.toSkillCatalogEntries([{ name: 'listed-skill', description: '  Use <x>  ' }])).toEqual([
      { name: 'listed-skill', description: 'Use <x>' },
    ])
    expect(toolSkill.renderSkillCatalogLines([{ name: 'listed-skill', description: 'Use <x>' }])).toEqual([
      '- `listed-skill`: Use &lt;x&gt;',
    ])
    expect(toolSkill.SKILL_CATALOG_EMPTY).toContain('dsh_skill')
  })

  it('registers the dsh_skill tool schema and removes it on dispose', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(AgentRegistry)
    ...
    expect(ctx.tools.schemas().map(tool => tool.name)).toEqual(['dsh_skill'])
    expect(await composePrefix(ctx, '/workspace')).toHaveLength(1)
    expect(ctx.tools.get('dsh_skill')?.presentCall?.({ name: 'project-skill' })).toEqual({
      card: 'generic',
      ...
      },
      {
        ...
        content: [{
          ...
          text: [
            ...
            'A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the `dsh_skill` tool again for that skill.',
            '</system-reminder>',
          ].join('\n'),
        }],
      },
  ...
  it('omits catalog guidance when the calling agent restricts away the shipped skill tool', async () => {
    ...
    scope.ctx.tools.restrict({ deny: ['dsh_skill'] })

    expect(ctx.tools.get('dsh_skill', agent)).toBeUndefined()
    await composePrefixForAgent(ctx, agent)
    expect(catalogMessages(session)).toEqual([])
    await fireStep(ctx, agent, 1, 1)
    expect(catalogMessages(session)).toEqual([])
  ...
  it('does not attach shipped catalog guidance to a scoped same-name tool shadow', async () => {
    ...
    expect(ctx.tools.get('dsh_skill', agent)).not.toBe(ctx.tools.get('dsh_skill'))
    expect(await composePrefixForAgent(ctx, agent)).toEqual([])
    expect(await composePrefix(ctx, '/workspace')).toHaveLength(1)
    await scope.dispose()
  })
  ...
  it('renders provider-managed resource hints for non-local skills', async () => {
    ...
    const opaque = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c2'), name: 'dsh_skill', arguments: { name: 'opaque-skill' } })
    const url = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c3'), name: 'dsh_skill', arguments: { name: 'url-skill' } })
    const provider = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c4'), name: 'dsh_skill', arguments: { name: 'provider-skill' } })
  ...
  it('rejects an unknown resource-base kind at the canonical output boundary', async () => {
    ...
    const result = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c5'), name: 'dsh_skill', arguments: { name: 'rogue-resource-skill' } })
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    const unknown = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c1'), name: 'dsh_skill', arguments: { name: 'missing' } })
    const invalid = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c2'), name: 'dsh_skill', arguments: { name: 'Bad_Name' } })
    const disabled = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c3'), name: 'dsh_skill', arguments: { name: 'hidden-skill' } })
    const modelOnly = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c4'), name: 'dsh_skill', arguments: { name: 'model-only-skill' } })
  ...
  it('checks model policy before provider loading and rechecks the loaded definition', async () => {
    ...
    const denied = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c6'), name: 'dsh_skill', arguments: { name: 'denied-skill' } })
    const raced = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c7'), name: 'dsh_skill', arguments: { name: 'policy-race-skill' } })
    const vanished = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c8'), name: 'dsh_skill', arguments: { name: 'vanishing-skill' } })
