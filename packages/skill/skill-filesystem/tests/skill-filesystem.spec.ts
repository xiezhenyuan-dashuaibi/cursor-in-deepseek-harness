...
describe('FileSystemSkillProvider', () => {
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(path)
    emitObserved(path, {})
    emitObserved(path, { name: 'dsh_read' })
    emitObserved(join(home, 'outside.md'), { name: 'dsh_write' })
    emitObserved(root, { name: 'dsh_write' })
    emitObserved(join(root, 'observed-skill/references/notes.md'), { name: 'dsh_write' })
    emitObserved(join(home, '.dsh/skills/.system/SKILL.md'), { name: 'dsh_write' })
    emitObserved(join(root, 'flat-skill.md'), { name: 'dsh_write' })
    ctx.emit(
      'fs/observed',
      { targetKey: path as never, displayPath: path },
      { kind: 'present', version: FsVersion('observed') },
