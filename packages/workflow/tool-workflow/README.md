...
| Key | Default | Meaning |
|---|---|---|
| `toolName` | `dsh_workflow` | The model-facing tool name to register. |
| `maxResultChars` | `50000` | Rendered-result ceiling; longer success text is truncated with a notice. |
...
When visible, the generated default [`dsh_workflow` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-workflow) carries the complete JavaScript hook and metadata contract; `toolName` can rename the definition, and the model submits script, metadata, and optional args.
...
- **`args` must be an object and Native result text is bounded** — callers wrap top-level arrays/scalars in a field; the canonical workflow result remains complete, while success text beyond `maxResultChars` is truncated in the model-facing projection rather than stored behind a retrieval handle.
