...
describe('dsh-tool-workflow', () => {
  ...
  it('appends failed-child diagnostics after the script return value', async () => {
    const { ctx, engine, parent } = await setup()
    ...
    engine.settleRun(runId, { value: { text: null }, stopReason: 'completed', agentsStarted: 1 })
    const result = await pending
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected workflow success')
    expect(result.value).toEqual({
      ...
      agentsStarted: 1,
      result: { text: null },
      childDiagnostics: ['llm-deepseek: no API key for provider route "deepseek-official"'],
    })
    const rendered = (result.content[0] as { text: string }).text
    expect(rendered).toContain('workflow "audit" completed (1 agent)')
    expect(rendered).toContain('"text": null')
    expect(rendered).toContain('Child failures:')
    expect(rendered).toContain('- llm-deepseek: no API key for provider route "deepseek-official"')
  })
  ...
  it('registers under a configured toolName and unregisters on fiber dispose (HMR safety)', async () => {
    ...
    expect(ctx.tools.get('dsh_workflow')).toBeUndefined()
  ...
  it('presents a generic pending card titled by the meta name, with the script as rawInput', async () => {
    ...
    const tool = ctx.tools.get('dsh_workflow')!
