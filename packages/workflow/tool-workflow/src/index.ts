/**
 * The model-facing `dsh_workflow` tool: run a JavaScript orchestration script that fans out
...
export const Config: z<Config> = z.object({
  toolName: z.string().default('dsh_workflow'),
...
/** Render the run's outcome text: the meta name, agent count, JSON value, and failed-child diagnostics. */
function renderResult(
  name: string,
  agentsStarted: number,
  value: JsonValue,
  maxChars: number,
  childDiagnostics: readonly string[],
): string {
  // The engine returns JSON data (null for a valueless script), so stringify never yields undefined.
  const rendered = JSON.stringify(value, null, 2)
  let text = `workflow "${name}" completed (${agentsStarted} agent${agentsStarted === 1 ? '' : 's'}).\nReturn value:\n${rendered}`
  if (childDiagnostics.length > 0) {
    text += `\nChild failures:\n${childDiagnostics.map(line => `- ${line}`).join('\n')}`
  }
...
/**
 * Collect failed-child diagnostics for one run until `dispose()`.
 */
function tapFailedChildDiagnostics(ctx: Context, runId: WorkflowRunId): FailedChildDiagnosticTap {
  const diagnostics: string[] = []
  const dispose = ctx.on('workflow/agent-end', (info, agent) => {
    if (info.id !== runId || agent.outcome !== 'failed') return
    const text = agent.diagnostic
...
interface FailedChildDiagnosticTap {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
      ...
      schema: {
        ...
        properties: {
          ...
          childDiagnostics: {
            type: 'array',
            items: { type: 'string' },
            description: 'Turn/end failure text from each failed child, in start order. Omitted when every child completed.',
          },
        },
      },
      render: (args, value) => [{
        ...
        text: renderResult(
          ...
          value.result,
          maxResultChars,
          Array.isArray(value.childDiagnostics)
            ? value.childDiagnostics.filter((line): line is string => typeof line === 'string')
            : [],
        ),
      }],
    },
    async execute(args, exec) {
      ...
      const diagnostics = tapFailedChildDiagnostics(ctx, run.id)
      ...
      try {
        ...
        const childDiagnostics = [...diagnostics.snapshot()]
        return {
          runId: run.id,
          agentsStarted: result.agentsStarted,
          result: result.value as JsonValue,
          ...childDiagnostics.length > 0 ? { childDiagnostics } : {},
        }
      } finally {
        diagnostics.dispose()
