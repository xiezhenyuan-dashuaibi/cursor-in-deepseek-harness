...
import type { WorkflowResult, WorkflowRun, WorkflowRunId } from '@deepseek-ai/dsh-workflow'
...
for (let round = 1; round <= args.maxRounds; round += 1) {
  ...
  const prompt = [
    'You are one fresh worker in a foreground Ralph loop. You receive no parent conversation and no prior child session. Do not call the dsh_ralph tool: this round already is its worker.',
    'Immutable objective:\n' + args.objective,
...
function renderRoundFailure(
  ...
  const reason = diagnostic !== undefined && diagnostic.length > 0 ? `\n${diagnostic}` : ''
...
/**
 * Collect failed-child diagnostics for one Ralph run until dispose.
 */
function tapFailedChildDiagnostics(ctx: Context, runId: WorkflowRunId): {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    name: 'tool:ralph',
    order: 116,
    text: 'Use the dsh_ralph tool ONLY when the direct human explicitly asks for a Ralph loop or fresh-agent iterative execution. Each Ralph round starts a fresh child with no conversation seed and uses the shared workspace as durable memory. Completion and blockers are worker reports, not independent evaluation. Use same-session goal tools for ordinary long-running objectives, and plain subagents or workflows for bounded delegation and fan-out.',
  })
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        if (value.status === 'round-failed') {
          throw new Error(renderRoundFailure(value, resolved.maxResultChars, diagnostics.snapshot().at(-1)))
        }
        return {
          runId: run.id,
