...
import type { WorkflowAgentEndInfo, WorkflowResult, WorkflowRun, WorkflowStartRequest } from '@deepseek-ai/dsh-workflow'
...
class StubEngine extends WorkflowEngine {
  ...
  agentEnd(agent: WorkflowAgentEndInfo): void {
    ...
    if (request === undefined) throw new Error('no ralph run')
...
describe('dsh-tool-ralph', () => {
  ...
  it('includes the failed child diagnostic in a round-failed error', async () => {
    ...
    engine.agentEnd({
      ...
      outcome: 'failed',
      diagnostic: 'llm-deepseek: no API key for provider route "deepseek-official"',
    })
    engine.settle({
    ...
    const text = (result.content[0] as { text: string }).text
    expect(text).toContain('Ralph round 1 child failed before producing a structured report.')
    expect(text).toContain('llm-deepseek: no API key for provider route "deepseek-official"')
    expect(text).toContain('No previous handoff was available.')
  })
  ...
  it('registers scoped guidance and pure replay-safe generic presentation', async () => {
    ...
    const tool = ctx.tools.get('dsh_ralph')!
    ...
    await fiber.dispose()
    expect(ctx.tools.get('dsh_ralph')).toBeUndefined()
