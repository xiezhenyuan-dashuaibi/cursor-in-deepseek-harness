...
/** Pair a failed `agent()` with the child's turn/end message when one exists. */
function failedAgentEnd(info: WorkflowAgentInfo, diagnostic: string | undefined): WorkflowAgentEndInfo {
  return {
    ...info,
    outcome: 'failed',
  ...
  private async agent(rawPrompt: unknown, rawOpts: unknown): Promise<unknown> {
    ...
    try {
      ...
      try {
        ...
        this.observer.agentEnd(failedAgentEnd(info, result.diagnostic))
        return null
      } finally {
        await run.dispose()
      }
