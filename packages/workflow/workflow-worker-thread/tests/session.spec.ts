...
describe('runWorkerSession over an in-process MessageChannel', () => {
  ...
  it('forwards a failed child diagnostic onto the agent-end payload', async () => {
    const host = fakeHost({
    ...
    const result = await host.result()
    expect(result.value).toBeNull()
    expect(host.ofType(WorkerToHostType.AgentEnd)[0]!.info).toMatchObject({
      outcome: 'failed',
      diagnostic: 'llm-deepseek: no API key',
    })
    host.close()
  })
