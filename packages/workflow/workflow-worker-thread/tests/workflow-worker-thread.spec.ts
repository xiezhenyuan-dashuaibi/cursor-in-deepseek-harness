...
import type { WorkflowAgentEndInfo, WorkflowMeta, WorkflowResult, WorkflowResultInfo, WorkflowRun, WorkflowRunInfo } from '@deepseek-ai/dsh-workflow'
...
describe('dsh-workflow-worker-thread', () => {
  describe('script execution over a real worker thread', () => {
    ...
    it('forwards a failed child diagnostic onto workflow/agent-end and still returns null', async () => {
      const { ctx, parent } = await setup({
        reply: () => ({ output: [], stopReason: 'error', diagnostic: 'llm-deepseek: no API key' }),
      })
      let end: WorkflowAgentEndInfo | undefined
      ctx.on('workflow/agent-end', (_info, agent) => { end = agent })
      const result = await run(ctx, parent, scripted("return await agent('p')"))
      expect(result.stopReason).toBe('completed')
      expect(result.value).toBeNull()
      expect(end).toMatchObject({ outcome: 'failed', diagnostic: 'llm-deepseek: no API key' })
    })
