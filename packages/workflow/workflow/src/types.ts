...
export interface WorkflowAgentEndInfo extends WorkflowAgentInfo {
  ...
  /**
   * Failure text copied from the child's last `turn/end` `error.message` when
   * `outcome` is `failed` and that message is non-empty. Absent on completed
   * and cancelled ends, and on failures that recorded no turn/end message.
   */
  diagnostic?: string
}
