...
- `workflow/agent-start` / `workflow/agent-end` 按 `seq` 为每次子 agent 调用配对；提供方的异步启动调用被拒绝时，该子 agent 不会发出其中任何一个事件。失败的 end 可携带 `diagnostic`：子 agent 最后一次 `turn/end` 的非空 `error.message`。
...
子 agent 若以非完成的结束原因正常兑现，并不属于基础设施异常：`agent()` 返回 `null`，使脚本可以处理普通的子 agent 失败。监听 `workflow/agent-end` 的父级工具可以把 `diagnostic` 写入面向模型的结果，而不改变该 `null` 契约。
