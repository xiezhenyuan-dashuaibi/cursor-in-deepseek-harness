/**
 * The `dsh_read_image` tool over the REAL local filesystem and attachment store:
...
class CatalogAdapter extends LlmAdapter {
  ...
  override stream(_options: GenerateOptions): AsyncIterable<StreamChunk> {
    throw new Error('dsh_read_image tests never stream')
  }
}
...
function readImage(ctx: Context, args: unknown, agent?: object) {
  return call(ctx, 'dsh_read_image', args, agent)
}
...
describe('dsh_read_image happy path', () => {
  ...
  it('forwards a nested Code Mode image through the outer dsh_run_code context', async () => {
    await writeFile(join(dir, 'red.png'), PNG_1X1)
    const ctx = await setup({ toolMode: 'code' })
    const runtime = ctx.codeRuntime as FakeRuntime
    runtime.behavior = async (request) => {
      const value = await request.bindings[0]!.functions.dsh_read_image!({ file_path: 'red.png' })
      return { logs: [], value }
    }

    const result = await call(ctx, RUN_CODE_NAME, {
      code: 'return await tools.dsh_read_image({ file_path: "red.png" })',
...
describe('argument and service preconditions', () => {
  ...
  it('refuses when no attachment service is mounted', async () => {
    ...
    expect(ctx.tools.get('dsh_read_image')).toBeUndefined()
    expect(ctx.tools.schemas().map(schema => schema.name)).not.toContain('dsh_read_image')
    const result = await readImage(ctx, { file_path: 'red.png' }, agentOn('vision-model'))
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('unknown tool "dsh_read_image"')
  })
...
describe('registration surface', () => {
  it('withdraws dsh_read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    ...
    expect(names()).toEqual(['dsh_edit', 'dsh_read', 'dsh_read_image', 'dsh_write'])

    // Disposing only the attachment store tears down the scoped inject fiber:
    // dsh_read_image withdraws while the unconditional tools stay registered.
    await attachmentsFiber.dispose()
    expect(names()).toEqual(['dsh_edit', 'dsh_read', 'dsh_write'])
    ...
    // Disposing the whole plugin withdraws every tool, dsh_read_image included.
    await toolFsFiber.dispose()
    expect(names()).toEqual([])
    await remounted.dispose()
  })

  it('declares dsh_read_image parallel-safe and presents a read-family card', async () => {
    const ctx = await setup()
    expect(ctx.tools.executionMode({
      signal: testToolSignal, callId: CallId('img-parallel'), name: 'dsh_read_image', arguments: { file_path: 'a.png' },
    })).toEqual({ kind: 'parallel' })
    expect(ctx.tools.get('dsh_read_image')?.presentCall?.({ file_path: 'shot.png' })).toEqual({
      card: 'generic',
...
describe('read keeps its text-only contract', () => {
  it('still refuses a PNG as a binary file and line-numbers text', async () => {
    ...
    const png = await call(ctx, 'dsh_read', { file_path: 'red.png' }, agentOn('vision-model'))
    expect(png.isError).toBe(true)
    expect(text(png)).toContain('binary file')

    const txt = await call(ctx, 'dsh_read', { file_path: 'note.txt' }, agentOn('text-model'))
