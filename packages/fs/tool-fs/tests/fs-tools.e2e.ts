...
const SYSTEM = 'You are a coding assistant. Use the dsh_write tool to create files, the dsh_read tool to inspect '
  + 'them, and the dsh_edit tool for literal replacements. Read a file before editing it. Keep replies terse.'
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('fs tools with-key smoke', () => {
  it('creates, reads, then edits a file — verified on disk', async () => {
    ...
    expect(calls).toContain('dsh_write')
    expect(calls).toContain('dsh_read')
    expect(calls).toContain('dsh_edit')
  }, 180_000)
  ...
  it('resolves a relative path against the per-session cwd (factory meta.cwd)', async () => {
    ...
    try {
      ...
      handle.agent.followup(createUserMessage({
        content: [{ type: 'text', text:
        'Use the dsh_write tool to create a file named where.txt containing exactly the line: here. Tell me when done.' }], source: { kind: 'user' } }))
