...
describe('default deployment (with dsh-fs-observation-policy)', () => {
  ...
  describe('write → disk', () => {
    it('creates a file with exactly the requested bytes', async () => {
      const result = await call('dsh_write', { file_path: 'new.txt', content: 'line one\nline two\n' })
    ...
    it('rejects overwriting an existing file without reading it first', async () => {
      ...
      const result = await call('dsh_write', { file_path: 'a.txt', content: 'clobber' })
    ...
    it('allows overwriting after a read', async () => {
      await writeFile(join(dir, 'a.txt'), 'original')
      expect((await call('dsh_read', { file_path: 'a.txt' })).isError).toBe(false)
      const result = await call('dsh_write', { file_path: 'a.txt', content: 'replaced' })
      expect(result.isError).toBe(false)
    ...
    it('rejects a full overwrite when the file changed since the read (stale)', async () => {
      await writeFile(join(dir, 'a.txt'), 'original')
      await call('dsh_read', { file_path: 'a.txt' })
    ...
    it('the stale remedy is actionable: re-reading the changed file unblocks the retried write', async () => {
      ...
      await call('dsh_read', { file_path: 'a.txt' })
      await writeFile(join(dir, 'a.txt'), 'changed-externally') // out-of-band change
      const stale = await call('dsh_write', { file_path: 'a.txt', content: 'replaced' })
      ...
      expect((await call('dsh_read', { file_path: 'a.txt' })).isError).toBe(false)
      const retried = await call('dsh_write', { file_path: 'a.txt', content: 'replaced' })
  ...
  describe('read', () => {
    it('returns line-numbered content', async () => {
      ...
      const result = await call('dsh_read', { file_path: 'a.txt' })
    ...
    it('reports a binary file as an error', async () => {
      ...
      const result = await call('dsh_read', { file_path: 'bin' })
      expect(result.isError).toBe(true)
    ...
    it('paginates a multi-line file with offset/limit', async () => {
      ...
      const result = await call('dsh_read', { file_path: 'a.txt', offset: 2, limit: 2 })
  ...
  describe('edit → disk', () => {
    it('applies a unique literal replacement after a read', async () => {
      ...
      await call('dsh_read', { file_path: 'a.txt' })
      const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'world', new_string: 'there' })
      expect(result.isError).toBe(false)
    ...
    it('lets a WINDOWED read authorize an edit when the file is unchanged (freshness, not full-view)', async () => {
      ...
      const read = await call('dsh_read', { file_path: 'a.txt', offset: 1, limit: 1 })
      ...
      const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'line 12', new_string: 'LINE 12' })
      expect(result.isError).toBe(false)
    ...
    it('rejects an edit when the file changed since the windowed read (stale before matching)', async () => {
      await writeFile(join(dir, 'a.txt'), 'hello world')
      await call('dsh_read', { file_path: 'a.txt', offset: 1, limit: 1 })
    ...
    it('the stale remedy is actionable: re-reading the changed file unblocks the retried edit', async () => {
      ...
      const stale = await call('dsh_edit', { file_path: 'a.txt', old_string: 'world', new_string: 'there' })
      expect(stale.isError).toBe(true)
      expect(stale.error).toMatchObject({ info: { code: 'FS_STALE_VERSION' } })
      // Follow the remedy: re-read (refreshes the observed version), then retry.
      expect((await call('dsh_read', { file_path: 'a.txt' })).isError).toBe(false)
      const retried = await call('dsh_edit', { file_path: 'a.txt', old_string: 'world', new_string: 'there' })
      expect(retried.isError).toBe(false)
    ...
    it('rejects an ambiguous match without replace_all', async () => {
      ...
      await call('dsh_read', { file_path: 'a.txt' })
      const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'a', new_string: 'b' })
      expect(result.isError).toBe(true)
    ...
    it('replaces all matches with replace_all', async () => {
      await writeFile(join(dir, 'a.txt'), 'a a a')
      await call('dsh_read', { file_path: 'a.txt' })
      const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'a', new_string: 'b', replace_all: true })
      expect(result.isError).toBe(false)
    ...
    it('supports a full write→edit cycle without an intervening read', async () => {
      await call('dsh_write', { file_path: 'a.txt', content: 'one two' })
      const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'two', new_string: 'three' })
      expect(result.isError).toBe(false)
  ...
  describe('deleted observed target', () => {
    it('a failed reread records absence so write can safely recreate the file', async () => {
      ...
      const edit = await call('dsh_edit', { file_path: 'a.txt', old_string: 'original', new_string: 'x' })
      expect(edit.isError).toBe(true)
      expect(edit.error).toMatchObject({ info: { code: 'FS_STALE_VERSION' } })
      const write = await call('dsh_write', { file_path: 'a.txt', content: 'premature' })
      ...
      const reread = await call('dsh_read', { file_path: 'a.txt' })
      ...
      const retriedEdit = await call('dsh_edit', { file_path: 'a.txt', old_string: 'original', new_string: 'x' })
      ...
      const recovered = await call('dsh_write', { file_path: 'a.txt', content: 'fresh' })
  ...
  describe('stat budget', () => {
    it('read stats once; write and edit never stat in the tool (the gate stats zero too)', async () => {
      ...
      statSpy.mockClear()
      const edited = await call('dsh_edit', { file_path: 'a.txt', old_string: 'world', new_string: 'there' })
      expect(edited.isError).toBe(false)
      expect(statSpy).not.toHaveBeenCalled()

      // write (guarded replace, after the edit refreshed observed state): zero stat.
      statSpy.mockClear()
      const written = await call('dsh_write', { file_path: 'a.txt', content: 'fresh' })
    ...
    it('a missing read still stats once and its recovery write stats zero times', async () => {
      const statSpy = vi.spyOn(ctx.fs, 'stat')
      const missing = await call('dsh_read', { file_path: 'missing.txt' })
      expect(missing.isError).toBe(true)
      expect(missing.error).toMatchObject({ info: { code: 'FS_NOT_FOUND' } })
      expect(statSpy).toHaveBeenCalledTimes(1)

      statSpy.mockClear()
      const created = await call('dsh_write', { file_path: 'missing.txt', content: 'fresh' })
...
describe('bare provider (no dsh-fs-observation-policy)', () => {
  ...
  it('write unconditionally creates a new file', async () => {
    const result = await call('dsh_write', { file_path: 'new.txt', content: 'fresh' })
    expect(result.isError).toBe(false)
  ...
  it('write unconditionally OVERWRITES an existing unread file', async () => {
    await writeFile(join(dir, 'a.txt'), 'original')
    const result = await call('dsh_write', { file_path: 'a.txt', content: 'clobbered' })
    expect(result.isError).toBe(false)
  ...
  it('edit of a MISSING target reports FS_STALE_VERSION even on the unguarded path', async () => {
    const result = await call('dsh_edit', { file_path: 'missing.txt', old_string: 'a', new_string: 'b' })
    expect(result.isError).toBe(true)
    expect(result.error).toMatchObject({ info: { code: 'FS_STALE_VERSION' } })
  ...
  it('edit still enforces literal-match codes (FS_EDIT_NOT_FOUND), unrelated to freshness', async () => {
    await writeFile(join(dir, 'a.txt'), 'hello world')
    const result = await call('dsh_edit', { file_path: 'a.txt', old_string: 'absent', new_string: 'x' })
    expect(result.isError).toBe(true)
  ...
  it('neither write nor edit stats in the tool on the bare path', async () => {
    await writeFile(join(dir, 'a.txt'), 'hello world')
    const statSpy = vi.spyOn(ctx.fs, 'stat')
    expect((await call('dsh_write', { file_path: 'a.txt', content: 'x y' })).isError).toBe(false)
    expect((await call('dsh_edit', { file_path: 'a.txt', old_string: 'y', new_string: 'z' })).isError).toBe(false)
    expect(statSpy).not.toHaveBeenCalled()
    statSpy.mockRestore()
  })
})
...
describe('per-session cwd', () => {
  ...
  it('writes a relative path into the SESSION cwd, not config.cwd', async () => {
    const result = await callIn({ header: { cwd: sessionDir } }, 'dsh_write', { file_path: 'note.txt', content: 'hi' })
    expect(result.isError).toBe(false)
  ...
  it('read + edit both resolve against the session cwd (end-to-end)', async () => {
    ...
    expect((await callIn(session, 'dsh_read', { file_path: 'code.txt' })).isError).toBe(false)
    const edited = await callIn(session, 'dsh_edit', { file_path: 'code.txt', old_string: 'alpha', new_string: 'beta' })
    expect(edited.isError).toBe(false)
...
describe('signal, concurrency, and the fs/observed contract', () => {
  ...
  it('a pre-aborted registry call skips read/write/edit with ABORTED_BEFORE_DISPATCH', async () => {
    ...
    const read = await callSig(AbortSignal.abort(), 'dsh_read', { file_path: 'a.txt' })
    expect(read.isError).toBe(true)
    expect(read.error).toMatchObject({ info: { name: 'AbortError', code: TOOL_ABORTED_BEFORE_DISPATCH } })

    const write = await callSig(AbortSignal.abort(), 'dsh_write', { file_path: 'new.txt', content: 'x' })
    expect(write.isError).toBe(true)
    ...
    expect((await callOwned('dsh_read', { file_path: 'a.txt' })).isError).toBe(false)
    const edit = await callSig(AbortSignal.abort(), 'dsh_edit', { file_path: 'a.txt', old_string: 'hello', new_string: 'bye' })
    expect(edit.isError).toBe(true)
  ...
  it('two concurrent edits of the same file, same session: one wins, one FS_STALE_VERSION', async () => {
    ...
    const [one, two] = await Promise.all([
      callOwned('dsh_edit', { file_path: 'a.txt', old_string: 'base', new_string: 'ONE', replaceAll: false }),
      callOwned('dsh_edit', { file_path: 'a.txt', old_string: 'value', new_string: 'TWO', replaceAll: false }),
    ])
  ...
  it('a stale observed version from an older read fails closed at edit CAS', async () => {
    ...
    const edit = await callOwned('dsh_edit', {
      file_path: 'a.txt',
      old_string: 'newer',
      new_string: 'edited',
    })
  ...
  it('a throwing fs/observed listener surfaces as isError, but the mutation already hit disk', async () => {
    ...
    const result = await callOwned('dsh_write', { file_path: 'w.txt', content: 'durable' })
    expect(result.isError).toBe(true)
