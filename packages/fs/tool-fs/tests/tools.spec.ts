...
describe('registration', () => {
  it('registers read, write, and edit', async () => {
    ...
    expect(ctx.tools.schemas().map(s => s.name).sort()).toEqual(['dsh_edit', 'dsh_read', 'dsh_write'])
  })

  it('declares read parallel-safe while write/edit remain exclusive', async () => {
    const { ctx } = await setup()
    expect(ctx.tools.executionMode({ signal: testToolSignal, callId: CallId('read-safe'), name: 'dsh_read', arguments: { file_path: 'a.txt' } }))
      .toEqual({ kind: 'parallel' })
    expect(ctx.tools.executionMode({ signal: testToolSignal, callId: CallId('write-exclusive'), name: 'dsh_write', arguments: { file_path: 'a.txt', content: 'x' } }))
      .toEqual({ kind: 'exclusive' })
    expect(ctx.tools.executionMode({ signal: testToolSignal, callId: CallId('edit-exclusive'), name: 'dsh_edit', arguments: { file_path: 'a.txt', old_string: 'x', new_string: 'y' } }))
      .toEqual({ kind: 'exclusive' })
  })
  ...
  it('registers prompt sections for each tool', async () => {
    ...
    expect(prompt).toContain('Use the dsh_read tool')
    expect(prompt).toContain('Use the dsh_write tool')
    expect(prompt).toContain('Use the dsh_edit tool')
  })
...
describe('read tool', () => {
  it('formats line-numbered content with a footer', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'a.txt' })
...
(End of file - total 2 lines)
  ...
  it('returns an explicit empty canonical line window for an empty file', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'empty.txt' })
  ...
  it('rejects a non-positive offset via arg validation', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_read', { file_path: 'a.txt', offset: 0 })
  ...
  it('rejects a fractional offset and a zero/negative limit', async () => {
    ...
    for (const args of [
      ...
      const result = await call(ctx, 'dsh_read', args)
  ...
  it('rejects a non-JSON numeric offset before tool-specific validation', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_read', { file_path: 'a.txt', offset: Number.NaN })
    expect(result.isError).toBe(true)
  ...
  it('rejects a limit above the cap', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_read', { file_path: 'a.txt', limit: 99999 })
    expect(result.isError).toBe(true)
  ...
  it('rejects a blank file_path', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_read', { file_path: '   ' })
    expect(result.isError).toBe(true)
  ...
  it('records observed state so a follow-up edit by the same session is authorized', async () => {
    ...
    fs.files.set('key:a.txt', 'hello')
    expect((await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })).isError).toBe(false)
    const edited = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'hello', new_string: 'bye' }, { session })
  ...
  it('propagates FS_NOT_FOUND for an absent file', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_read', { file_path: 'missing.txt' })
    expect(result.isError).toBe(true)
  ...
  it('rejects a non-regular target', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'd' })
    expect(result.isError).toBe(true)
  ...
  it('streams a large file (size at/above the cap) instead of reading whole', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'big.txt' })
    expect(result.isError).toBe(false)
  ...
  it('attaches the structured window as presentation meta, and presentResult narrows it into a read card', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'a.ts' })
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected read success')
    ...
    const view = ctx.tools.get('dsh_read')?.presentResult?.({ file_path: 'a.ts' }, result)
    expect(view).toEqual({
      card: 'read',
      path: '/abs/a.ts',
      offset: 1,
  ...
  it('omits the lang hint in meta for an extension that maps to no language', async () => {
    ...
    const result = await call(ctx, 'dsh_read', { file_path: 'notes' })
    if (result.isError) throw new Error('expected read success')
...
describe('write tool', () => {
  it('formats a create result and uses createIfAbsent (unobserved, with the gate)', async () => {
    const { ctx, fs } = await setup()
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'hi' }, { session: { header: {} } })
    expect(result.isError).toBe(false)
  ...
  it('rejects a blank file_path', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_write', { file_path: '   ', content: 'hi' })
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('file_path must be a non-empty string')
  })
  ...
  it('propagates a backend FsError as an isError result carrying its code and remedy', async () => {
    ...
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'hi' })
    expect(result.isError).toBe(true)
...
describe('edit tool', () => {
  it('formats a single-replacement success after a read', async () => {
    ...
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'a', new_string: 'b' }, { session })
  ...
  it('formats the replace_all success message distinctly', async () => {
    ...
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'a', new_string: 'b', replace_all: true }, { session })
  ...
  it('rejects identical old/new strings', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'x', new_string: 'x' })
    expect(result.isError).toBe(true)
  ...
  it('rejects an empty old_string', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: '', new_string: 'x' })
    expect(result.isError).toBe(true)
  ...
  it('rejects a blank file_path', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_edit', { file_path: '  ', old_string: 'a', new_string: 'b' })
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('file_path must be a non-empty string')
  })
  ...
  it('propagates FS_NOT_OBSERVED when the file was never read (the gate decides)', async () => {
    const { ctx, fs } = await setup()
    fs.files.set('key:a.txt', 'hello')
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'a', new_string: 'b' }, { session: { header: {} } })
    expect(result.isError).toBe(true)
...
describe('tool-owned presentation (pure presentCall)', () => {
  ...
  it('read: generic card titled by file with the read window, read kind, location with the offset line', async () => {
    expect(await presentCall('dsh_read', { file_path: 'src/a.ts', offset: 12, limit: 40 })).toEqual({
  ...
  it('read: bare title and line-1 location when offset/limit are unset', async () => {
    expect(await presentCall('dsh_read', { file_path: 'a.txt' })).toEqual({
  ...
  it('read: completed presentation is a read card carrying the structured window with the envelope stripped', async () => {
    ...
    expect(await presentResult('dsh_read', { file_path: 'a.ts' }, {
    ...
    expect(await presentResult('dsh_read', { file_path: 'notes' }, {
  ...
  it('read: completed presentation declines errors and non-single-text content', async () => {
    ...
    expect(await presentResult('dsh_read', { file_path: 'a.txt' }, {
  ...
  it('read: "from line N" window when only offset is set', async () => {
    expect(await presentCall('dsh_read', { file_path: 'a.txt', offset: 5 })).toEqual({
  ...
  it('write: diff card (new-file style, oldText null), location', async () => {
    expect(await presentCall('dsh_write', { file_path: 'out.txt', content: 'hello' })).toEqual({
  ...
  it('read: a limit with no offset windows from line 1', async () => {
    expect(await presentCall('dsh_read', { file_path: 'a.txt', limit: 10 })).toEqual({
  ...
  it('edit: an empty old_string maps to oldText null (a whole-file replace diff)', async () => {
    ...
    expect(await presentCall('dsh_edit', { file_path: 'a.txt', old_string: '', new_string: 'seed' })).toEqual({
...
describe('result-time contextual diff (meta + presentResult)', () => {
  ...
  it('edit: execute attaches the applied hunk as meta { diffs }', async () => {
    ...
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'OLD', new_string: 'NEW' }, { session })
    expect(result.isError).toBe(false)
    expect(result.meta).toEqual({
  ...
  it('edit: presentResult turns the meta into a diff result card', async () => {
    ...
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'OLD', new_string: 'NEW' }, { session })
    const view = ctx.tools.get('dsh_edit')?.presentResult?.({ file_path: 'a.txt', old_string: 'OLD', new_string: 'NEW' }, result)
    expect(view).toEqual({
      card: 'diff', title: 'Edit a.txt',
      diffs: [{ path: 'a.txt', oldText: 'a\nb\nc\nOLD\nd\ne\nf', newText: 'a\nb\nc\nNEW\nd\ne\nf' }],
    })
  ...
  it('write OVERWRITE: execute attaches a contextual hunk; presentResult renders a diff card', async () => {
    ...
    fs.files.set('key:a.txt', withContext)
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'a\nb\nc\nNEW\nd\ne\nf\n' }, { session })
    expect(result.isError).toBe(false)
    expect(result.meta).toEqual({ diffs: [{ path: 'a.txt', oldText: 'a\nb\nc\nOLD\nd\ne\nf', newText: 'a\nb\nc\nNEW\nd\ne\nf' }] })
    const view = ctx.tools.get('dsh_write')?.presentResult?.({ file_path: 'a.txt', content: 'x' }, result)
  ...
  it('write CREATE: an empty applied-diff projection still falls back to the whole-file diff card', async () => {
    ...
    const { ctx } = await setup()
    const session = { header: {} }
    const result = await call(ctx, 'dsh_write', { file_path: 'new.txt', content: 'fresh\n' }, { session })
    expect(result.isError).toBe(false)
    expect(result.meta).toEqual({ diffs: [] })
    const view = ctx.tools.get('dsh_write')?.presentResult?.({ file_path: 'new.txt', content: 'fresh\n' }, result)
  ...
  it('write OVERWRITE with identical content: an empty applied-diff projection falls back to a whole-file diff', async () => {
    ...
    await call(ctx, 'dsh_read', { file_path: 'a.txt' }, { session })
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'same\n' }, { session })
    expect(result.isError).toBe(false)
    expect(result.meta).toEqual({ diffs: [] })
    const view = ctx.tools.get('dsh_write')?.presentResult?.({ file_path: 'a.txt', content: 'same\n' }, result)
  ...
  it('presentResult returns undefined on an error result (nothing applied)', async () => {
    ...
    expect(ctx.tools.get('dsh_edit')?.presentResult?.({ file_path: 'a.txt', old_string: 'x', new_string: 'y' }, errorResult)).toBeUndefined()
    expect(ctx.tools.get('dsh_write')?.presentResult?.({ file_path: 'a.txt', content: 'y' }, errorResult)).toBeUndefined()
  })
  ...
  it('edit presentResult returns undefined on malformed meta (defensive narrowing)', async () => {
    ...
    expect(ctx.tools.get('dsh_edit')?.presentResult?.({ file_path: 'a.txt', old_string: 'x', new_string: 'y' }, badMeta)).toBeUndefined()
  })
  ...
  it('write presentResult falls back to a whole-file diff on malformed meta (never leaks the result text)', async () => {
    ...
    const { ctx } = await setup()
    const badMeta = { content: [{ type: 'text' as const, text: 'ok' }], isError: false, meta: { diffs: 'nope' } }
    const view = ctx.tools.get('dsh_write')?.presentResult?.({ file_path: 'a.txt', content: 'y' }, badMeta)
...
describe('read caps are plugin config', () => {
  ...
  it('a configured readLimit is both the default and the cap, and the schema names it', async () => {
    ...
    const overCap = await call(ctx, 'dsh_read', { file_path: 'a.txt', limit: 3 })
    expect(overCap.isError).toBe(true)
    expect(text(overCap)).toContain('less than or equal to 2')
    const readSchema = ctx.tools.schemas().find(s => s.name === 'dsh_read')
...
describe('sandbox escalation API (write/edit)', () => {
  ...
  function fsSchema(ctx: Context, name: 'dsh_write' | 'dsh_edit') {
  ...
  it('advertises no escalation fields under a non-confining backend', async () => {
    ...
    for (const name of ['dsh_write', 'dsh_edit'] as const) {
  ...
  it('a plain write stamps the default mode with the calling session root', async () => {
    ...
    await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x' }, escalationAgent())
  ...
  it('a standing session override folds onto the stamp', async () => {
    const { ctx, fs } = await setupConfining()
    await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x' }, escalationAgent([{ type: 'sandbox/mode', data: { mode: 'read-only' } }]))
  ...
  it('a denied write maps to the shared marker plus the escalation hint (isError)', async () => {
    ...
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x' }, escalationAgent())
    expect(result.isError).toBe(true)
  ...
  it('a rejected escalation fails closed with its own text and never mutates', async () => {
    ...
    const result = await call(ctx, 'dsh_edit', { file_path: 'a.txt', old_string: 'x', new_string: 'y', sandbox_permissions: 'danger-full-access', justification: 'the test needs it' }, escalationAgent())
    expect(result.isError).toBe(true)
  ...
  it('escalation without an approval service fails closed', async () => {
    const { ctx } = await setupConfining()
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x', sandbox_permissions: 'danger-full-access', justification: 'why' }, escalationAgent())
    expect(result.isError).toBe(true)
  ...
  it('escalation with an approval service but no agent fails closed', async () => {
    ...
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x', sandbox_permissions: 'danger-full-access', justification: 'why' })
    expect(result.isError).toBe(true)
  ...
  it('rejects the escalation argument pairing (one field without the other)', async () => {
    const { ctx } = await setupConfining()
    const missing = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x', sandbox_permissions: 'workspace-write' }, escalationAgent())
  ...
  it('sandbox_permissions under a non-confining backend fails closed (unadvertised field still reaches execute)', async () => {
    const { ctx } = await setup()
    const result = await call(ctx, 'dsh_write', { file_path: 'a.txt', content: 'x', sandbox_permissions: 'workspace-write', justification: 'why' }, escalationAgent())
    expect(result.isError).toBe(true)
