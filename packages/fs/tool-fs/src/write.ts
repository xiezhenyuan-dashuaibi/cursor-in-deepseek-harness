...
/**
 * The `dsh_write` tool's validated arguments: the base parameters plus the
...
/**
 * Register the `dsh_write` tool and its system-prompt guidance.
...
export function applyWriteTool(ctx: Context, sandbox: FsSandboxController): void {
  ctx.systemPrompt.section({
    name: 'tool:write',
    order: 101,
    text: 'Use the dsh_write tool to create files or completely replace file contents. Existing files are overwritten, so read an existing file first (the default fs-observation-policy requires it) and prefer edit for targeted changes.',
  })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: WriteToolArgs, exec) {
      ...
      const sandboxPolicy = await sandbox.resolvePolicy(exec.name, args, exec)
