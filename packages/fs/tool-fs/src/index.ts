/**
 * Model-facing `dsh_read`, `dsh_read_image`, `dsh_write`, and `dsh_edit` tools over `ctx.fs`. This package owns schemas, validation,
...
/** Register the full `dsh_read`/`dsh_write`/`dsh_edit` filesystem tool suite, plus `dsh_read_image` while `attachments` is mounted. */
export function apply(ctx: Context, config: Config): void {
  ...
  // dsh_read_image is composition-conditional: without a mounted attachment store
