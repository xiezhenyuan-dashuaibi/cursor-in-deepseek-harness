/**
 * The model-facing `dsh_read_image` tool: reads a PNG/JPEG/WebP/GIF file, durably
...
/** Extensions `dsh_read_image` accepts; magic-byte validation at the attachment service stays authoritative. */
...
/** The canonical outcome declared by the `dsh_read_image` output schema. */
...
/**
 * Register the `dsh_read_image` tool into the given context. The composing plugin
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (mediaType === undefined) {
        throw new Error(`cannot read "${args.file_path}": dsh_read_image only accepts PNG/JPEG/WebP/GIF paths`)
      }
