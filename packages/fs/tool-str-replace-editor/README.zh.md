...
基于 `ctx.fs`、面向模型的独立 `dsh_str_replace_editor`。它可与持久 Bash、一次性 Bash、沙箱 Bash 或其他终端接口组合。
...
生成的 [`dsh_str_replace_editor` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-str-replace-editor)，其中包含配置的 `description`。本插件不贡献独立系统提示词段。

#### Token 影响

`dsh_str_replace_editor` 可见时产生固定的 schema 成本。
