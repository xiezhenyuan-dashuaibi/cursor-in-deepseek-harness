...
describe('tool-str-replace-editor', () => {
  it('registers the standalone schema and configurable description', async () => {
    ...
    expect(ctx.tools.schemas().map(item => item.name)).toEqual(['dsh_str_replace_editor'])
    ...
    expect(ctx.tools.get('dsh_str_replace_editor')?.presentCall?.({
      command: 'view',
    ...
    expect(ctx.tools.get('dsh_str_replace_editor')).toBeUndefined()
  })
