/**
 * The model-facing `dsh_grep` tool: search file contents with a ripgrep regular
...
/**
 * Register the `dsh_grep` tool and its system-prompt guidance.
 *
...
export function applyGrepTool(ctx: Context, caps: GrepToolCaps): void {
  ctx.systemPrompt.section({
    name: 'tool:grep',
    order: 104,
    text: 'Use the dsh_grep tool — not shell grep or rg — to search file contents. Use read on a matched file when you need surrounding context.',
  })
