/**
 * The model-facing `dsh_glob` tool: discover files whose paths match a glob
...
/**
 * Register the `dsh_glob` tool and its system-prompt guidance.
 *
...
export function applyGlobTool(ctx: Context, caps: GlobToolCaps): void {
  ...
  ctx.systemPrompt.section({
    name: 'tool:glob',
    order: 103,
    text: 'Use the dsh_glob tool — not shell find — to discover files by path pattern. A pattern with no "/" matches basenames at any depth, so "*" matches every file in the tree rather than its top level. '
