...
| `@deepseek-ai/dsh-tool-lsp` | Consumer: the model-facing `dsh_lsp` tool over `ctx.lsp` |
...
Indirectly, through `dsh-tool-lsp`, which owns the model-facing `dsh_lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.
