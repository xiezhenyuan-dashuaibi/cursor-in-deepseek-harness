...
The language-server capability seam: an LSP Service Definition, a generic stdio provider, and the model-facing `dsh_lsp` tool. All **product** packages.
...
| `tool-lsp/` | Model-facing `dsh_lsp` tool (four operations, one-based UTF-16 cursor coordinates) | (registers on `ctx.tools`) |
