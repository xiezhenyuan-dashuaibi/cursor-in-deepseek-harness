/**
 * Model-facing `dsh_lsp` tool over `ctx.lsp`. One read-only tool with four operations
...
/**
 * Register the `dsh_lsp` tool and its system-prompt guidance.
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (workspaceRoot === undefined) {
        throw new LspError('the dsh_lsp tool requires a session workspace cwd', 'LSP_WORKSPACE_REQUIRED')
      }
