...
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active. Adding or removing a client plugin through this profile\'s or the home cordis.patch.yml updates the open page without a refresh. '
    + 'Edits to an already-rostered plugin reload without a refresh only while `pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic source updates. '
