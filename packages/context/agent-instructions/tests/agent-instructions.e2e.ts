...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('workspace context e2e: real model sees AGENTS.md baseline', () => {
  ...
  it('loads a nested AGENTS.md after the real read tool touches a descendant file', async () => {
    ...
    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'Use the dsh_read tool to inspect pkg/deep/file.txt. After reading it, answer: nested instruction handshake?' }], source: { kind: 'user' } }))
    await waitForIdle(live.ctx, live.agent)
  ...
  it('appends changed baseline instructions after a real file-tool touch without rewriting the frozen prefix', async () => {
    ...
    live.agent.followup(createUserMessage({ content: [{ type: 'text', text: 'You must use the dsh_read tool to inspect trigger.txt. After reading it, answer: updated workspace context handshake?' }], source: { kind: 'user' } }))
    await waitForIdle(live.ctx, live.agent)
