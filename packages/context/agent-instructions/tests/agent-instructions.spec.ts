...
describe('workspace context request injection', () => {
  ...
  it('queues a replacement when a frozen baseline file changes before a later fs tool call', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-baseline-change'), name: 'dsh_read', arguments: { file_path: 'file.txt' }, agent,
      })
  ...
  it('queues a removal when a frozen baseline file is deleted before a later fs tool call', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-baseline-remove'), name: 'dsh_read', arguments: { file_path: 'file.txt' }, agent,
      })
...
describe('dynamic nested workspace context injection', () => {
    ...
    try {
      ...
      const adapter = new MockAdapter([
        [
          ...
          { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('read-before-abort'), name: 'dsh_read', arguments: '{"file_path":"pkg/deep/file.txt"}' } },
          { type: 'block-start', index: 1, blockType: 'tool-call' },
          { type: 'block-end', index: 1, block: { type: 'tool-call', id: CallId('abort-after-read'), name: 'abort_step', arguments: '{}' } },
          { type: 'finish', reason: { kind: 'tool-calls' } },
        ] satisfies StreamChunk[],
        toolCallResponse('read-after-abort', 'dsh_read', { file_path: join('pkg', 'deep', 'file.txt') }),
        textResponse('done'),
      ])
  ...
  it('skips instruction content reads while provider version and effective state are unchanged', async () => {
    ...
    try {
      ...
      const first = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-version-fast-path'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      const second = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-with-version-fast-path'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('re-reads a changed provider version, then refreshes metadata when SHA-1 is unchanged', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-same-digest-version-change'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      ...
      const afterVersionChange = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-same-digest-version-change'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
      const afterRefresh = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-version-cache-refresh'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('isolates instruction version caches between sessions that touch the same scope', async () => {
    ...
    try {
      ...
      const first = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-from-first-session'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent: firstAgent,
      })
      const second = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-from-second-session'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent: secondAgent,
      })
  ...
  it('replaces previously loaded instructions when the same file content changes', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-change'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      await write(join(root, 'pkg/AGENTS.md'), 'new package rule with more detail')
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-change'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('reconciles distinct sibling candidates as independent scopes', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-both-siblings'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-one-sibling-removed'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('drops a newly discovered sibling whose content duplicates an earlier candidate in the scope', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-nested-dup-siblings'), name: 'dsh_read', arguments: { file_path: join('pkg', 'deep', 'file.txt') }, agent,
      })
  ...
  it('removes a previously rendered sibling once its content becomes a duplicate of an earlier candidate', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-dup-convergence'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      const firstText = blocksText(((await syncedWorkspaceContext(ctx, agent))).content)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-dup-convergence'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('removes an unchanged sibling when an earlier candidate changes to match its content', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-earlier-converges'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-earlier-converges'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('removes previously loaded instructions when no candidate remains in the scope', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-remove'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      await rm(join(root, 'pkg/AGENTS.md'))
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-remove'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('removes a previously loaded instruction file once it resolves to a directory through a symlink', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-symlink-dir'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      const firstText = blocksText(((await syncedWorkspaceContext(ctx, agent))).content)
      await appendAdditionalContexts(ctx, agent)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-symlink-dir'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('loads a candidate again after a logged removal tombstone', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-tombstone'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      await rm(join(root, 'pkg/AGENTS.md'))
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-to-create-tombstone'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-after-tombstone'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('does not report removal when a previously loaded scope is temporarily unavailable', async () => {
    ...
    try {
      ...
      const first = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-provider-failure'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await appendAdditionalContexts(ctx, agent)
      fs.throwOnStat.add(join(root, 'pkg/AGENTS.md'))
      const duringFailure = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-during-provider-failure'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
  ...
  it('appends an update during resumed prefix composition when visible nested instructions changed offline', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-before-offline-change'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent: original,
      })
  ...
  it('ignores failed, aborted, agentless, and non-file final results', async () => {
    ...
    try {
      ...
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal, callId: CallId('agentless'), name: 'dsh_read', arguments: { file_path: 'file.txt' },
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal, callId: CallId('failed'), name: 'dsh_read', arguments: { file_path: 'failed/file.txt' }, agent,
      }), { content: [], isError: true, error: { message: 'failed' } })
      ctx.emit('tools/result', stubToolExecution({
        signal: aborted.signal, callId: CallId('aborted'), name: 'dsh_read', arguments: { file_path: 'aborted/file.txt' }, agent,
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('null-arguments'), name: 'dsh_read', arguments: null, agent,
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('missing-path'), name: 'dsh_read', arguments: {}, agent,
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('non-string-path'), name: 'dsh_read', arguments: { file_path: 1 }, agent,
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('blank-path'), name: 'dsh_read', arguments: { file_path: ' ' }, agent,
      }), plainResult)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      const first = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-tiny-budget-1'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
      const second = await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('read-tiny-budget-2'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('tiny-budget-touch'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      }), acceptedResult)
  ...
  it('settles same-scope replacement and deletion against files instead of pending prose', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('pending-v1'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('pending-v2'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
      expect(agent.inbox.nextStep).toHaveLength(1)
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('pending-delete'), name: 'dsh_read', arguments: { file_path: join('pkg', 'file.txt') }, agent,
      })
      await syncWorkspaceContext(ctx, agent)
      expect(agent.inbox.nextStep).toEqual([])
    } finally {
  ...
  it('keeps a completed tool projection when a later pre-step aborts', async () => {
    ...
    try {
      ...
      const first = stubToolExecution({
        signal: testToolSignal,
        callId: CallId('projected-before-abort'), name: 'dsh_read', arguments: { file_path: join('a', 'file.txt') }, agent,
      })
      ...
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
        callId: CallId('projected-after-abort'), name: 'dsh_read', arguments: { file_path: join('b', 'file.txt') }, agent,
      }), acceptedResult)
      await syncWorkspaceContext(ctx, agent)
  ...
  it('serializes concurrent final results and merges both touched scopes into one pending context', async () => {
    ...
    try {
      ...
      const first = stubToolExecution({
        signal: testToolSignal,
        callId: CallId('concurrent-a'), name: 'dsh_read', arguments: { file_path: join('a', 'file.txt') }, agent,
      })
      const second = stubToolExecution({
        signal: testToolSignal,
        callId: CallId('concurrent-b'), name: 'dsh_read', arguments: { file_path: join('b', 'file.txt') }, agent,
      })
  ...
  it('merges a recovered pending context with a fresh touched scope', async () => {
    ...
    try {
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('recover-pending-a'), name: 'dsh_read', arguments: { file_path: join('a', 'file.txt') }, agent: original,
      })
      ...
      await ctx.tools.execute({
        signal: testToolSignal,
        callId: CallId('recover-pending-b'), name: 'dsh_read', arguments: { file_path: join('b', 'file.txt') }, agent: resumed,
      })
