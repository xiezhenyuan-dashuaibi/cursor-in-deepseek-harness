...
const DELETE_DESCRIPTION =
  'Delete one active reminder in the current session by the exact id returned by dsh_schedule_create '
  + 'or dsh_schedule_list. Unknown or already-finished ids return deleted false.'
    ...
    disposers.push(toolCtx.tools.register(defineTool({
      ...
      async execute(args, exec): Promise<ScheduleDeleteValue> {
        if (args.id.length === 0 || args.id.trim() !== args.id) {
          return { code: 'invalid_rule', message: 'dsh_schedule_delete id must be non-empty without surrounding whitespace.' }
        }
