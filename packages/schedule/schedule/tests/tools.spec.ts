...
describe('Schedule tool protocol', () => {
  it('registers three exclusive generic tools and disposes them together', async () => {
    ...
    expect(['dsh_schedule_create', 'dsh_schedule_list', 'dsh_schedule_delete'].map(name => test.ctx.tools.get(name)?.name))
      .toEqual(['dsh_schedule_create', 'dsh_schedule_list', 'dsh_schedule_delete'])
    const outputSchema = test.ctx.tools.get('dsh_schedule_create')?.output.schema as {
    ...
    for (const name of ['dsh_schedule_create', 'dsh_schedule_list', 'dsh_schedule_delete']) {
      expect(test.ctx.tools.executionMode({ signal, callId: CallId(name), name, arguments: {}, agent: test.agent }))
        .toEqual({ kind: 'exclusive' })
    }
    expect(test.ctx.tools.get('dsh_schedule_create')?.presentCall?.({ prompt: 'x', after_seconds: 1 }))
      .toEqual({ card: 'generic', title: 'Create reminder', kind: 'other', rawInput: 'x' })
    expect(test.ctx.tools.get('dsh_schedule_list')?.presentCall?.({}))
      .toEqual({ card: 'generic', title: 'List reminders', kind: 'read' })
    expect(test.ctx.tools.get('dsh_schedule_delete')?.presentCall?.({ id: 'schedule-1' }))
      .toEqual({ card: 'generic', title: 'Delete reminder', kind: 'other', rawInput: 'schedule-1' })
    test.disposeTools()
    test.disposeTools()
    expect(test.ctx.tools.get('dsh_schedule_create')).toBeUndefined()
    expect(test.ctx.tools.get('dsh_schedule_list')).toBeUndefined()
    expect(test.ctx.tools.get('dsh_schedule_delete')).toBeUndefined()
  })

  it('rolls back earlier tool registrations when a later name conflicts', async () => {
    const test = await harness()
    const list = test.ctx.tools.get('dsh_schedule_list')
    ...
    expect(test.ctx.tools.get('dsh_schedule_create')).toBeUndefined()
    expect(test.ctx.tools.get('dsh_schedule_list')).toBe(list)
    expect(test.ctx.tools.get('dsh_schedule_delete')).toBeUndefined()
    disposeConflict()
  })
  ...
  it('rejects shape-known invalid create input before persistence', async () => {
    const test = await harness()
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: ' ', after_seconds: 1 })))
      .toEqual({ code: 'invalid_prompt', message: 'prompt must be non-empty after trimming.' })
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 0 })))
      .toEqual({ code: 'invalid_rule', message: 'after_seconds must be a positive safe integer.' })
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1.5 })))
      .toEqual({ code: 'invalid_rule', message: 'after_seconds must be a positive safe integer.' })
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1, at: 'later' })))
      .toEqual({
        code: 'invalid_selector',
        message: 'dsh_schedule_create accepts exactly one of after_seconds, at, or every_seconds.',
      })
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', every_seconds: 1.5 })))
      .toEqual({ code: 'invalid_rule', message: 'every_seconds must be a safe integer.' })
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', every_seconds: 299 })))
  ...
  it('creates, lists, marks overdue, deletes, and never reuses an id', async () => {
    const test = await harness()
    expect(value(await execute(test, 'dsh_schedule_create', {
    ...
    expect(value(await execute(test, 'dsh_schedule_list', {}))).toEqual([
      expect.objectContaining({ id: 'schedule-1', state: 'overdue' }),
    ])
    expect(test.flushes.count).toBe(3)
    expect(test.changes.count).toBe(3)

    expect(value(await execute(test, 'dsh_schedule_delete', { id: 'schedule-1' })))
    ...
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'next', after_seconds: 1 })))
  ...
  it('rejects an empty or padded delete id before persistence', async () => {
    ...
    for (const id of ['', ' schedule-1']) {
      expect(value(await execute(test, 'dsh_schedule_delete', { id }))).toEqual({
        code: 'invalid_rule',
        message: 'dsh_schedule_delete id must be non-empty without surrounding whitespace.',
      })
    }
    expect(test.flushes.count).toBe(0)
  })
  ...
  it('returns a range error only after the create preflight', async () => {
    ...
    expect(value(await execute(internal, 'dsh_schedule_create', { prompt: 'clock', after_seconds: 1 })))
  ...
  it('contains a projection observer failure after the create barrier', async () => {
    ...
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'still committed', after_seconds: 1 })))
  ...
  it('treats missing persistence as uncertainty rather than a successful no-op', async () => {
    ...
    expect(value(await execute(test, 'dsh_schedule_list', {}))).toEqual({
      ...
      message: 'Schedule persistence is uncertain; retry with dsh_schedule_list before relying on this result.',
      operation: 'list',
    })
  })
})
...
describe('Schedule persistence failure boundaries', () => {
  it('does not fold an unconfirmed corrupt live suffix before preflight succeeds', async () => {
    ...
    expect(value(await execute(test, 'dsh_schedule_list', {}))).toMatchObject({
  ...
  it('reports a create barrier rejection with the known appended id and recovers on list preflight', async () => {
    ...
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'persist me', after_seconds: 10 })))
      .toEqual({
        code: 'persistence_uncertain',
        message: 'Schedule persistence is uncertain; retry with dsh_schedule_list before relying on this result.',
        operation: 'create',
  ...
  it('serializes concurrent management transactions across both persistence barriers', async () => {
    ...
    const creating = execute(test, 'dsh_schedule_create', { prompt: 'persist me', after_seconds: 10 })
    await vi.waitFor(() => { expect(test.flushes.count).toBe(1) })
    const listing = execute(test, 'dsh_schedule_list', {})
  ...
  it('does not persist a create cancelled while it waits in the Schedule FIFO', async () => {
    ...
    const creating = execute(test, 'dsh_schedule_create', {
  ...
  it('does not persist a delete cancelled during its first preflight', async () => {
    const test = await harness()
    await execute(test, 'dsh_schedule_create', { prompt: 'keep me', after_seconds: 60 })
    ...
    const controller = new AbortController()
    const deleting = execute(test, 'dsh_schedule_delete', { id: 'schedule-1' }, test.agent, controller.signal)
    ...
    expect(test.agent.session.events.filter(event => event.type === 'schedule/change'))
      .toHaveLength(1)
    expect(value(await execute(test, 'dsh_schedule_list', {})))
  ...
  it('returns uncertainty before create or delete reads when their preflight rejects', async () => {
    ...
    expect(value(await execute(createTest, 'dsh_schedule_create', { prompt: 'later', after_seconds: 1 })))
    ...
    await execute(deleteTest, 'dsh_schedule_create', { prompt: 'keep', after_seconds: 1 })
    deleteTest.flushes.outcomes.push('reject')
    expect(value(await execute(deleteTest, 'dsh_schedule_delete', { id: 'schedule-1' })))
  ...
  it('maps corrupt and unreadable folds for create, list, and delete', async () => {
    ...
    expect(value(await execute(corrupt, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1 })))
      .toMatchObject({ code: 'corrupt_schedule_log' })
    expect(value(await execute(corrupt, 'dsh_schedule_delete', { id: 'schedule-1' })))
      .toMatchObject({ code: 'corrupt_schedule_log' })
    ...
    expect(value(await execute(unreadable, 'dsh_schedule_list', {})))
      .toEqual({ code: 'internal_error', message: 'The schedule operation failed.' })
  })

  it('reports a delete barrier rejection and lets the next preflight clarify the terminal record', async () => {
    const test = await harness()
    await execute(test, 'dsh_schedule_create', { prompt: 'delete me', after_seconds: 10 })
    test.flushes.outcomes.push('resolve', 'reject', 'resolve')
    expect(value(await execute(test, 'dsh_schedule_delete', { id: 'schedule-1' }))).toMatchObject({
  ...
  it('contains append failures and refuses cross-owner execution', async () => {
    ...
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1 })))
      .toEqual({ code: 'internal_error', message: 'The schedule operation failed.' })
    stop()

    const other = stubAgent(test.ctx, `other-${Math.random()}`)
    test.ctx.agents.register(other)
    expect(value(await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1 }, other)))
      .toEqual({ code: 'internal_error', message: 'The schedule operation failed.' })
    expect(value(await execute(test, 'dsh_schedule_list', {}, other)))
      .toEqual({ code: 'internal_error', message: 'The schedule operation failed.' })
    expect(value(await execute(test, 'dsh_schedule_delete', { id: 'schedule-1' }, other)))
      .toEqual({ code: 'internal_error', message: 'The schedule operation failed.' })
  })

  it('contains a delete append failure after a successful preflight', async () => {
    const test = await harness()
    await execute(test, 'dsh_schedule_create', { prompt: 'x', after_seconds: 1 })
    const stop = test.ctx.on('internal/dispatch', (_mode, eventName, args) => {
