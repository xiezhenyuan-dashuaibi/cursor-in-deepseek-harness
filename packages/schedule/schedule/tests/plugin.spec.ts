...
describe('Schedule plugin composition', () => {
  ...
  it('installs only on future root agents and unwinds on plugin disposal', async () => {
    ...
    expect(ctx.tools.get('dsh_schedule_create', existing.agent)).toBeUndefined()
    expect(ctx.tools.get('dsh_schedule_create')).toBeUndefined()

    const root = await ctx.agents.create({ sessionId: SessionId('schedule-root') })
    expect(ctx.tools.get('dsh_schedule_create', root.agent)?.name).toBe('dsh_schedule_create')
    expect(ctx.tools.get('dsh_schedule_list', root.agent)?.name).toBe('dsh_schedule_list')
    expect(ctx.tools.get('dsh_schedule_delete', root.agent)?.name).toBe('dsh_schedule_delete')
    expect(ctx.tools.get('dsh_schedule_create')).toBeUndefined()

    const created = await ctx.agents.withInitiator(root.agent, () => ctx.tools.execute({
      signal: new AbortController().signal,
      callId: CallId('schedule-plugin-create'),
      name: 'dsh_schedule_create',
    ...
    expect(ctx.tools.get('dsh_schedule_create', child.agent)).toBeUndefined()

    const departing = await ctx.agents.create({ sessionId: SessionId('schedule-departing') })
    expect(ctx.tools.get('dsh_schedule_create', departing.agent)).toBeDefined()
    await departing.dispose()
    expect(ctx.tools.get('dsh_schedule_create', departing.agent)).toBeUndefined()

    await plugin.dispose()
    expect(ctx.tools.get('dsh_schedule_create', root.agent)).toBeUndefined()
    expect(ctx.tools.get('dsh_schedule_list', root.agent)).toBeUndefined()
    expect(ctx.tools.get('dsh_schedule_delete', root.agent)).toBeUndefined()
