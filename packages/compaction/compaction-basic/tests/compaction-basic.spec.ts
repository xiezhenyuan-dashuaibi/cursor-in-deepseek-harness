...
function toolConversation(): Session {
  ...
  for (let turn = 1; turn <= 3; turn += 1) {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        ...
        content: [
          ...
          { type: 'tool-call', id: callId, name: 'dsh_read', arguments: '{}' },
        ],
        source: {
          kind: 'model',
          ...{ provider: MODEL, model: MODEL },
    ...
    session.append('tool/call', { turn, step: 1, callId, name: 'dsh_read', arguments: '{}' })
...
function oversizedToolResult(chars = 3_000, withCompactablePrompt = false): Session {
  ...
  session.append('assistant/message', {
    ...
    message: createMessage({
      role: 'assistant',
      content: [{ type: 'tool-call', id: callId, name: 'dsh_bash', arguments: '{}' }],
      source: {
        kind: 'model',
        ...{ provider: MODEL, model: MODEL },
      },
  ...
  session.append('tool/call', { turn: 1, step: 1, callId, name: 'dsh_bash', arguments: '{}' })
  session.append('tool/result', {
    turn: 1,
    step: 1,
    message: createToolResultMessage({
...
describe('pressure measurement and retention', () => {
  ...
  it('declines forced overflow when the whole surface is one indivisible tool pair', async () => {
    ...
    session.append('assistant/message', {
      ...
      message: createMessage({
        role: 'assistant',
        content: [{ type: 'tool-call', id: callId, name: 'dsh_read', arguments: '{}' }],
        source: {
          kind: 'model',
          ...{ provider: MODEL, model: MODEL },
        },
    ...
    session.append('tool/call', { turn: 1, step: 1, callId, name: 'dsh_read', arguments: '{}' })
    session.append('tool/result', {
      turn: 1,
      step: 1,
      message: createToolResultMessage({
