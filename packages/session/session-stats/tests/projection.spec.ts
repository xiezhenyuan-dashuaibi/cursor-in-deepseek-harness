...
describe('sessionStats wall-time fold (controlled timestamps)', () => {
  ...
  it('pairs tool wall time by callId, ignores orphan results, and prunes leftovers at turn/end', () => {
    ...
    const paired = fold([
      at(1_000, 'step/start', { turn: 1, step: 1 }),
      at(1_100, 'tool/call', { turn: 1, step: 1, callId: 'a', name: 'dsh_read', arguments: '{}' }),
      at(1_200, 'tool/call', { turn: 1, step: 1, callId: 'b', name: 'dsh_read', arguments: '{}' }),
    ...
    const pruned = fold([
      at(1_000, 'step/start', { turn: 1, step: 1 }),
      at(1_100, 'tool/call', { turn: 1, step: 1, callId: 'orphan', name: 'dsh_read', arguments: '{}' }),
      at(2_000, 'step/end', { turn: 1, step: 1 }),
  ...
  it('pairs only own pendingCalls keys: a prototype-name callId without a recorded call stays unmatched', () => {
    ...
    expect(fold([
      at(1_000, 'step/start', { turn: 1, step: 1 }),
      at(1_100, 'tool/call', { turn: 1, step: 1, callId: 'constructor', name: 'dsh_read', arguments: '{}' }),
