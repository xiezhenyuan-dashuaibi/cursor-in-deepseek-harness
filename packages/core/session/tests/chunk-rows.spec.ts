...
describe('packChunkRuns', () => {
  ...
  it('packs reasoning and tool-call runs under their own tags', () => {
    ...
    const toolCall = [4, 5, 6].map(seq =>
      chunkEvent(seq, 1000 + seq, { type: 'tool-call-delta', index: 1, id: CallId('c1'), name: 'dsh_write', argumentsDelta: `a${seq}` }))
    ...
    expect(row.data).toMatchObject({ id: 'c1', name: 'dsh_write', args: ['a4', 'a5', 'a6'] })
