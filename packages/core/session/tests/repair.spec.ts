...
describe('interruptedTurnClosers', () => {
      ...
      { type: 'assistant/message', seq: 2, time: 2, data: {
        ...
        message: createMessage({
          ...
          content: [
            ...
            { type: 'tool-call', id: CallId('call-1'), name: 'dsh_bash', arguments: '{}' },
          ],
          source: {
            kind: 'model',
    ...
    const events: SessionEvent[] = [
      ...
      { type: 'assistant/message', seq: 2, time: 2, data: {
        ...
        message: createMessage({
          role: 'assistant',
          content: [
            { type: 'tool-call', id: CallId('old-call'), name: 'dsh_bash', arguments: '{}' },
          ],
          source: {
            kind: 'model',
            ...{ provider: 'mock', model: 'mock' },
      ...
      { type: 'assistant/message', seq: 8, time: 8, data: {
        ...
        message: createMessage({
          role: 'assistant',
          content: [
            { type: 'tool-call', id: CallId('new-call'), name: 'dsh_bash', arguments: '{}' },
          ],
          source: {
            kind: 'model',
            ...{ provider: 'mock', model: 'mock' },
    ...
    const events: SessionEvent[] = [
      ...
      { type: 'assistant/message', seq: 2, time: 2, data: {
        ...
        message: createMessage({
          role: 'assistant',
          content: [
            { type: 'tool-call', id: CallId('call-a'), name: 'dsh_bash', arguments: '{}' },
            { type: 'tool-call', id: CallId('call-b'), name: 'dsh_bash', arguments: '{}' },
          ],
          source: {
            kind: 'model',
            ...{ provider: 'mock', model: 'mock' },
  ...
  it('synthesized tool/result carries surfaceOp and sourceEventSeqs when tool/call was logged', () => {
    const events: SessionEvent[] = [
      ...
      { type: 'tool/call', seq: 3, time: 3, data: { turn: 1, step: 1, callId: CallId('call-1'), name: 'dsh_bash', arguments: '{}' } },
    ]
    const closers = interruptedTurnClosers(events)
    expect(closers.map(e => e.type)).toEqual(['tool/result', 'step/end', 'turn/end'])
    const result = closers[0]!
  ...
  it('handles tool/call without a matching assistant/message entry gracefully', () => {
    ...
    const events: SessionEvent[] = [
      userTurnStart(1, 0),
      { type: 'step/start', seq: 1, time: 1, data: { turn: 1, step: 1 } },
      { type: 'tool/call', seq: 2, time: 2, data: { turn: 1, step: 1, callId: CallId('orphan'), name: 'dsh_bash', arguments: '{}' } },
    ]
    const closers = interruptedTurnClosers(events)
