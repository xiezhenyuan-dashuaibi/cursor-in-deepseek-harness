 ...
 * misconfiguration rejections, the dsh_run_code dispatch bridge (serialization,
...
/** Dispatch dsh_run_code through the registry pipeline, as the loop would. */
...
describe('mode-aware wire contribution', () => {
  it("mode 'native' contributes every schema, no dsh_run_code, no SDK section — and needs no runtime", async () => {
  ...
  it("mode 'code' contributes exactly [dsh_run_code] plus the SDK section declaring the other tools", async () => {
    ...
    expect(sdk?.text).not.toContain('dsh_run_code:')
  })

  it("mode 'code' states the dsh_run_code-only rule BEFORE the per-tool guidance that names each tool", async () => {
    const { ctx, systemPrompt } = await setup({ mode: 'code' })
    registerEcho(ctx)
  ...
  it("mode 'both' omits the dsh_run_code-only rule, because native calls do execute there", async () => {
  ...
  it("mode 'both' contributes every native schema plus dsh_run_code, and the SDK section", async () => {
    const { ctx, systemPrompt } = await setup({ mode: 'both' })
    registerEcho(ctx)
    const assembly = await systemPrompt.assemble()
    expect(assembly.tools.map(tool => tool.name)).toEqual([RUN_CODE_NAME, 'echo'])
    expect(assembly.sections.some(section => section.name === 'tools:sdk')).toBe(true)
  })

  it.each(['code', 'both'] as const)('keeps the dsh_run_code transport outside scoped allow-list filtering in mode %s', async (mode) => {
    ...
    expect(unrestricted.tools.map(tool => tool.name)).toEqual(mode === 'code'
      ? [RUN_CODE_NAME]
      : [RUN_CODE_NAME, 'echo', 'hidden'])
  })

  it.each(['code', 'both'] as const)('keeps the dsh_run_code transport outside scoped deny-list filtering in mode %s', async (mode) => {
    const { ctx, systemPrompt, runtime } = await setup({ mode })
  ...
  it.each(['code', 'both'] as const)('reserves dsh_run_code against scoped shadows and explicit restrictions in mode %s', async (mode) => {
    const { ctx, systemPrompt } = await setup({ mode })
    const { scope, agent } = await mintAgentScope(ctx)
    ...
    expect(result.content).toEqual([{ type: 'text', text: '(dsh_run_code completed with no output)' }])
  })

  it.each(['code', 'both'] as const)('keeps dsh_run_code in the toolOrder universe without exposing it as a restriction target in mode %s', async (mode) => {
  ...
  it("never exposes dsh_run_code to programs, even under mode 'both' (no recursive dispatch path)", async () => {
    ...
    runtime.behavior = (request) => {
      ...
      return Promise.resolve({
        ...
        value: JSON.stringify({
          ...
          // there is no handle a program could re-enter dsh_run_code through.
  ...
  it("assembles under a python runtime in mode 'both' as well, SDK and schema together", async () => {
    ...
    // `both` keeps the native tools alongside dsh_run_code; `code` does not.
    expect(assembly.tools.map(tool => tool.name)).toContain('echo')
  })

  it('emits a TypeScript-flavored dsh_run_code schema under a typescript runtime', async () => {
  ...
  it('emits a Python-flavored dsh_run_code schema under a python runtime (matches the SDK language)', async () => {
    const { ctx, systemPrompt } = await setup({ mode: 'code', runtime: { language: 'python' } })
    registerEcho(ctx)
    const assembly = await systemPrompt.assemble()
    const runCodeSchema = assembly.tools.find(tool => tool.name === RUN_CODE_NAME)
  ...
  it('resolves the dsh_run_code schema flavor lazily and fails loud on a language absent from the flavor table', async () => {
    ...
    expect(() => definition?.description)
      .toThrow(/no dsh_run_code schema flavor registered for runtime language "ruby" \(known: "typescript", "python"\)/)
  })

  it('degrades the dsh_run_code flavor to TypeScript when no runtime is mounted', async () => {
    ...
    // `mode: code` and reads dsh_run_code's schema WITHOUT a runtime. peekRuntime
  ...
  it('removes dsh_run_code and the SDK section when the registry fiber disposes (HMR safety)', async () => {
    const ctx = new Context()
...
describe('the sub-dispatch scheduler (native concurrency contract)', () => {
  ...
  it('run settlement drains a commit already in progress: the settle event is appended inside the turn', async () => {
    ...
    // preceded the dsh_run_code turn closing (all appends happen inside
  ...
  it('a queued-unstarted call abandoned by run settlement logs no start event', async () => {
    ...
    expect(abandoned).toEqual(['dsh_run_code run is over (dsh_run_code settled); writer tool call abandoned'])
  })
})

describe('the dsh_run_code dispatch bridge', () => {
  it('bridges tool calls, returns only the curated output, and logs one event per dispatch', async () => {
    ...
    expect(result.isError).toBe(false)
    if (result.isError) throw new Error('expected dsh_run_code success')
  ...
  it('forwards a nested terminal conclusion onto the successful dsh_run_code result', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    ctx.tools.register(defineTool({
      name: 'finalize',
  ...
  it('defers sub-call additionalContexts onto the outer dsh_run_code result', async () => {
    const { ctx, runtime } = await setup({ mode: 'code' })
    registerEcho(ctx)
    ctx.on('tools/post-execute', (exec, _result, next): Promise<PostToolDecision> => {
      if (exec.name === 'echo') {
  ...
  it('keeps sub-call contexts when dsh_run_code fails after the nested dispatch', async () => {
    const { ctx, runtime } = await setup({ mode: 'both' })
    registerEcho(ctx)
    ctx.on('tools/post-execute', (exec, _result, next): Promise<PostToolDecision> => {
  ...
  it('a runtime that starts a binding call and then REJECTS still reaches quiescence before returning', async () => {
    ...
    // logged INSIDE the dsh_run_code execution, not after it returned.
    expect(sawAbort).toBe(true)
  ...
  it('executing dsh_run_code under a missing runtime is a structured isError, not a crash', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt, {})
  ...
  it.each([
    ...
    ['no output', { logs: [] }, '(dsh_run_code completed with no output)'],
  ...
  it('renders every non-string JSON root as pretty JSON while preserving strings raw', async () => {
    ...
    expect(absent.content[0]).toEqual({ type: 'text', text: '(dsh_run_code completed with no output)' })
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    ...
    // scope's effective mode would announce [dsh_run_code] and still execute the
  ...
  it('keeps dsh_run_code out of a native agent\'s dispatch table', async () => {
  ...
  it('reserves dsh_run_code even where no agent presents it', async () => {
    const { ctx } = await setup({ mode: 'native' })
