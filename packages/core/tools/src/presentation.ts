 ...
 * newline, already truncated to the dsh_read tool's per-line cap.
 */
 ...
 * model-facing result text alone, so the dsh_read tool projects them through its
 ...
 * by a web tool whose call retrieves from the web (`dsh_web_search`, `dsh_web_fetch`).
 ...
 * analogue of the `dsh_web_search`/`dsh_web_fetch` calls' generic call views
...
/**
 * The completed state of a `dsh_web_search` call: the structured sources the model
...
/**
 * The completed state of a `dsh_web_fetch` call: the fetched URL, its HTTP status,
