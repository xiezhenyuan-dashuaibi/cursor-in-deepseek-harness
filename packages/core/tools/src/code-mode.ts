/**
 * Code Mode `dsh_run_code` transport. Programs call the registry's agent-visible
...
export const RUN_CODE_NAME = 'dsh_run_code'
...
/**
 * The language-specific `dsh_run_code` schema text: the tool `description` and its
...
/** Per-language `dsh_run_code` schema flavors (see {@link RunCodeFlavor}); one entry per {@link CodeSdkLanguage}. */
 ...
 * schema-emission time so the model-visible `dsh_run_code` schema always matches
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`dsh-tools: no dsh_run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}

/**
 * Thrown by `dsh_run_code` when the program run itself failed — a program
...
/** Two-space JSON presentation, matching the existing shallow `dsh_run_code` text contract. */
const JSON_INDENT = '  '
...
/**
 * Build the `dsh_run_code` {@link ToolDefinition}: required `code` and
  ...
  const definition = defineTool({
    ...
    async execute(args, exec): Promise<RunCodeOutput> {
      ...
      const drainDispatches = async (): Promise<void> => {
        ...
        await drive()
        // Every settle event is appended inside the open dsh_run_code turn
      ...
      const binding = (name: string): CodeBindingFunction => async (rawArgs: unknown): Promise<JsonValue> => {
        if (runOver()) {
          throw new Error(`dsh_run_code run is over (${String(runController.signal.reason)}); ${name} not dispatched`)
        }
        ...
        const outcome = await new Promise<DispatchOutcome>((resolve, reject) => {
          ...
          pendingQueue.push({
            ...
            abandon: () => {
              reject(new Error(`dsh_run_code run is over (${String(runController.signal.reason)}); ${name} tool call abandoned`))
            },
        ...
        if (runOver()) {
          throw new Error(`dsh_run_code run is over (${String(runController.signal.reason)}); ${name} result discarded`)
        }
      ...
      try {
        ...
        try {
          ...
          runController.abort('dsh_run_code settled')
