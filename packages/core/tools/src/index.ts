 ...
 * its `dsh_run_code` schema strings — plus the renderer function this table points
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * `dsh_run_code` sub-dispatch outcome before the bridge appends its
...
export interface ToolExecutionInput {
   ...
   * the outer `dsh_run_code` outcome without receiving its live mutable execution.
...
/**
 * One settled `dsh_run_code` sub-dispatch about to be logged, as seen by the
...
export interface CodeDispatchLog {
  /** The outer `dsh_run_code` execution. */
...
export interface Config {
   ...
   * sends only `dsh_run_code` plus a generated SDK prompt and collapses the
   * executor to the same surface (a model-direct call may only name
   * `dsh_run_code`; `dsh_run_code` SDK sub-dispatches keep every visible tool); `both`
  ...
  /**
   * Concurrency cap for a `dsh_run_code` program's overlapping sub-calls
...
/** Resolve the dsh_run_code overlap cap at the owning config boundary (direct construction bypasses the Loader schema). */
...
export class ToolRuntime extends Service {
   ...
   * `dsh_run_code` may be called, so it emits a native call, receives
  ...
  /**
   * The reserved `dsh_run_code` transport, built on first need.
   *
  ...
  private wireSchemas(scope?: ScopeKey): ToolProviderResult {
    ...
    // dsh_run_code's language-aware description/parameters getters, whose own
   ...
   * Read at use time (assembly / dsh_run_code execution), NOT via static
   * `inject`: an inject entry would hold `ctx.tools` — and every tool plugin
   * behind it — hostage to a code runtime existing even under `mode:
   * 'native'` (the loop's optional-backend idiom, same as
   * `sessionPersistence`).
   *
   * Assembly and `dsh_run_code` execution read separately, so the language is not
   ...
   * globals; duplicates within one layer and the reserved `dsh_run_code` name fail.
  ...
  private view(scope?: ScopeKey): ToolView {
    ...
    // changes. Per scope: a native agent must not find `dsh_run_code` in its
   ...
   * may only name the reserved `dsh_run_code` transport, while a nested
   * sub-dispatch (a `parent` token set — the `dsh_run_code` SDK calling a tool
   ...
   * the ONE consumer is the `dsh_run_code` bridge this registry constructs, which
   ...
   * reserved `dsh_run_code` transport may be named. Nested sub-dispatches (a
