...
describe('the shipped Web composition', () => {
  it('leaves the global tool layer empty', () => {
    // Every model-facing tool belongs to a preset, `dsh_ask_user_question`
  ...
  it('composes the full agent from `standard`', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent).filter(name => name !== 'dsh_glob' && name !== 'dsh_grep')).toEqual([
        'dsh_ask_user_question', 'dsh_bash', 'dsh_create_goal', 'dsh_edit', 'dsh_exit_plan_mode',
        'dsh_get_goal', 'dsh_interrupt_agent', 'dsh_job_kill', 'dsh_job_list', 'dsh_job_output', 'dsh_list_agents', 'dsh_ralph', 'dsh_read', 'dsh_read_image', 'dsh_send_message', 'dsh_skill',
        'dsh_subagent', 'dsh_subagent_fork', 'dsh_todo_write', 'dsh_update_goal', 'dsh_web_search',
        'dsh_workflow', 'dsh_write',
      ])
    } finally {
      await handle.dispose()
    }
  ...
  it('composes the exact RL prompt and two tools from `minimal`', async () => {
    ...
    try {
      ...
      expect(assembly.tools.map(tool => tool.name)).toEqual(['dsh_bash', 'dsh_str_replace_editor'])
      expect(assembly.tools.find(tool => tool.name === 'dsh_bash')?.description).toBe(MINIMAL_BASH_DESCRIPTION)
      expect(JSON.stringify(assembly.tools.find(tool => tool.name === 'dsh_str_replace_editor')?.parameters))
  ...
  it('keeps two differently composed sessions independent', async () => {
    ...
    try {
      expect(toolNames(ctx, minimal.agent)).toEqual(['dsh_bash', 'dsh_str_replace_editor'])
  ...
  it('composes the cordis agent with its own toolset', async () => {
    ...
    try {
      ...
      expect(tools).toEqual(expect.arrayContaining([
        'dsh_cordis_inspect_list', 'dsh_cordis_inspect_query', 'dsh_cordis_inspect_self',
        'dsh_cordis_define', 'dsh_cordis_run', 'dsh_cordis_stop', 'dsh_cordis_undefine',
      ]))
      // And it keeps the standard agent's own tools rather than replacing them.
      expect(tools).toEqual(expect.arrayContaining(['dsh_bash', 'dsh_read', 'dsh_edit', 'dsh_skill']))
      expect(tools).not.toContain('dsh_str_replace_editor')
  ...
  it('presents `code` as Code Mode without disturbing a native session beside it', async () => {
    ...
    try {
      ...
      expect(assembly.tools.map(tool => tool.name)).toEqual(['dsh_run_code'])
      expect(toolNames(ctx, coded.agent)).not.toContain('dsh_str_replace_editor')
      const sdk = assembly.sections.find(section => section.name === 'tools:sdk')?.text ?? ''
      expect(sdk).not.toContain('dsh_str_replace_editor')
      expect(sdk).toContain('dsh_web_search')
      ...
      expect(nativeAssembly.tools.map(tool => tool.name)).toContain('dsh_bash')
      expect(nativeAssembly.tools.map(tool => tool.name)).not.toContain('dsh_run_code')
  ...
  it('keeps the self-referential toolset out of every other preset', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent)).not.toContain('dsh_cordis_define')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent)).toEqual(['dsh_bash', 'dsh_str_replace_editor'])
    } finally {
      await handle.dispose()
    }
  })
...
describe('product subagent rows in user presets', () => {
  ...
  it('composes none, either product, or both without changing the shared host registry', async () => {
    const expected = new Map<string, string[]>([
      ['products-none', []],
      ['products-codex', ['dsh_subagent_codex']],
      ['products-claude', ['dsh_subagent_claude_code']],
      ['products-both', ['dsh_subagent_claude_code', 'dsh_subagent_codex']],
    ])
    ...
    for (const [id, productTools] of expected) {
      ...
      try {
        ...
        expect(tools.filter(name => name === 'dsh_subagent_codex' || name === 'dsh_subagent_claude_code'))
  ...
  it('applies a product-row edit only to later sessions on the preset', async () => {
    ...
    try {
      expect(toolNames(productCtx, existing.agent)).not.toContain('dsh_subagent_codex')
      ...
      try {
        expect(toolNames(productCtx, existing.agent)).not.toContain('dsh_subagent_codex')
        expect(toolNames(productCtx, later.agent)).toContain('dsh_subagent_codex')
      } finally {
        await later.dispose()
      }
    } finally {
...
describe('a delegated child', () => {
  it('runs on the composition its parent runs on', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, child.agent)).toContain('dsh_bash')
...
describe('a launcher that configures no writable root', () => {
  ...
  it('discovers and mounts a preset the person authored under the harness home', async () => {
    ...
    try {
      expect(toolNames(derivedCtx, handle.agent)).toContain('dsh_todo_write')
    } finally {
      await handle.dispose()
    }
  })
...
describe('authoring a preset on the shipped composition', () => {
  ...
  it('copies a shipped preset a session then really composes from', async () => {
    ...
    try {
      ...
      expect(toolNames(authorCtx, handle.agent)).toEqual(['dsh_bash', 'dsh_str_replace_editor'])
    } finally {
      await handle.dispose()
    }
  })
