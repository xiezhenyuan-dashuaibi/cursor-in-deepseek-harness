...
Read `dsh_cordis_inspect_query` (`platform: "host"`, `provider: "Service"`, `method: "listService"`, `input: { service: "agentPresets" }`) for the current signatures before writing the code. What this skill relies on:
...
Whether a row publishes a service is not visible from its name, and package READMEs are absent from an installed deployment. Read signatures with `dsh_cordis_inspect_query` (`platform: "host"`, `provider: "Service"`, `method: "listService"`). For a row not in your current composition, mount-validate and read the rejection — it names the offending service.
...
The inspect tools report THIS session's composition, so they confirm what a row does in the runtime you are already in, never what your new preset will do.
...
- id: tool-subagent-claude-code
  ...
  config:
    ...
    toolName: dsh_subagent_claude_code
    enableRunInBackground: false
    maxDepth: provider-managed
```
