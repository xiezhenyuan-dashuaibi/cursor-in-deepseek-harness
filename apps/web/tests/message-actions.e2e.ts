...
const PROMPT = 'Use the dsh_read tool twice in one assistant message: read a.txt and b.txt. Then reply with the single word DONE and stop.'
...
describe('web e2e: message IconActions and clocks on settled history', () => {
  ...
  it.skipIf(MODE === 'record')('forks through the settled-message and session-row actions', async () => {
    ...
    await expect.poll(
      ...
      { timeout: 10_000 },
    ).toContain('Use the dsh_read tool twice (2)')
