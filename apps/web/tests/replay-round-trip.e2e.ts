...
// drift apart.
const PROMPT = 'Use the dsh_bash tool to run exactly: echo WEB_E2E_OK. Then reply with the single word DONE and stop.'
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('rendered the settled turn: markdown, tool row, composer restore', async () => {
    ...
    const bashCall = sessionEvents.find(event => event.type === 'tool/call' && event.data.name === 'dsh_bash')
    if (bashCall?.type !== 'tool/call') throw new Error('the replayed turn did not call the dsh_bash tool')
    const bashResult = sessionEvents.find(event =>
      event.type === 'tool/result' && event.data.message.source.callId === bashCall.data.callId)
    if (bashResult?.type !== 'tool/result') throw new Error('the dsh_bash tool call produced no durable result')
