...
const PROMPT = 'Load the snapshot-skill skill with the dsh_skill tool, then reply DONE.'
...
describe.skipIf(MODE === 'record')('web e2e: dedicated Skill tool row', () => {
  ...
  beforeAll(async () => {
    ...
    await page.locator('[data-tool="dsh_skill"]').waitFor({ timeout: 15_000 })
  }, 120_000)
  ...
  it('expands the loaded skill to its exact recorded instructions', async () => {
    ...
    const call = page.locator('[data-tool="dsh_skill"]')
