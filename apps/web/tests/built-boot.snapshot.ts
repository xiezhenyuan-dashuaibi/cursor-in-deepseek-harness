...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  // dsh_web_search / dsh_web_fetch turns render their keyed WebRow cards, proving the
  ...
  const webSearchRow = await waitFor(() => {
    const row = document.querySelector('[data-tool="dsh_web_search"]')
    expect(row).not.toBeNull()
    expect(document.querySelector('[data-tool="dsh_web_fetch"]')).not.toBeNull()
    return row!
  }, { timeout: 10_000 })
  // Expand the dsh_web_search row to prove its WebBlock card renders end to end.
