...
describe.skipIf(MODE === 'record')('web e2e: conversational reminders', () => {
  ...
  it('uses request-local browser context to create an explicit local At reminder', async () => {
    ...
    expect(firstRequest.tools?.some(tool => tool.name === 'dsh_schedule_create')).toBe(true)
    ...
    const toolCall = atHandle.agent.session.events.find(event => (
      event.type === 'tool/call' && event.data.name === 'dsh_schedule_create'
    ))
    if (toolCall?.type !== 'tool/call') throw new Error('missing dsh_schedule_create tool call')
