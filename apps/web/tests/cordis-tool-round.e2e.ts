...
const CORDIS_TOOLS = ['dsh_cordis_inspect_self', 'dsh_cordis_define', 'dsh_cordis_run', 'dsh_cordis_stop'] as const
...
const PROMPT = 'Use only Cordis tools. First call dsh_cordis_inspect_self with no arguments. '
  + 'Then call dsh_cordis_define with plugin kind "new", idPrefix "snap", name "snapshot noop", '
  ...
  + 'Read its returned pluginId and packageId, then call dsh_cordis_run with those exact IDs and mode "run". '
  + 'After the run request returns, reply exactly CORDIS_UI_READY and stop.'
const STOP_PROMPT = 'Use only Cordis tools. Call dsh_cordis_stop with pluginId "snap-1". '
...
describe('web e2e: Cordis tools use their owned cards', () => {
  ...
  it.skipIf(MODE === 'record')('renders localized Cordis lifecycle cards', async () => {
    ...
    const inspectRow = page.locator('[data-tool="dsh_cordis_inspect_self"]').filter({ hasText: 'Inspect' }).first()
    await inspectRow.waitFor({ timeout: 10_000 })

    // dsh_cordis_define does NOT go through the generic row: ui-cordis registers a
    ...
    const defineRow = page.locator('[data-tool="dsh_cordis_define"]').filter({ hasText: 'Cordis Plugin' }).first()
    ...
    const runRow = page.locator('[data-tool="dsh_cordis_run"]').filter({ hasText: 'Run Cordis Plugin' }).first()
    await runRow.waitFor({ timeout: 10_000 })
    await expect.poll(() => runRow.textContent()).toContain('snap-')

    const stopRow = page.locator('[data-tool="dsh_cordis_stop"]').filter({ hasText: 'Stop Cordis Plugin' }).first()
