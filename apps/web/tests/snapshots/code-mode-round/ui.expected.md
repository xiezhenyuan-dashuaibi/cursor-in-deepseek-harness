- banner:
  - navigation "Session hierarchy":
    - 'button "Using ONE dsh_run_code program: run" [disabled]'
  - img
  - text: Standard mode
  - button "Session log":
    - text: Session log
  ...
  - tablist:
    - tab "Chat" [selected]
    - tab "Trajectory"
- text: "Using ONE dsh_run_code program: run bash `echo CODE_ROUND_OK`, then read the file missing.txt catching its error in the program. Return an object with both outcomes. Then reply DONE and stop. {{clock}}"
- button "Copy":
  - img
...
- 'button "Think The user wants me to write a single `dsh_run_code` program that:"':
  - img
  - img
  - text: "Think The user wants me to write a single `dsh_run_code` program that:"
