...
describe('dsh web keyless CLI smoke', () => {
  ...
  it('routes web runtime context and workspace instructions through the real CLI request', async () => {
    ...
    try {
      ...
      expect(captured.tools?.map(tool => tool.function?.name)
        .filter(name => name === 'dsh_web_search' || name === 'dsh_web_fetch'))
  ...
  it('DSH_TOOLS_MODE=code collapses the provider wire tools to dsh_run_code with the SDK prompt section', async () => {
    requireDist()
    ...
    try {
      ...
      expect(captured.tools?.map(tool => tool.function?.name)).toEqual(['dsh_run_code'])
      const system = captured.messages?.find(message => message.role === 'system')
      expect(system?.content).toContain('## Writing code for dsh_run_code')
