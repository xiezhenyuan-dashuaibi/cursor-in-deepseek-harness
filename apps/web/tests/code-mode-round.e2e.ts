...
// shipped tree with the tools row patched to mode: code (the dsh_run_code-only
// wire), a real chromium sends a prompt engineered to elicit one dsh_run_code
...
const PROMPT = 'Using ONE dsh_run_code program: run bash `echo CODE_ROUND_OK`, then read the file missing.txt '
...
describe('web e2e: Code Mode round renders nested sub-calls', () => {
  ...
  it.skipIf(MODE === 'record')('the durable log carries dsh_run_code with full-content sub-dispatches', () => {
    // Wire discipline: code mode collapsed the call surface to dsh_run_code.
    const calls = sessionEvents.filter(event => event.type === 'tool/call')
    expect(calls.length).toBeGreaterThanOrEqual(1)
    expect(new Set(calls.map(call => (call.data as { name: string }).name))).toEqual(new Set(['dsh_run_code']))
    ...
    const bash = dispatches.find(dispatch => (dispatch.data as { name: string }).name === 'dsh_bash')
  ...
  it.skipIf(MODE === 'record')('renders the code parent row with always-visible nested sub-rows', async () => {
    ...
    // The parent dsh_run_code row wears the code variant with the model-authored
