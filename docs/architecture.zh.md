...
[扩展实操手册](cookbook/extension-cookbook.md)将功能映射到能力，并索引[包](cookbook/adding-a-package.md)、[工具](cookbook/adding-a-tool.md)、[LLM（大语言模型）适配器](cookbook/adding-an-llm-adapter.md)和 [Chat 节点](cookbook/adding-a-conversation-node.md)的分步指南。overlay 卡片窗口的操作 HOW 是 skill [`dsh-overlay-web-plugins`](../.agents/skills/dsh-overlay-web-plugins/SKILL.md)；桌面 HOW 是 [`dsh-overlay-canvas-plugins`](../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md)；任意形状悬件仍是预留 skill [`dsh-overlay-shaped-plugins`](../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md)。[overlay 网页插件](cookbook/overlay-web-plugins.md)页是卡片形态在文档站点上的指针。
