...
export interface Config {
   ...
   * `code` sends only `dsh_run_code` plus a generated SDK, `both` sends both.
...
<a id="deepseek-aidsh-cursor-agent-gateway"></a>
...
来源：[`packages/cursor/mcp-server/src/index.ts:52`](../packages/cursor/mcp-server/src/index.ts)
...
export interface ReplayModelConfig {
  ...
  contextWindow?: number
  /** Optional declared input modalities, so a scenario can exercise capability gates (e.g. image-capable `dsh_read_image`). */
...
```ts config-catalog
/** Configuration for the dsh_bash tool. */
export interface Config {
...
```ts config-catalog
/** Configures bounded `dsh_job_output` waits and completion-notice delivery. */
export interface Config {
  /** Wait duration applied when `dsh_job_output` sets `wait` without `timeout_ms` (default 30s). */
...
```ts config-catalog
/** Configuration for the dsh_pwsh tool. */
export interface Config {
  /** Expose `run_in_background` (default true); disabled calls are also rejected. */
  enableRunInBackground?: boolean
}
...
export interface Config {
  ...
  provider: string
  /**
   * Model-facing tool name (default `dsh_subagent`). Each loaded instance must use
   * a distinct name.
   */
  toolName?: string
  /**
...
export interface Config {
  ...
  search?: boolean
  /** Register `dsh_web_fetch`. Defaults to true. */
  fetch?: boolean
  /** Upper bound on sources returned by one `dsh_web_search` call. */
  searchMaxResults?: number
  /** Cooperative timeout budget (ms) for `dsh_web_fetch`. Defaults to 30000. */
  fetchTimeoutMs?: number
  /** Cooperative timeout budget (ms) for `dsh_web_search`. Defaults to 30000. */
  searchTimeoutMs?: number
  /** Cap on source characters converted and complete `dsh_web_fetch` output characters. Defaults to 200000. */
...
export interface Config {
  /** The model-facing tool name to register (default `dsh_workflow`). */
  toolName?: string
...
export interface Config {
  ...
  maxTokens?: number
  /** Maximum `dsh_web_search` server-tool uses per request. Defaults to 5. */
  maxUses?: number
}
```
...
- `@deepseek-ai/dsh-cursor-mcp-prompt`（[`packages/cursor/mcp-prompt/src/index.ts`](../packages/cursor/mcp-prompt/src/index.ts)）
