...
Shipped model-facing names match `^dsh_[a-z0-9_]+$` (`dsh_read`, `dsh_bash`, `dsh_run_code`). Tests and demos may use other names such as `echo`. MCP-imported remote tools keep the remote name. [Rationale](../../.agents/notes/implemented/architecture/2026-08-31-model-facing-tool-names-dsh-prefix.md).
...
- `presentResult(args, { content, isError, meta? })` returns the completed card:
  ...
  - `web` supplies a completed web retrieval, discriminated by `kind: 'search' | 'fetch'` (the structured search sources or the fetch summary), derived from `result.meta`; it carries no body copy, so a UI without the `web` capability falls back to the raw result content. (tool-web `dsh_web_search`/`dsh_web_fetch`.)
