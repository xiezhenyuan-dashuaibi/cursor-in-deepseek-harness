...
已交付的面向模型名称匹配 `^dsh_[a-z0-9_]+$`（`dsh_read`、`dsh_bash`、`dsh_run_code`）。测试与演示可以使用其他名称，例如 `echo`。MCP 导入的远端工具保留远端名称。[理由](../../.agents/notes/implemented/architecture/2026-08-31-model-facing-tool-names-dsh-prefix.md)。
...
请把 `output.schema` 设计为实用的程序化 API：直接返回句柄与字段；当标量、数组或 null 确实就是结果时，允许采用相应的根类型；将面向人类的解释放入 `output.render`。中间值只存在于执行期间，不会被持久化或按提示词上限截断，也不设字节上限，因此生产方如实声明的采集边界和进程内存仍然重要。只有外层 `dsh_run_code` 日志／结果会受到可配置输出上限和面向模型的 spill 流水线约束。
...
- `presentResult(args, { content, isError, meta? })` 返回完成后的卡片：
  ...
  - `web` 提供已完成的 web 检索，以 `kind: 'search' | 'fetch'` 区分（结构化的搜索来源或抓取摘要），由 `result.meta` 派生；它不携带正文副本，因此不具备 `web` 能力的 UI 回退到原始结果内容。（tool-web `dsh_web_search`／`dsh_web_fetch`。）
