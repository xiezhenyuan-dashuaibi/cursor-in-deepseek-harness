...
overlay 插件是普通 React 页面；有后端时则是普通 Node 服务。它组合进正在运行的 `dsh web`，并且不占据 `root`、也不重启该进程。CSS Modules 和宿主 RPC 按独立应用来写。**卡片窗口**形态复用 overlay 卡片、只写页面；加载 [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md)。任意形状悬件和整页白画板是预留 skill [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md) 和 [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md)。现场插入用 `pnpm overlay:live` 写被监视的 profile patch，而不是 web-app 组合包文件。[overlay 网页插件](overlay-web-plugins.md)页是卡片形态在文档站点上的指针。
...
| 动态工作流 | `ctx.workflowEngine` + worker-thread 引擎 + `workflow` 工具；结构化的进程内子任务通过作用域化的提示词/工具注册、单调工具守卫、最终 `tools/result` 提交（包括外层 `dsh_run_code`）和结构化输出执行的单调 `concludeTurn()` 标记来强制输出 |
...
| Plan mode | [`@deepseek-ai/dsh-plan-mode`](../../packages/plan/plan-mode/README.md)：落日志的 `plan/mode` 状态、`plan:policy` 引导段、`/plan [message]` 入口、`/plan off` 直接退出，以及经用户评审的 `dsh_exit_plan_mode` 出口；强制约束留在独立的沙箱/审批轴上 |
