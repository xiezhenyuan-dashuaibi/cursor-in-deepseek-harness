...
## An overlay web plugin

An overlay plugin is an ordinary React page and, when it has a backend, an ordinary Node service, composed onto a running `dsh web` without occupying `root` and without restarting that process. Write CSS Modules and host RPC like a standalone app. The **card-window** form reuses the overlay card and writes only the page; load [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md). The **desktop** form reuses the overlay desktop board and writes only the page; load [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md). Arbitrary-shape floaters reuse the overlay shaped board and write only the occupant; load [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md). Live insert uses `pnpm overlay:live` against the watched profile patch, not the web-app bundle file. The [overlay web plugins](overlay-web-plugins.md) page is the card-form documentation-site pointer. The [overlay desktop plugins](overlay-desktop-plugins.md) page is the desktop-form pointer. The [overlay shaped plugins](overlay-shaped-plugins.md) page is the shaped-form pointer.
...
| Dynamic workflow | `ctx.workflowEngine` + the worker-thread engine + the `dsh_workflow` tool; structured in-process children enforce output with scoped prompt/tool registrations, a monotonic tool guard, final `tools/result` commit (including enclosing `dsh_run_code`), and the structured-output execution's monotonic `concludeTurn()` marker |
...
| Plan mode | [`@deepseek-ai/dsh-plan-mode`](../../packages/plan/plan-mode/README.md) — logged `plan/mode` state, the `plan:policy` guidance section, `/plan [message]` entry, `/plan off` direct exit, and the user-reviewed `dsh_exit_plan_mode` exit; enforcement stays on the independent sandbox/approval axes |
