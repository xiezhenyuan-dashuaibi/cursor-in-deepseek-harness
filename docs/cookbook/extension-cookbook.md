...
An overlay plugin is an ordinary React page and, when it has a backend, an ordinary Node service, composed onto a running `dsh web` without occupying `root` and without restarting that process. Write CSS Modules and host RPC like a standalone app. The **card-window** form reuses the overlay card and writes only the page; load [dsh-overlay-web-plugins](../../.agents/skills/dsh-overlay-web-plugins/SKILL.md). Arbitrary-shape floaters and the full-page white canvas are reserved skills [dsh-overlay-shaped-plugins](../../.agents/skills/dsh-overlay-shaped-plugins/SKILL.md) and [dsh-overlay-canvas-plugins](../../.agents/skills/dsh-overlay-canvas-plugins/SKILL.md). Live insert uses `pnpm overlay:live` against the watched profile patch, not the web-app bundle file. The [overlay web plugins](overlay-web-plugins.md) page is the card-form documentation-site pointer.
