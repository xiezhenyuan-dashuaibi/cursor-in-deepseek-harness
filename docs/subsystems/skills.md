...
`SkillDefinition` is the complete parsed result returned by `ctx.skills.get()` and used by the `dsh_skill` tool. `resourceBase` tells the tool how to render relative-resource guidance for local, URL, or provider-managed skills.
