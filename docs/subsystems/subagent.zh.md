...
单次 run 的最终产出，由 `SubagentRun.result` resolve。`structured` 仅在请求了 `outputSchema` 且成功满足时才存在；请求 schema 不保证一定能得到它，当子 agent 失败或结束时未产出有效 capture 时，提供方可能返回 `stopReason: 'error'`。`diagnostic` 是 `stopReason` 为 `error` 且该消息非空时，最后一次 `turn/end` 的 `error.message`。非 `completed` 的 `stopReason` 意味着 `output` 可能不完整——消费方将其映射为 `isError` 的工具结果，而非将部分输出报告为成功。
...
interface SubagentResult {
  ...
  /**
   * Failure text from the child's last `turn/end` `error.message` when
   * `stopReason` is `error` and that message is non-empty. Absent otherwise.
   */
