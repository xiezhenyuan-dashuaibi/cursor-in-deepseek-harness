...
`SkillDefinition` 是 `ctx.skills.get()` 返回的完整解析结果，供 `dsh_skill` 工具使用。`resourceBase` 告知工具如何为本地、URL 或提供方管理的 skill 渲染相对资源引导。
...
`dsh-tool-skill` 在存活会话中第一个观察到非空完整视图的 `agent/pre-step` 注入初始的持久 user-role `<system-reminder>`。目录只包含已排序的 skill `name` 和规范化、经 XML 转义的 `description`；不包含正文、路径、来源、提供方或路由提示。发现通过 `SkillLookupOptions` 转发该步骤的 abort signal。`catalogDescriptionMaxLength` 是消费方用于 description 上限的配置，默认值为 `500`，整数最小值为 `3`。Cursor MCP 不会在客户端对话上运行该 pre-step；[`dsh-cursor-mcp-prompt`](../../packages/cursor/mcp-prompt/README.md) 把相同的模型可调用条目写入 initialize instructions 和 `dsh_system_prompt`，[`dsh-cursor-mcp-server`](../../packages/cursor/mcp-server/README.md) 为 owner Agent 对 `ctx.skills` 做 snapshot。
