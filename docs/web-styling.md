...
- Use `--dsw-alias-*` semantic tokens in feature chrome that should follow the theme. Do not copy the static scale or invent a second global theme.
- A product page whose identity is a specific palette may set local custom properties to those colors. Do not map product colors through unrelated semantic aliases such as success, error, or warn.
