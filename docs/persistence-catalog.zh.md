 ...
 * parent `dsh_run_code`'s execution (the bridge drains in-flight dispatches
...
```ts persistence-catalog
/**
 * One sub-dispatch STARTING inside a `dsh_run_code` program: the parent
 * `dsh_run_code` call id, the deterministic sub-call id (`<parent>:code:<n>`,
