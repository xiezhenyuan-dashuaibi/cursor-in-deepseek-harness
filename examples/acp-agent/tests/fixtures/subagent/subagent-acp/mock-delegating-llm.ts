 ...
 * `dsh_subagent` tool once, and the follow-up streams the tool result text back
...
class MockDelegatingAdapter extends LlmAdapter {
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResultText.length === 0) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: CallId('call-delegate'), name: 'dsh_subagent', argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('call-delegate'), name: 'dsh_subagent', arguments: args } }
