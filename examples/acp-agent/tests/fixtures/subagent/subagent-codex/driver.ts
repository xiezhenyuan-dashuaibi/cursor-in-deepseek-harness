...
try {
  ...
  const tool = ctx.tools.schemas().find(schema => schema.name === 'dsh_subagent_codex')
  if (tool === undefined) throw new Error('dsh_subagent_codex tool was not registered')
  const properties = tool.parameters.properties
  if (typeof properties !== 'object' || properties === null || Array.isArray(properties)) {
    throw new Error('dsh_subagent_codex tool has invalid parameter properties')
  }
