...
const SCENARIOS: Scenario[] = [
  ...
  // real dsh_read_image tool executes against the workspace fixture and the real
  ...
  // with the attachment store, independent of route), so they share one class.
  {
    ...
    pinsHeader: true,
    headerClass: 'image',
    // The overlay adds no prompt section (dsh_read_image carries no guidance), so
  ...
  // dsh_web_fetch markdown rendering end to end: the overlay's loopback fixture
  ...
  // the fixture scripts five identical dsh_todo_write calls and pins BOTH reminder
  ...
  // only the durable subagent id, two dsh_send_message calls queue as later FIFO
  // turns on that same child (the parent is never woken with their output),
  // dsh_send_message to an unknown subagent id fails without delivering, and the
  ...
  // `dsh_list_agents({ scope: 'descendants' })` deterministically reads the
  // persisted child as complete, then `dsh_interrupt_agent` executes its accepted
  ...
  // the real dsh_ask_user_question tool, the runtime-ownership guard rejects before
  ...
  // Code Mode: the registry in `mode: code` — the wire tool list collapses to [dsh_run_code], the
  ...
  // A nested fs dispatch inside dsh_run_code discovers workspace instructions. The
  ...
  // the two modes agree on every section except the dsh_run_code-only rule, which
