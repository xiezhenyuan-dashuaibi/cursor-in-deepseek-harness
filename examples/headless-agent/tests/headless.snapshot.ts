...
describe('headless stream-json snapshots', () => {
  ...
  it('replays persisted goal tools through the one-shot app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      inspect: async (cwd) => {
        ...
        expect(calls).toEqual(['dsh_update_goal', 'dsh_create_goal', 'dsh_get_goal'])
  ...
  it('delivers a continuable child result without parent polling', async () => {
    ...
    const task = 'Start one continuable background subagent and answer from its completion notice. Do not call dsh_list_agents, dsh_send_message, dsh_job_output, or dsh_job_list.'
    let runCwd = ''
    const result = await runLoaderSmoke({
      ...
      inspect: async (cwd) => {
        ...
        expect(calls.map(record => (record.data as JsonObject | undefined)?.name)).toEqual(['dsh_subagent'])
        ...
        expect(normalizedChild).not.toContain('"name":"dsh_report"')
      },
    })
