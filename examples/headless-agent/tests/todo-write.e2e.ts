...
/**
 * A REAL model drives the REAL dsh_todo_write tool: verify the WORLD (the session
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('dsh_todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    ...
    agent.followup(createUserMessage({
      ...
      'Use the dsh_todo_write tool to record a plan of exactly three steps for work '
      ...
      + 'Send all three in one dsh_todo_write call, then reply with the single word DONE.' }], source: { kind: 'user' } }))
    ...
    expect(calls.some(event => event.data.name === 'dsh_todo_write')).toBe(true)
