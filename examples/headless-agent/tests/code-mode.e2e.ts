...
/**
 * With-key Code Mode proof: a real model receives only `dsh_run_code`, composes two
...
const PERSONA = 'You are a coding agent. You work by writing TypeScript programs for dsh_run_code: '
...
/** Read the optional completion from a successful canonical `dsh_run_code` value. */
function completion(result: ToolExecutionResult): unknown {
  ...
  if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('invalid dsh_run_code result')
  return value.result
}
...
describe('Code Mode typed values: keyless real-worker contracts', () => {
  ...
  it('returns a background job id, settles the outer run, and polls that id to completion', async () => {
    ...
    const jobId = completion(await runCode(ctx, `
      const started = await tools.dsh_bash({
    ...
    const polled = completion(await runCode(ctx, `
      return await tools.dsh_job_output({ job_id: ${JSON.stringify(jobId)}, wait: true, timeout_ms: 5000 });
    `))
    if (typeof polled !== 'object' || polled === null || Array.isArray(polled)) throw new Error('invalid dsh_job_output completion')
  ...
  it('pre-abort spawns nothing; post-publication abort leaves dsh_job_kill as the cancellation owner', async () => {
    ...
    const preResult = await runCode(ctx, `
      return await tools.dsh_bash({ command: 'sleep 10', description: 'Must never start', run_in_background: true });
    `, pre.signal)
    ...
    const running = runCode(ctx, `
      const started = await tools.dsh_bash({ command: 'sleep 10', description: 'Wait for explicit task kill', run_in_background: true });
    ...
    const killed = completion(await runCode(ctx, `
      return await tools.dsh_job_kill({ job_id: ${JSON.stringify(job!.id)}, reason: 'test owns cancellation' });
    `))
    expect(killed).toMatchObject({ outcome: 'cancellation-requested', job: { id: job!.id } })
    const settled = completion(await runCode(ctx, `
      return await tools.dsh_job_output({ job_id: ${JSON.stringify(job!.id)}, wait: true, timeout_ms: 5000 });
    `))
  ...
  it('keeps foreground bash coupled to the outer signal', async () => {
    ...
    const pending = runCode(ctx, `
      return await tools.dsh_bash({ command: 'sleep 10', description: 'Run cancellable foreground command' });
    `, controller.signal)
  ...
  it('uses versioned Cordis DTO ids directly for running and pending Plugins, then confirms removal', async () => {
    ...
    const value = completion(await runCode(ctx, `
      const activeDefinition = await tools.dsh_cordis_define({
      ...
      const active = await tools.dsh_cordis_run({
        pluginId: activeDefinition.pluginId,
        packageId: activeDefinition.packageId,
        mode: 'run',
      });
      const pendingDefinition = await tools.dsh_cordis_define({
      ...
      const pending = await tools.dsh_cordis_run({
        pluginId: pendingDefinition.pluginId,
        packageId: pendingDefinition.packageId,
        mode: 'run',
      });
      const before = await tools.dsh_cordis_inspect_self({});
      const removed = await tools.dsh_cordis_undefine({ pluginId: active.pluginId });
      const after = await tools.dsh_cordis_inspect_self({});
      await tools.dsh_cordis_undefine({ pluginId: pending.pluginId });
      return {
        active: {
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('Code Mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [dsh_run_code], bridges sub-calls, and returns curated output', async () => {
    ...
    agent.followup(createUserMessage({
      content: [{
        type: 'text',
        text: 'Using one dsh_run_code program: run `echo alpha-7` with the dsh_bash tool, run `echo beta-9` with the dsh_bash tool, '
    ...
    // tool — dsh_run_code (the logged header snapshots the assembled list).
    const headers = events.filter(event => event.type === 'request/header')
    expect(headers.length).toBeGreaterThan(0)
    for (const header of headers) {
      expect(header.data.header.tools?.map(tool => tool.name)).toEqual([RUN_CODE_NAME])
    }
    // The model actually went through dsh_run_code…
    ...
    expect(dispatches.every(event => event.data.name === 'dsh_bash')).toBe(true)
  ...
  it('projects nested workspace instructions discovered by an fs sub-call', async () => {
    ...
    handle.agent.followup(createUserMessage({
      content: [{
        type: 'text',
        text: 'Use one dsh_run_code program to call tools.dsh_read on pkg/deep/task.txt. After it finishes, answer: Code Mode workspace handshake?',
      }], source: { kind: 'user' } }))
    await waitForIdle(ctx, handle.agent)

    const events: SessionEvent[] = [...handle.agent.session.events]
    const dispatch = events.find(event => event.type === 'tool/code-dispatch' && event.data.name === 'dsh_read')
