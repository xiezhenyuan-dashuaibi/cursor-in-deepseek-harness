...
离开页面不再停下 Cursor。想关掉 CLI 的操作者必须结束那条轨上的会话（或停下 `dsh web`）。宿主重启仍会丢掉所有 runtime；下一次打开页面从 Chat 1 起算，新起 CLI，snapshot 为空。两个标签页抢同一个会话时会互相抢走查看者。内存里的 snapshot 事件只活到该 runtime 为止。

## Testing

网关测试覆盖查看者关闭后 PTY 仍在、重新 bind 时回放 running snapshot、只在 `{op:"shutdown"}` 时停下 PTY，以及 `?session=` 复用而不二次 spawn。客户端测试覆盖 `close` 后重连、应用 snapshot fold、轨上关闭发送 `{op:"shutdown"}`、host boot meta 匹配时从 localStorage 恢复命名会话、不匹配时从 Chat 1 起算，以及忽略畸形 snapshot 帧。
