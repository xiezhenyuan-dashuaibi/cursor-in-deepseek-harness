...
| `dsh_web_search` / `dsh_web_fetch` | The normalized `WebSearchResult` / `WebFetchResult` |
...
| `dsh_terminal_open` / `dsh_terminal_list` / `dsh_terminal_send` / `dsh_terminal_read` / `dsh_terminal_signal` / `dsh_terminal_close` | Public session snapshots, bounded read/send DTOs, signal/close outcomes, or a background job handle |
| `dsh_job_output` / `dsh_job_list` / `dsh_job_kill` | Public task snapshots without owner or notification bookkeeping |
...
| `dsh_todo_write` | `{ todos, counts }` |
| `dsh_ask_user_question` | `{ answers: [{ id, selected, custom? }] }` |
| `dsh_exit_plan_mode` | `{ approved: true }` |
...
| `dsh_run_code` | `{ logs: string[], result?: JsonValue }` |
