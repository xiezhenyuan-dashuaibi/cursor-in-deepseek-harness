...
独立 fiber 是现场 **profile** 上的 Loader 行：`plugins/<id>/package.json` 有 `dsh.client`、没有 `overlayBody`，且不是受保护 id（`ui-float-window`、`ui-overlay-desktop`、`ui-overlay-shaped`、`ui-cursor-agent` / `cursor-agent`、overlay-card RPC id、`overlay-plugin-roster-rpc`、`overlay-plugin-rail-rpc`）。桌面基模也会按 npm 名 `@deepseek-ai/dsh-client-ui-overlay-desktop` 跳过。异形基模按 npm 名 `@deepseek-ai/dsh-client-ui-overlay-shaped` 跳过。桌面占用者是同一次扫描里 `overlayBody: overlay-desktop.body` 的行。异形占用者是同一次扫描里 `overlayBody: overlay-shaped.body` 的行，列为 `shaped`（隐藏加拔出；[异形隐藏](2026-09-14-overlay-shaped-hide.md)）。标题优先用非空的 `dsh.client.panelTitle`，否则用 Loader id。不列出捆绑的 DSH 铬框。已插入的占用者不在下列表重复。
...
**占用 `overlay-card.body`，让背景变成卡片。** 否决 — 穿透桌面页占据 `overlay-desktop.body`；非卡片轮廓占据 `overlay-shaped.body`。

**给 fiber 单独做隐藏文件，与 Loader `disabled` 分开。** 对普通 fiber 否决 — 那些行没有隐藏标志；轨只提供插入/拔出。桌面产品也没有隐藏文件（[桌面基模](2026-09-10-overlay-desktop-host.md)）。异形占用者使用宿主 `hidden.json`（[异形隐藏](2026-09-14-overlay-shaped-hide.md)）。
