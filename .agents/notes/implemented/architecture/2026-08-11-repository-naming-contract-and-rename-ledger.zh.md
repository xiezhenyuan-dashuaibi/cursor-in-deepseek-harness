...
| `task_output`, `task_list`, `task_kill` | `dsh_job_output`, `dsh_job_list`, `dsh_job_kill` | 这些模型工具操作的是作业，而不是用户任务。`run_in_background` 返回 `JobId`。 |
...
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | 这些名称说明唯一受支持的交互形式。保留 `AskUserQuestion*`、`dsh_ask_user_question` 工具和 `@deepseek-ai/dsh-tool-ask-user`。 |
