...
宿主 `/overlay-card`（以及现场 `./overlay-card-plug-rpc.mjs` 的 `/overlay-card-plug`）提供 `instances.setHidden` 和 `occupants.setInserted`。卡片包的 `overlay:live update` 会写 `./overlay-card-hide-rpc.mjs`，并把已有的 plug-rpc Loader `name` 改到该 specifier，以便已经 import 过第一个 plug-rpc URL 的进程在 `/overlay-card-plug` 上重新挂上这些处理。`setInserted` 拒绝不在该卡 `occupants` 上的 id，拒绝桌面 / Cursor / RPC 辅助 id（`ui-float-window`、`ui-cursor-agent` / `cursor-agent`、`overlay-card-roster-rpc`、`overlay-card-plug-rpc`、`overlay-card-hide-rpc`、`overlay-card-rpc`），并使用 trusted-host 权限。`overlay:live` 解析/转储会保留插入行上的 `disabled: true`，因此之后再插入别的包不会把已拔出的占用者重新启用。

Cursor 插件浮层列出 `{ id, title, hidden, inserted, occupants }`，并同时提供隐藏/显示和插入/拔出。Overlay 叠放与轨铬框仍由 [overlay 叠放](2026-09-07-overlay-stack-and-card-plug.md) 拥有。
