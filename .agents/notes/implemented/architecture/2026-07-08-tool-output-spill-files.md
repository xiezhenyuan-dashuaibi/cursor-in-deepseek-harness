...
## Showcase: dsh_web_fetch

`dsh_web_fetch` is the first showcase because it returns a naturally large text result and needs no tool-specific spill code. The tool is ordinary:
...
- `dsh-tool-web` integration drives `dsh_web_fetch` through `ctx.tools.execute` with the real `spill-local` backend + policy, proving the model-facing text changes only by the deliberate spill notice while the spill file holds the full formatted result.
...
**Use `ctx.fs.writeText` or the model-facing `dsh_write` tool.** Rejected: workspace filesystem writes carry project-file semantics, write/edit policy, observation state, and user-facing side effects. Spill files are runtime artifacts, not model-authored workspace edits. The existing `dsh_read` tool may inspect them later, but creation belongs to the runtime spill seam.
