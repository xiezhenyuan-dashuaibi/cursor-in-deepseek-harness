...
`dsh_web_fetch` and `dsh_web_search` are migrated. `dsh-tool-web` keeps ownership of their model-facing schemas, and those schemas expose no timeout knob: `dsh_web_fetch` dropped its `timeout_ms` parameter to match the reference-agent shape, and `dsh_web_search` stays query-only. The tool bodies do not import `@deepseek-ai/dsh-timeout`; they forward `exec.signal` to `ctx.web`.
...
`read`, `write`, `edit`, `dsh_todo_write`, `dsh_job_list`, and `dsh_job_kill` do not opt into tool-call timeout. `dsh_job_output` owns its bounded wait because a wait timeout is a successful live-status result, not a tool failure.
