...
3. `@deepseek-ai/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `dsh_web_search` and `dsh_web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
- `dsh_web_search` is registered when web search is enabled for the product/app, `dsh_web_fetch` when web fetch is.
...
flowchart LR
  ...
  toolWeb -->|ctx.tools.register| webSearch["tool: dsh_web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: dsh_web_fetch"]
```
...
The `dsh_web_search` model-facing tool is small. The only model-facing argument is:
...
SSRF / private-network protection (blocking private, loopback, link-local, multicast, and otherwise non-public destinations, with DNS-resolve-then-validate to defeat rebinding and per-hop re-validation on redirects) is **deferred** — see [Deferred work](#deferred-work). Until it lands, `dsh_web_fetch` is an SSRF primitive and must not be enabled in a deployment that can reach sensitive internal network targets.

## Tool consumer behavior

`dsh-tool-web` owns two `ToolDefinition`s: `dsh_web_search` and `dsh_web_fetch`. It owns model-facing JSON schemas, snake_case argument names, prompt sections, result rendering to `ContentBlock[]`, `presentCall`, and `presentResult`.
...
The prompt guidance explains the semantic split — `dsh_web_search` for discovery and current information, `dsh_web_fetch` when the model needs the content of a specific URL — and the prompt and tool result tell the model to cite relevant URLs with markdown links.
...
**Fetch is a network boundary, not just a read-only tool.** `dsh_web_fetch` can reach sensitive network targets or exfiltrate data through URLs. Only the basic transport hygiene ships (http/https-only, credential rejection, byte/time caps, cross-origin redirect blocking); SSRF / private-network blocking is deferred (see [Deferred work](#deferred-work)), so until it lands `dsh_web_fetch` must not be enabled where it can reach internal targets.
...
- SSRF / private-network protection for `dsh_web_fetch`: block private, loopback, link-local, multicast, and otherwise non-public destinations so `dsh_web_fetch` is not an SSRF primitive. Doing it correctly is more than a URL-string check — it needs DNS-resolve-then-connect-to-the-validated-IP (to defeat DNS rebinding / TOCTOU), per-hop re-validation across redirects, and IPv6 edge handling (private ranges, IPv4-mapped addresses). Neither reference implementation surveyed does IP-level blocking (OpenCode does a prefix check then fetches; Claude Code relies on a centralized hostname blocklist plus a "private URLs will fail" prompt), so there is no implementation to copy and this is the harness's only SSRF defense — it warrants its own focused design/spike. Until it lands, `dsh_web_fetch` must only be enabled in deployments that cannot reach sensitive internal targets.
...
- Provider-backed extraction as a separate `web_extract` capability, rather than widening `dsh_web_fetch` silently.
