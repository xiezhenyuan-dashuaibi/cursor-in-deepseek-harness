...
A one-shot subagent that calls `dsh_ask_user_question` can block indefinitely. The call waits for a human answer, but the child has no independently owned human channel, so the child's completion and the parent waiting on that completion both stall.

Durable session lineage cannot decide whether an answerer exists. A child session may later be resumed as a new top-level runtime root, while a live runtime-owned child may carry a zero or absent durable delegation depth. Error guidance at the shared seam must also fit every consumer: `dsh_exit_plan_mode` uses `ctx.userQuestions.ask()` without calling `dsh_ask_user_question`.
...
**Reject only inside `dsh-tool-ask-user`.** Rejected because `dsh_exit_plan_mode` and direct callers share `ctx.userQuestions.ask()`. The service is the narrow operation boundary common to every human-interaction consumer.
