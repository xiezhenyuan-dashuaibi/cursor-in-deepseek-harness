...
- 公开名称是会话历史和权限/配置 API 的一部分；命名算法是由测试固定的 v1 约定，发布后变更即为破坏性变更。导入的 MCP 名称不进入已交付的 `dsh_*` 目录采集（[面向模型的工具名](../architecture/2026-08-31-model-facing-tool-names-dsh-prefix.md)）。
