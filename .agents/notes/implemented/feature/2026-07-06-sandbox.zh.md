...
- **授权的升级会持久化吗？** 不会。授权由发起请求的确切前台或后台调用消费；每个相邻调用保留自己的有效模式。后续的后台拒绝通过 `dsh_job_output` 呈现，并且可以作为一次新的精确命令重试的依据。
