...
**A Codex-style asynchronous spawn/wait redesign.** Continuable children plus `dsh_send_message` already provide the asynchronous channel; rebuilding the foreground contract around a mailbox would discard a working synchronous result path to solve a scheduling problem one declaration fixes.
