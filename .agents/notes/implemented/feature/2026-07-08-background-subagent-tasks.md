...
Background subagents use the [generic background job runtime](../architecture/2026-06-20-generic-long-running-tool-runtime.md). Collection, listing, cancellation, completion notices, and prompt guidance come from `dsh_job_output`, `dsh_job_list`, and `dsh_job_kill`; there are no subagent-specific companion tools.
...
- `readOutput` is absent. While live, `dsh_job_output` returns status only; after settlement, it returns final output idempotently. Intermediate child activity remains in the child session.
...
The generic task prompt teaches the shared habit: retain ids, continue independent work instead of busy-polling, collect relevant tasks before answering, and kill irrelevant work. The subagent schema adds only that background mode returns a job id and that `dsh_job_output` collects the result. Authorization and owner cleanup enforce the runtime boundary independently of prompt compliance.
