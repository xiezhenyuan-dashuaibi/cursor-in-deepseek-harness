...
`ctx.jobs` already runs every long-lived piece of work the harness starts in the background — `bash`, `pwsh`, `pty-send`, and one-shot background subagents — but its only reader was the model. [`dsh-tool-jobs`](../../../../packages/jobs/tool-jobs/README.md) exposes `dsh_job_list`, `dsh_job_output`, and `dsh_job_kill`, and nothing else observed the registry.
...
**No web path calls `ctx.jobs.read()`.** It consumes the single output cursor, so a browser read would silently take bytes the model's `dsh_job_output` will never see. This is an invariant worth a test rather than a convention, because the failure is invisible at the call site.
...
**`stopping` is nearly unreachable today.** Only the model's `dsh_job_kill` produces it, so the state is rendered but rarely seen until human cancellation lands. It is in the union now because leaving a status out would have made that phase a wire change.
