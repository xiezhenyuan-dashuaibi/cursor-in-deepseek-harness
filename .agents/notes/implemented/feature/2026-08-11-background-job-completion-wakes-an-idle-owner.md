...
The gap was recorded as a limitation rather than reasoned about, so the fallback was `dsh_job_output(wait: true)` — the blocking wait the same prompt discourages.
