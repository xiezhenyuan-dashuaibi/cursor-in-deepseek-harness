...
Code Mode remains outside this scheduler because the model emits one native `dsh_run_code` call. `dsh_run_code` and its internal dispatch queue remain serial; native sibling calls in `mode: 'both'` use the normal scheduler.
