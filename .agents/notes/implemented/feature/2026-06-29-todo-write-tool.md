# Agent Note: The `dsh_todo_write` tool — model task list as event-sourced session state
...
- **Full-loop integration** — a scripted mock model calls `dsh_todo_write` through the real agent loop; the `todo/write` event lands and a second call replaces it.
- **Resume/replay** — a persisted `todo/write` folds back into the current task list.
- **With-key e2e + snapshots** — a real prompt induces `dsh_todo_write`; assembled snapshots pin the log event and interactive rendering.
...
- **Tool in `core/`** — `dsh_todo_write` is an extension tool registering on `ctx.tools`, not part of the spine; it lives in its own `packages/todo/` group like other tool families.
