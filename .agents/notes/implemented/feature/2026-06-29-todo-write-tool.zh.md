# Agent Note: `dsh_todo_write` 工具——将模型任务列表作为事件溯源的会话状态
...
- **全循环集成**——一个脚本化的 mock 模型通过真实 agent loop（智能体循环）调用 `dsh_todo_write`；`todo/write` 事件落地，第二次调用替换它。
- **恢复/回放**——持久化的 `todo/write` 折叠回当前任务列表。
- **带密钥 e2e + 快照**——真实提示词诱导 `dsh_todo_write`；组装后的快照固定日志事件和交互式渲染。

## 曾考虑的替代方案

- **内存中的 `ctx.todos` 服务**——需要重新发明日志免费提供的持久性、回放和恢复重建。
- **逐项 delta 协议**——仅在共享多所有者列表时需要，超出当前范围；整列表替换更简单，且与参考实现一致。
- **工具放在 `core/` 中**——`dsh_todo_write` 是注册在 `ctx.tools` 上的扩展工具，不属于主干；它像其他工具族一样位于自己的 `packages/todo/` 分组中。
