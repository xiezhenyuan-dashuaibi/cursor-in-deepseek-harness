...
agent-core 主干包含一个目录贡献者、一个本地提供方和一个面向模型的工具。skill 发现是 cwd 敏感的，因此以不同会话 cwd 值创建 agent 的调用方可以按设计观察到不同的项目 skill 覆盖。Cursor MCP 通过额外工具投影发布相同的模型可调用条目，因为该客户端收不到 pre-step 目录；[MCP skill 目录笔记](../bug-fix/2026-09-04-mcp-skill-catalog-projection.md) 拥有该通道。
