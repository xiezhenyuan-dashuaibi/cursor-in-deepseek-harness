...
`@deepseek-ai/dsh-tool-session-query` 是 `ctx.sessionQuery` 面向模型的消费方。它注册五个职责单一的只读工具：`dsh_session_search`、`dsh_session_event_search`、`dsh_session_trace`、`dsh_session_event_trace` 和 `dsh_session_event_read`。该包依赖接口而非 SQLite 实现，负责模型参数校验与易读文本渲染，并贡献一个精简的提示词段，说明历史搜索以及从搜索转向追踪／读取的工作流。
...
`dsh_session_search` 按会话聚合全文匹配，并公开带类型的会话与事件元数据过滤条件。`dsh_session_event_search` 搜索一个会话，默认目标为调用者的当前会话。`dsh_session_trace` 返回完整的已授权祖先链与递归后代树。`dsh_session_event_trace` 返回一个事件所有已知的位置替换关系与直接引用来源事件的关系。`dsh_session_event_read` 以未删节的 JSON 原样返回目标事件，并可选择汇总一个有界的原始事件窗口；省略 `before` 与 `after` 时只返回目标。
...
搜索工具公开的是既往工作，而不是正在执行搜索的操作本身。`dsh_session_search` 排除调用者会话。`dsh_session_event_search` 以调用者会话为目标时，会把请求的序号范围与当前 `step/start` 之前的最后一个事件取交集，从而排除当前 assistant 消息、工具调用，以及从该次调用中建立索引的查询参数。
