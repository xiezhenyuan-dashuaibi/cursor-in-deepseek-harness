...
后台 bash 任务携带一个不透明的 owner token，其值等于所属会话 id。`dsh_job_output` 和 `dsh_job_kill` 在读取或终止之前，将调用方的 token 与执行器的任务归属进行比较；仅凭可预测的 job id 不能获得访问权。归属信息与执行器任务一起存储，因此工具插件重载不会擦除它。
