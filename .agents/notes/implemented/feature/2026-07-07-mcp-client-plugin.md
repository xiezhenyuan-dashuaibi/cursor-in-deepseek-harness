...
- Public names are part of session history and permission/configuration APIs; the naming algorithm is a v1 contract pinned by tests, and changing it after release is a breaking change. Imported MCP names stay outside the shipped `dsh_*` catalog harvest ([model-facing tool names](../architecture/2026-08-31-model-facing-tool-names-dsh-prefix.md)).
