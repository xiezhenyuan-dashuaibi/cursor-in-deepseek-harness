...
The existing `dsh_bash`, `dsh_read`, `dsh_write`, and `dsh_edit` tools remain the reliable default for bounded, auditable operations. A PTY is an additional capability for work that genuinely requires terminal state, not evidence that those tools are defective or candidates for removal.
...
There are no plugin-load auto-start sessions. `dsh_terminal_open` creates a session only during an agent tool call, when ownership and the owning event-sourced session are known. A future declarative startup feature must compose through unpublished agent setup rather than create shared global terminals.
...
| Tool | Purpose | Result |
|---|---|---|
| `dsh_terminal_open` | Create an owner-scoped session from a registered backend type | `{ sessionId, name, type, motd }` |
| `dsh_terminal_send` | Send text, optionally submit Enter, and wait for readiness or register a background job | bounded viewport plus wait and session status; background also returns `jobId` |
| `dsh_terminal_read` | Read a bounded page from retained scrollback | `{ text, totalLines, lineBegin, lineEnd, truncated }` |
| `dsh_terminal_signal` | Send one allowed signal to the current foreground process group | `{ delivered, targetPgid }` |
| `dsh_terminal_close` | Close one session and await process-tree quiescence | `{ killed }` |
| `dsh_terminal_list` | List the caller's live sessions | owner-scoped session summaries |

The UI render contract is exact and location-free. `dsh_terminal_send` uses terminal call/result cards only for foreground sends; its background form is generic `execute`. `dsh_terminal_open`, `dsh_terminal_read`, `dsh_terminal_signal`, `dsh_terminal_close`, and `dsh_terminal_list` use generic `execute`, `read`, `execute`, `delete`, and `read` cards respectively. No PTY tool emits `locations`.
...
With `run_in_background: true`, `dsh-tool-terminal` registers the in-flight send on `ctx.jobs` and returns immediately with `jobId`. The producer places `maxResultBytes` on the task snapshot so `dsh_job_output`, terminal kill status, and completion notices enforce the same complete-result cap after generic metadata. `dsh_job_output(wait: true)` waits, reads incremental output, and records the final result; `dsh_job_kill` resolves the current foreground PGID and delivers a real `SIGINT`, including when the application has disabled terminal `ISIG`, and escalates only through the PTY backend's owned teardown path. If the task surface is absent, background mode fails before writing input. No PTY-specific `sleep` tool or general wake-up API is added.

`dsh_terminal_read` pages backward from the newest retained line. The backend enforces both line and UTF-8 byte caps on retained scrollback and the returned page payload, so one oversized line cannot bypass the backend bound; the tool then caps the fully rendered page including pagination and truncation metadata. `truncated` distinguishes retention loss from an ordinary viewport delta.

`dsh_terminal_signal` accepts the closed set `SIGINT | SIGTERM | SIGKILL | SIGTSTP | SIGHUP`. The backend resolves the terminal foreground process group at execution time. `SIGKILL` is rejected when that group is the top-level shell, directing the caller to `dsh_terminal_close`; a failed group lookup fails the operation instead of signaling a guessed PID.
...
**Persistent terminal state is available without weakening one-shot tools.** Shell and REPL state can survive tool calls, while `dsh_bash`, `dsh_read`, `dsh_write`, and `dsh_edit` retain their narrower validation, approval, and replay contracts.
