...
The agent-core spine includes one catalog contributor, one local provider, and one model-facing tool. Skill discovery is cwd-sensitive, so callers that create agents with different session cwd values can observe different project skill overrides by design. Cursor MCP publishes the same model-invocable entries through the extra-tool projection because that client never receives the pre-step catalog; [the MCP skill-catalog note](../bug-fix/2026-09-04-mcp-skill-catalog-projection.md) owns that channel.
