...
- `tool-subagent` 的工具名是 `config.toolName ?? 'dsh_subagent'`——加载时选定，并非字面量。
...
`packages/*/tool-*` 下已发布的产品工具包，每个都使用默认配置启动，包括 `dsh-tool-bash`（`bash`）、`dsh-tool-jobs`（`dsh_job_output`、`dsh_job_list`、`dsh_job_kill`）和 `dsh-tool-subagent`（`subagent`）。仅供示例使用的工具不在范围内。

目录的单位是包，而非经过配置的每个工具实例。每个包以默认配置启动一次；加载时的别名（如 `dsh_subagent_fork`）会注明，但不枚举所有部署配置组合。部署清单覆盖的是一个独立且无界的范围。
