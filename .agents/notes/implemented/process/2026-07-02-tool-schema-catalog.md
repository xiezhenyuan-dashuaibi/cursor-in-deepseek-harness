...
- `tool-subagent`'s tool name is `config.toolName ?? 'dsh_subagent'` — chosen at load, not a literal.
...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `dsh-tool-bash` (`bash`), `dsh-tool-jobs` (`dsh_job_output`, `dsh_job_list`, `dsh_job_kill`), and `dsh-tool-subagent` (`subagent`). Example-only tools are excluded.

The catalog unit is a package, not every configured tool instance. Each package boots once with default config; load-time aliases such as `dsh_subagent_fork` are noted without enumerating every deployment permutation. A deployment inventory is a separate, unbounded surface.
