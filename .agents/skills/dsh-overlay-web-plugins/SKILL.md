...
description: "Use when writing card-window overlay frontend on dsh web (卡片式, 卡片窗口, overlay-card, ui-float-window, preferFrame). Declare dsh.client.overlayBody; dual-face host+page is one Loader row so the Cursor rail can hide/unplug the whole card. This skill is only the card form. For arbitrary-shape floaters (精灵, 电视悬件) load dsh-overlay-shaped-plugins; for full-page white canvas (整页画板) load dsh-overlay-canvas-plugins. Never restart dsh web."
---
...
| Full-page white canvas (not floating) | [`dsh-overlay-canvas-plugins`](../dsh-overlay-canvas-plugins/SKILL.md) | Reserved slot |

Do not occupy `overlay-card.body` for a sprite, a TV-shaped widget, or any other non-card outline. Do not treat this skill as the page-canvas path. If the task is a reserved form, load that skill; do not invent its procedure from this file.
...
A product must not occupy `shell.overlay` with its own chrome. Additional windows are instances of this card, each with an insert-time unique id. Arbitrary-shape floaters and the page canvas are the other two forms.
...
The Cursor rail lists **cards**, not arbitrary Loader rows. 隐藏 skips the window (`hidden`). 拔出 sets occupant `disabled` on the live profile patch and unmounts the **whole card** while `inserted` is false. It does not delete checkout. Title-bar 缩小 is viewing chrome that collapses the card into an edge tag, not these flags ([edge tag](../../notes/implemented/architecture/2026-09-07-overlay-card-edge-tag.md)). Rationale: [hide vs Loader disabled](../../notes/implemented/architecture/2026-09-07-overlay-card-hide-and-loader-disabled.md).
...
4. Do not reuse Loader ids `ui-float-window`, `ui-cursor-agent` / `cursor-agent`, or `overlay-card-*-rpc`. Do not disable those rows. Do not occupy `root` or `shell.overlay` with product chrome.
...
- Write a designed webpage (type hierarchy, spacing, filled primary actions, `prefers-reduced-motion`). Sparse plugin chrome is not acceptable.
...
Build `lib/` in the checkout package (`pnpm --filter @deepseek-ai/dsh-client-<name> bundle`). Do not run `tsc -p packages/client/<name>/tsconfig.json` alone: that project is not a composite emit root.
...
Repeat `insert packages/client/ui-float-window` appends another card (at most 8) without copying `lib/` again (that would remount existing cards). The same command writes `$DSH_HOME/profiles/web/overlay-card-roster-rpc.mjs` and a Loader row when that roster channel is not already present, and `./overlay-card-plug-rpc.mjs` so `instances.setHidden` and `occupants.setInserted` can mount on `/overlay-card-plug`. Card `update` retargets that Loader row to `./overlay-card-hide-rpc.mjs` when the first plug-rpc specifier is already cached. Omitted flags take `卡片`, the next seat as `--card-id`, and 360×280. Unload one window with `pnpm overlay:live remove overlay-card-<id>`. Unload the whole desk with `pnpm overlay:live remove ui-float-window` (or that Loader id). The Cursor plugin panel hide/show writes `hidden`; insert/unplug writes occupant `disabled` on the live profile patch. That UI does not call `overlay:live remove`.
